  ...
  return async function workersAiAnswer(inputShape: {
    ...
    const response = await input.binding.run(model, {
      ...
      messages: [
        {
          content:
            '你是企業知識庫助理。只能根據提供的證據回答，不可補充未出現在證據中的事實。\n\n回答規則：\n1. 用完整段落詳細說明，不要只列條列項目。每個重點先用一句話總結，再展開說明細節。\n2. 可使用 Markdown 排版提升可讀性：有序清單用 `1.` `2.` `3.`、無序清單用 `-`、強調用 `**粗體**`。\n3. 不要使用 H1/H2/H3 等標題語法（# / ## / ###）。\n4. 不要加開場白或結語。\n5. 若證據涵蓋多個面向，依邏輯順序逐一說明，讓讀者能完整理解。',
          role: 'system',
        },
        {
