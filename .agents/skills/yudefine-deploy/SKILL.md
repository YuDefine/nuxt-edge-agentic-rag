...
>
> **void.cloud 平台 SOP**（CLI 命令、`env.ts`、binding helper API、自動 migration、resource inference 等通用知識）走 **official `void` skill**（`.agents/skills/void/`）或 **`void mcp`**（`search_docs` / `get_page` / `list_pages`），與 `void` npm package version lockstep。

## Step 0 — 詢問 deploy target（**必跑**）

向使用者用 `request_user_input` 確認 deploy target，不要從專案名 / fleet 慣例直接推斷：
...
- symlink `.agents/skills/void` + `.agents/skills/migrate-vite-cloudflare-to-void`（跟 `void` package version 對齊）
...
- 所有 void CLI 命令、config 細節、API surface、runtime helper、resource inference、auth flow、env.ts 等問題 **MUST** 走以下任一：
  - `.agents/skills/void/SKILL.md` + `docs/**/*.md`（symlink 跟 void package version 對齊）
  - `void mcp` query (`list_pages` / `get_page docs/<path>.md` / `search_docs <keyword>`)
- 從 Vite + `@cloudflare/vite-plugin` 遷 void：走 `.agents/skills/migrate-vite-cloudflare-to-void/`
...
`void.json` + `wrangler.jsonc` 的 `compatibility_date` **MUST** 對齊 void Nuxt 官方範例（`.agents/skills/void/docs/integrations/frameworks/nuxt.md`）。2026-07-14 驗證的官方範例為 `2026-02-24`。
...
- **CF 帳號** + 對應 API token 已加進 GitHub repo secrets（`CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID`），per `~/.claude/rules/secrets.md`
- **D1 / R2** 已在 CF 帳號建好（手動或透過 Cloudflare MCP `d1_database_create` / `r2_bucket_create`）
- consumer 走 NuxtHub 派（`@nuxthub/core` + `hub: {}`）或 raw wrangler.jsonc 派，per `~/.claude/rules/cloudflare-workers.md`
...
兩種 pattern 擇一，per `~/.claude/rules/cloudflare-workers.md` § 4：
