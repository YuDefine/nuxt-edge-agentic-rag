...
- [ ] 1.3 Commit 批次 1（單獨 commit，方便之後 revert）

## 2. 批次 2：app/ 與 server/ 原始碼導入 alias（Alias-based cross-module imports 第一階段）

- [ ] 2.1 [P] 改 `app/composables/useUserRole.ts`：`../../shared/schemas/knowledge-runtime` → `#shared/schemas/knowledge-runtime`（Alias-based cross-module imports 第一筆）
- [ ] 2.2 [P] 改 `server/auth.config.ts`：`../shared/schemas/knowledge-runtime` → `#shared/schemas/knowledge-runtime`
- [ ] 2.3 [P] 改 `server/plugins/sync-admin-roles.ts`：`../utils/knowledge-runtime` → `#server/utils/knowledge-runtime`
- [ ] 2.4 [P] 改 `server/utils/**.ts` 中 13 處 `../../shared/...` 為 `#shared/...`（檔案：`allowlist.ts`、`document-list-store.ts`、`document-publish.ts`、`document-store.ts`、`document-sync.ts`、`knowledge-answering.ts`、`knowledge-retrieval.ts`、`knowledge-runtime.ts`、`mcp-ask.ts`、`mcp-auth.ts`、`mcp-token-store.ts`、`web-chat.ts` 等）
- [ ] 2.5 [P] 改 `server/api/**/*.ts` 中所有 `../../../utils/...`、`../../utils/...` 為 `#server/utils/...`（約 20+ 檔）
- [ ] 2.6 跑 `pnpm typecheck` 全綠
- [ ] 2.7 跑 `pnpm test`（所有 project）全綠
- [ ] 2.8 跑 `pnpm lint` 全綠
- [ ] 2.9 搜尋殘留：`grep -rE "from ['\"]\.\.\/" app/ server/ shared/` 應只剩 sibling 相對路徑（`./xxx`）或本次規則允許的 within-folder 匯入
- [ ] 2.10 Commit 批次 2
