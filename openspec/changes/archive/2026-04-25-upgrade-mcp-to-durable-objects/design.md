...
**Decision**：採 **Pivot C**（2026-04-24 使用者確認）。關鍵發現降低 C 成本：MCP SDK 的 `Transport` interface（`start / send / close + onmessage`）極簡，自寫 shim 約 30 行即可，不必實作完整 MCP Streamable HTTP spec——request 解析 / handler 派遣 / response 組裝仍由 SDK `Protocol` 基類處理。我們的 shim 只做「HTTP request ↔ JSONRPCMessage 橋接」，**從未接觸 `env` proxy**，根除 `Reflect.ownKeys` bug 家族。

詳見 `docs/solutions/mcp-streamable-http-session-durable-objects.md` § Pivot Decision — C。

下面其他 Decisions 仍成立但**建立在 Pivot C 已選定的前提上**。
...
### 採用自寫 minimal `DoJsonRpcTransport`（Pivot C 實作）

- 新建 `server/mcp/do-transport.ts`：`DoJsonRpcTransport` class 符合 SDK `Transport` interface（`start` / `send` / `close` + `onmessage` callback），約 30 行
  - `send(msg)`：把 JSON-RPC response 放入 per-request resolver，由 DO fetch handler 收集
  - `onmessage` 由 `McpServer.connect()` 設定
- DO fetch handler：parse request body → `transport.onmessage(msg, { authInfo, requestInfo })` → SDK Protocol 基類自動派 `_requestHandlers.get(method)` → 我們註冊的 tool handler → handler 回傳 response → SDK 呼 `transport.send(response)` → resolver 吐出 → 回 HTTP JSON response
- Per-session `McpServer` 實例在 DO 內 lazy init（首次收 `initialize` 時 `server.connect(transport)`）
- **關鍵**：transport shim **從未讀取 `env` 或反射 env proxy**；SDK 所有 handler 走 `Protocol._onrequest` → `_requestHandlers`，不經 env access 路徑
- 替代方案（已 rejected）：
  - `WebStandardStreamableHTTPServerTransport` on DO → Phase 1 spike 實證爆 `Reflect.ownKeys(env)`
  - `McpAgent` on DO → `agents/mcp` WorkerTransport shim 註解已明示同家族問題
...
### Phase 2 PoC：評估 McpAgent on DO 取代自寫 transport — Superseded

~~原計畫：建立最小 DO class 搭 `McpAgent`，跑 `tools/call` 判斷是否仍觸發 `ownKeys` error。~~

**2026-04-24 superseded**：Pivot C 決策下不需要這個 PoC。Phase 1 spike 已實證 `WebStandardStreamableHTTPServerTransport` 也爆同家族 ownKeys 問題，且 `agents/mcp` WorkerTransport 在 shim 註解已記錄同問題，結論相同——任何 SDK 內建 transport 都會碰 env proxy。Pivot C 改為自寫 `DoJsonRpcTransport`（見上一個 Decision），本 PoC 保留為歷史記錄。
