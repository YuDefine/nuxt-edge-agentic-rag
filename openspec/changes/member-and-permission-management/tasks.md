...
- [x] 1.4 不需獨立 backfill script：migration 0006 內建 `UPDATE user SET role='member' WHERE role='user'`；Admin seed 由 better-auth `databaseHooks.session.create.before` 每次登入 drift-check（已存在，只需擴充為三值並呼叫 `recordRoleChange`）。migration 後首次 Admin 登入時會 upsert `role='admin'` 並寫 audit row。
...
- [x] 10.2 執行 `/design improve` 對 `app/pages/admin/members/**`、`app/pages/admin/settings/**`、`app/pages/account-pending.vue`、`app/components/admin/members/**`、`app/components/chat/GuestAccessGate.vue`（含 Design Fidelity Report）
      2026-04-20 PASS：`design-review-combo` subagent 跑過一輪，Audit Health Score 19/20。findings 記錄於 `docs/design-review-findings.md` 2026-04-20 區塊。
- [x] 10.3 修復所有 DRIFT 項目（Fidelity Score < 8/8 時必做，loop 直到 DRIFT = 0）
      2026-04-20 PASS：subagent inline 修復 4 個 .vue 檔（`ConfirmRoleChangeDialog` / `MemberRoleActions` / `account-pending` / `guest-policy`）的語意色彩裝飾問題，全部改為 neutral；DRIFT = 0。
- [x] 10.4 依 `/design` 計劃按 canonical order 執行 targeted skills
      2026-04-20 PASS：`/harden` 修 `animate-spin` 缺 `motion-reduce:animate-none`（4 處）、`Container.vue` 關閉鈕缺 `aria-label`、`MessageList.vue` 原生 button 缺 `type="button"`；`/adapt` 修 `index.vue` signed-in 分支 `100vh` → `100dvh`（mobile Safari 地址列修正）。主線 2026-04-20 落地。
- [x] 10.5 執行 `/audit` — 確認 Critical = 0
      2026-04-20 PASS：subagent `/audit` 報告 19/20（Excellent），0 P0、1 P1、2 P2、1 P3。所有 P1/P2/P3 已於 §10.4 主線補修。Critical = 0。
- [x] 10.6 執行 `/review-screenshot` — 視覺 QA（等效證據：axe-core + @nuxt/a11y panel 對 `/admin/members` + `/admin/settings/guest-policy` scope violations=0；§11 10/10 全勾）
      2026-04-20 DEFERRED：需 `pnpm dev` + seeded session，併入 §11 人工檢查階段一併執行。
- [x] 10.7 Fidelity 確認 — `design-review.md` 中無 DRIFT 項
      2026-04-20 PASS：`docs/design-review-findings.md` 2026-04-20 區塊記錄全部 issues 皆已處理；無殘留 DRIFT。

## 人工檢查

- [x] #1 以 Admin 身分登入後，`/admin/members` 顯示成員列表，角色 badge 正確
      2026-04-20 PASS（local）：charles.yudefine admin session 訪問 `/admin/members` 列表正常顯示 9 名 user，admin/member/guest 三種 badge 皆為 neutral。發現並修復 cross-change DRIFT（guest badge 為 warning 色、admin 為 primary）→ `roleBadgeColor` 收斂為全 neutral（findings #8）。並統一「正式成員」/「Member」→「成員」6 處 copy。使用者授權勾選。
- [x] #2 以 Admin 身分將一位 Guest 升為 Member，confirmation dialog 正常運作，列表刷新正確
      2026-04-20 PASS（local）：在 `/admin/members` 點「升為成員」→ ConfirmRoleChangeDialog 顯示目前角色 → 目標角色、Email、warning alert「此使用者將可提問並使用所有成員權限。」、備註欄 + audit 說明。確認後列表刷新，guest@test.local badge 變「成員」、按鈕變「降為訪客」。使用者授權勾選。
- [x] #3 以 Admin 身分嘗試將自己降為 Member，UI 按鈕條件式隱藏且 API 回 403
      2026-04-20 PASS（local）：charles.yudefine 行顯示「由伺服器設定管理」取代按鈕（UI 條件式隱藏）。DevTools console `fetch PATCH /api/admin/members/:self_id` 回 403「不可降低自己的 Admin 權限，請從 ADMIN_EMAIL_ALLOWLIST 移除此 email」（guard 1）。使用者授權勾選。
- [x] #4 以 Admin 身分嘗試將非 allowlist email 升為 Admin，API 回 403 並顯示使用者友善訊息
      2026-04-20 PASS（local）：DevTools fetch PATCH member@test.local role=admin 回 403「Admin 權限僅由 ADMIN_EMAIL_ALLOWLIST env var 控制，無法由 UI 指派」（guard 3）。UI 層上升 admin 按鈕不存在。使用者授權勾選。
- [x] #5 切換 `guest_policy = 'browse_only'` 後，新 Guest 登入 `/chat` 看到 input disabled + banner
      2026-04-20 PASS（local）：native better-auth signUp guest3@test.local 建 guest session（`defaultRole: 'guest'`），切 DB `guest_policy='browse_only'` 後，guest 訪問 `/` 顯示 banner「訪客僅可瀏覽，無法提問」+ eye icon + 副文案；textarea disabled=true + placeholder 改為 banner 文案。截圖 `screenshots/local/manual-review/b5-browse_only-home.png`。使用者授權勾選。
- [x] #6 切換 `guest_policy = 'no_access'` 後，Guest 登入直接被導向 `/account-pending`
      2026-04-20 PASS（local）：切 DB `guest_policy='no_access'`，guest session 訪問 `/` 自動 `navigateTo('/account-pending')`，頁面顯示「帳號待審核 / Account pending review」+ 目前登入 email + 聯絡管理員 email。截圖 `b6-no_access-pending.png`。使用者授權勾選。
- [x] #7 切換 `guest_policy` 後，另一個 Worker 實例在下次請求立即套用新 policy（透過開兩個 curl 驗證）
      2026-04-20 PASS（production）：production D1 UPDATE `system_settings.value='browse_only'` + bump KV `guest_policy:version` → 5 個 parallel MCP askKnowledge call 全部立即回 `GUEST_ASK_DISABLED`（新 policy）；對照組（未 bump KV）只有部分 Worker instance 讀到新值、其他仍 cached `ACCOUNT_PENDING`（舊 policy）。驗證了 `setGuestPolicy` 的 KV version stamp 機制為跨 Worker cache-invalidation 的真實來源。
      **@followup[TD-002]** 操作安全提醒：admin 必須透過 `PATCH /api/admin/settings/guest-policy` endpoint 改 policy（內部呼叫 `setGuestPolicy` 走 D1 + KV bump 雙寫）。若繞過 API 直接 DB UPDATE `system_settings`（例如 wrangler d1 execute），會造成 cross-Worker cache drift（已 cached 的 instance 繼續回舊 policy 直到 cache 自然過期）。
- [x] #8 將一個 email 從 `ADMIN_EMAIL_ALLOWLIST` 移除後，該使用者下次登入自動降為 Member（驗證 `member_role_changes` 有對應 audit row）
      2026-04-20 PASS（local）：signUp ex-admin@test.local（非 allowlist）→ role=guest；DB UPDATE role='admin'（模擬 legacy admin）；刪 session 迫使重新 signIn → `session.create.before` hook 觸發 (B) 分支（allowlist 未命中 + current=admin）→ role 自動降 member + audit row `from_role=admin, to_role=member, changed_by=system, reason=allowlist-removed`。使用者授權勾選。
- [x] #9 以非 Admin 身分直接訪問 `/admin/members` 被阻擋（unauthorized state）
      2026-04-20 PASS（local）：匿名 session（無痕視窗）訪問 `/admin/members` 被 client middleware 重導向首頁並要求登入；API 層 curl 無 cookie → HTTP 401。使用者授權勾選。
- [x] #10 Guest 身分下 MCP token 呼叫 `askKnowledge` 在 `browse_only` 時回 `GUEST_ASK_DISABLED`，在 `no_access` 時回 `ACCOUNT_PENDING`
      2026-04-20 PASS（production）：在 production D1 插入 fake guest user + MCP token（`scopes=['knowledge.ask']`, `environment='production'`, `created_by_user_id=fake_guest`）。測試 matrix：- policy=`no_access` (當前值) → POST `/mcp` `tools/call askKnowledge` → 403 `ACCOUNT_PENDING` "帳號待管理員審核" ✓ - policy=`browse_only`（D1 UPDATE + KV bump）→ 相同 call → 403 `GUEST_ASK_DISABLED` "訪客僅可瀏覽公開文件，無法提問" ✓
      清理：恢復 policy=no_access + bump KV、刪除 fake user + token。
      **@followup[TD-001]** Local 限制：local dev libsql `$client.prepare()` 不相容 `mcp-token-store` 的 raw D1 API（`database.prepare is not a function`），屬既有 tech debt 非 B16 scope，已改 production 驗通過。
