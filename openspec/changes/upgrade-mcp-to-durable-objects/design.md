...
discuss 階段（2026-04-24）透過 MCP SDK `webStandardStreamableHttp.js` 靜態分析得出兩個關鍵結論：

1. **視角翻轉**：Claude re-init 的真因是「沒拿到 `Mcp-Session-Id`」，而非 400 本身。400 只是 re-init 後的 SDK 防守副作用。讓 Claude 停止 re-init 的充分條件是「首次 `initialize` response 帶 `Mcp-Session-Id`」。
2. **SDK 400 路徑壓縮**：POST initialize 只可能觸發 3 條 400 path（`-32700 Invalid JSON` / `-32700 Invalid JSON-RPC message` / `-32600 Server already initialized`）；現行 shim 傳 `sessionIdGenerator: undefined`，`Server already initialized` path 結構上不會命中，因此幾乎必然是 Parse error。
