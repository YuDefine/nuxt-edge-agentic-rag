...
The `notify` job in `.github/workflows/deploy.yml` SHALL contain exactly two steps: a "Build results JSON" pre-step that computes `target` and `results` outputs, followed by a `Notify Discord` step that calls `uses: YuDefine/clade/.github/actions/discord-deploy-notify@v1`. No inline `curl` / `jq` payload assembly is permitted in the notify job body.

#### Scenario: notify job structure

- **WHEN** GitHub Actions parses `deploy.yml`
- **THEN** the `notify` job has exactly two steps in `steps:` — one with `id: build` (or equivalent identifier) and one with `uses: YuDefine/clade/.github/actions/discord-deploy-notify@v1`
...
- name: Notify Discord
  uses: YuDefine/clade/.github/actions/discord-deploy-notify@v1
  with:
    webhook_url: ${{ secrets.DISCORD_WEBHOOK_URL }}
