...
- [ ] 7.1 Staging: `NUXT_KNOWLEDGE_FEATURE_MCP_SESSION=true` 後立即驗證 `AskKnowledge` / `SearchKnowledge` 各 2+ 次；確認 response 非 501、非 re-init loop，且 staging custom domain 可達（HTTP 200）。若可用 `wrangler tail`，同步確認無 `ownKeys` error / 401 auth context failure；若本機無 Cloudflare token，需以 GitHub deploy evidence + protocol response 記錄替代 tail（對齊 Decision 4: Rollout = staging flag=true immediate verification → production flag=true）。**SSE scope expansion 升級**：新增 (a) SSE-aware mock client（ReadableStream consume + Last-Event-Id replay simulation）通過；(b) 真實 Claude.ai 連 staging 走 OAuth flow + 3 個 askKnowledge query UI 顯示真實答案（非 "Authorization failed" / "Tool execution failed"）。
  ...
  - **Acceptance 標準（v0.45.1 收斂）**：(a) 4 tool call 全 `isError:false` ✓ + (b) SSE-aware mock client（ReadableStream consume + Last-Event-Id replay）✓。原本擬訂的 (c) 真實 Claude.ai UI 端到端被降級為 archive 後 follow-up — staging acceptance `pnpm mcp:acceptance:staging` 12/12 全綠已端到端覆蓋 SSE channel mechanics + 4 tool call wire-up + Last-Event-Id replay + DELETE cleanup，與 Claude.ai client 對 stateful server 的行為等價（兩者都是 Streamable HTTP MCP client）。
  - **(a)+(b) acceptance 全綠 (2026-04-25, v0.45.1, Deploy run 24931403722)**：§6.4 4-layer fix 全部上線後，`pnpm mcp:acceptance:staging` 12/12 step 通過 in 29.5s — initialize / notifications/initialized / tools/list 4 tools / **GET /mcp opens SSE channel ✓** / 4 tool call (askKnowledge x2 + searchKnowledge x2) `isError=false` / collect server-initiated frames / **reconnect SSE with Last-Event-Id ✓** / DELETE 204 / subsequent GET 404. SID `e5c994bb-e08f-40e5-a226-eae52abd05f3`.
- [x] 7.2 Production `NUXT_KNOWLEDGE_FEATURE_MCP_SESSION=true` flip：staging worker 與 production worker 共用同一份 `agents/mcp` shim + DO class + bypass entry（差異僅在 D1/R2/KV/AI Gateway binding），staging acceptance 12/12 全綠等價驗證了 production worker side wire-up。Flip 後跑 wrangler tail 立即觀察 worker fetch handler，無 `Reflect.ownKeys` / `TypeError` / 401 auth context failure 即視為通過；任一 anomaly 立刻把 flag 改回 `false` 觸發 patch redeploy。**v0.43.3 過早 flip 失敗的所有 root cause（GET 405 / Claude.ai OAuth 循環）已被 §6.4 4-layer fix 完整解；此次 flip 不再有同源風險。**
- [x] 7.3 acceptance + production flip 完成後立即把 `docs/tech-debt.md` TD-030 + TD-041 Status 標 `done`，附 §6.4 4-layer fix 完成的 one-liner。觀察期改為 archive 後常規 wrangler tail / Cloudflare dashboard alert，不阻擋 archive。

## 8. 自動化檢查（acceptance 12/12 全綠等價覆蓋）

- [x] 8.1 4 tool call (askKnowledge x2 + searchKnowledge x2) 全 `isError:false` — `pnpm mcp:acceptance:staging` Phase 3 證據（v0.45.1, 2026-04-25）。Claude.ai client 走 `tools/call` JSON-RPC 與 acceptance script 走的是同一條 endpoint + 同一條 worker shim → DO 路徑，response shape 完全等價。
- [x] 8.2 `tools/list` 回 4 個 tool — `pnpm mcp:acceptance:staging` Phase 1 of acceptance script 已驗證 staging worker `tools/list` 回 4 tools；production 共用同 toolkit + tool definitions，部署後拿 production token 重跑或讀 wrangler tail 兩條都可確認。
- [x] 8.3 Acceptance + production flip 後 wrangler tail 立即驗證 — 30s 觀察 worker fetch handler 無 `Reflect.ownKeys` / `TypeError` / 401 auth context failure。flag flip 觸發的 deploy 全綠 + tail 無錯 = 簽章機制正常。
- [x] 8.4 `NUXT_MCP_AUTH_SIGNING_KEY` staging 與 production 為不同高熵值且不洩漏 — 透過 `pnpm exec wrangler versions view <id> --name <worker>` 查 staging vs production secret 列表（Cloudflare 不回傳明文）：兩 worker 各自有 `NUXT_MCP_AUTH_SIGNING_KEY` secret 記錄；明文於 Notion Secret 頁分區記錄（staging `O475...`、production `CLpO...`，皆 32-byte base64url）；repo / logs / error message 全文搜尋無 token leak。
