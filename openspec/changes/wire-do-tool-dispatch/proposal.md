...
本 change 不觸動 DB schema、enum、shared types、或 migration；純 MCP 協議層重工。不需要 Entity Matrix。
...
- Truth layer / invariants: Auth context 的 HMAC 簽章 shared secret 必須走 runtime config（`NUXT_MCP_AUTH_SIGNING_KEY` 之類），絕不 hardcode；Nuxt middleware 是 auth 驗證唯一來源，DO 只 verify + consume serialized context；tool handler 行為（成功 / 失敗 / 404 / scope violation）response shape 必須與 stateless path 完全一致（既有 integration test 要同時綠）；`Reflect.ownKeys` workaround 需在 DO runtime 實測驗證是否必要
...
- Affected code:
  - Modified:
    - server/durable-objects/mcp-session.ts（non-initialize path 從 501 error 改為 `transport.dispatch` + McpServer lazy init + auth/env context rebuild）
