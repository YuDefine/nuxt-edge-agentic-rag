...
>
> **v0.52.0 ship 後 pending production verify**（commit `5a477e7` 順手收，未獨立 ship 新 tag）：TD-057 SSE wide event lifecycle child request logger 修法、TD-056/061 judge `max_completion_tokens: 200 → 1024`。下次 prod deploy 後觀察 wrangler tail（warning 應消失 + 子事件帶 `_parentRequestId`）+ D1 `query_logs` 24-48 hr 確認 pipeline_error 比例 < 5%（從 28.6%）。TD-060 retrieval query gap 為下一條 propose 主軸，第二輪 acceptance 依賴 TD-060 解。
>
...
- [high] **TD-061 / TD-056（同源）** judge `max_completion_tokens` 截斷 — fix code 已 commit（`5a477e7`：`workers-ai.ts:135` `200 → 1024` + 2 條 vitest unit test lock），**等下次 production deploy 後驗收**：(a) wrangler tail 觀察 1-2 條 SSE chat，`decision_path=pipeline_error` 比例 ≈ 0；(b) 24-48 hr 後撈 D1 `query_logs` 24h 內 pipeline_error 比例 < 5%（baseline 28.6%）。Production 35 筆 incident 證據保留：`created_at >= '2026-04-26T00:49:30'`
- [mid] **TD-057** evlog wide event lifecycle 警告 — fix code 已 commit（`5a477e7`：`server/api/chat.post.ts` + `server/utils/chat-sse-response.ts` 改用 `createRequestLogger` + `_deferDrain` 開 child request logger，stream settled 後手動觸發 `evlog:enrich` / `evlog:drain` hook 並把 drain promise 註冊到 `cloudflare.context.waitUntil`）。**等下次 production deploy 後驗收**：wrangler tail 觀察 1-2 條真實 SSE chat，warning 應消失，wide event 觀察到 `operation: 'web-chat-sse-stream'` 子事件帶 `_parentRequestId` + `result` / `error` 欄位
