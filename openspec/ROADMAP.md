...
> - **passkey reauth 第二層 mitigation** 已在 local 完成：新增 Better Auth safe logger，把 logging 邊界的 raw args 先轉字串並吞掉 sink serialization failure，後續再進一步收斂成 plain console sink 與單字串 console 輸出、顯式 `disableColors: true`，避免 Worker runtime 再碰到 Better Auth logger 的 color/env probing 或多參數 console trap；新的 unit test、既有 `verify-authentication` integration test、`pnpm check` 與 `pnpm build` 都已通過，本地 `npx wrangler --cwd .output dev` 也已能正常啟動且首頁 canary `200`。本輪再新增 `server/api/auth/passkey/verify-authentication.post.ts` exact route，先以 app route 接管 vendor catch-all，再把 forwarding 路徑從 `auth.api.verifyPasskeyAuthentication({ asResponse: true })` 收斂成 `auth.handler(new Request(...))`，盡量貼近 Better Auth 原生 request contract；runtime gate 與 handler guard 仍保留，確保 passkey 關閉時維持既有 `404` 語意、handler 缺失時回 `503`。進一步的 production live trace 又把 root cause 縮到 Better Auth `setSessionCookie -> setCookieCache -> filterOutputFields(structuredClone(...))` 這條 Worker-only 路徑，因此最新未部署版本已再補 `session.cookieCache.enabled = false`，避免 verify-authentication 在 session mint 後對 adapter proxy row 做 `structuredClone`。local preview 的完整 WebAuthn ceremony 仍會因 `.output` artifact 固化 production origin 而在 `verify-registration` 被 `Unexpected registration response origin` 擋下，因此整組 mitigation 仍需直接 redeploy 到 production 才能判定成敗。
...
- [high] 先 deploy local safe logger + exact route mitigation（最新為 `auth.handler(new Request(...))` forwarding）與 `session.cookieCache.enabled = false` hotfix，重新驗 production `POST /api/auth/passkey/verify-authentication`
