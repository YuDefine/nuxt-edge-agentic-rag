 ...
 *
 * 用途：防止兩個 AI Agent session 同時跑 /commit 造成 staging 撞車、
 ...
 *
 * 注意：AI Agent 每個 Bash tool call 都 spawn 新 process，
...
// 本檔是散播到 consumer `.cursor/scripts/` 的 .mjs，consumer 端沒有那支 TS lib
// 可以 import，因此只能內聯一份。改動任一邊 MUST 同步另一邊。
const RUNTIME_PROBES = [
  [
    'claude',
    ['PROJECT_DIR', 'CLAUDE_SESSION_ID', 'CLAUDE_CODE_SESSION_ID', 'CLAUDE_CONVERSATION_ID'],
  ],
