...
| --- | --- | --- | --- |
| 1 | `rules/core/*.md` | `sync-rules.ts` → `.cursor/rules/<name>.md`，無條件收 | 無 `paths:` → always-load；有 `paths:` → 命中才載 |
...
| 只有 clade 自己（治理、散播機械、clade 自治區紀律） | `.cursor/rules/local/`，**不散播** |
| 只有某一個 consumer，且已是穩定規約 | 該 consumer 自己的 `.cursor/rules/local/`，**不進 clade** |
