...
4. **Select the change**
   ...
   - If continue: run `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- unpark "<name>"` then proceed
   ...
   Wait for the user's response. If the user confirms, run `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- unpark "<name>"` then proceed.
...
4b. **Dispatch 路徑選擇（三選一選單）**
   ...
   **選項 A / B 流程**：
   ...
   1. **組裝 context pack + thin draft prompt**
      ...
      **Step 1b：Write thin draft prompt 到 `/tmp/<runtime>-spectra-ingest-<change-name>-prompt.md`**
      ...
      完成標準：`node .cursor/scripts/spectra-target-guard.ts --change "<change-name>" -- validate "<change-name>"` 通過 + `node .cursor/scripts/spectra-target-guard.ts --change "<change-name>" -- park <change-name>`。
   ...
   **Cross-check（A / B 共用，收到 `<task-notification status=completed>` 後立刻）**：
   ...
   2. 若 draft 已 `node .cursor/scripts/spectra-target-guard.ts --change "<change-name>" -- park <change-name>`：先 `node .cursor/scripts/spectra-target-guard.ts --change "<change-name>" -- unpark <change-name>`。
...
7. **Analyze-Fix Loop** (max 2 iterations)
   ...
   3. If Critical/Warning findings exist:
      ...
      c. Re-run `node .cursor/scripts/spectra-target-guard.ts --change <name> -- analyze <name> --json`
