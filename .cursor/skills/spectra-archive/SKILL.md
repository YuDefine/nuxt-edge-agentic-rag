...
|---|---|---|---|---|
| **Park** | Change is **postponed but still planned**. Premise still valid. | — | — | `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- park <name>` (NOT this skill) |
| **Standard archive** | Implementation `[x]` complete + `## 人工檢查` `[x]` complete | applied | already done | `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- archive <name>` (default flow) |
| **Skip-archive (A)** | **Won't be implemented.** Premise dissolved (decision changed / concept removed / change became redundant). | **skipped** | **skipped** | `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- archive <name> --mark-tasks-complete --skip-specs --no-validate -y` |
| **In-main-done archive (B)** | Implementation `[x]` complete (built directly on main, no session worktree). Only `## 人工檢查` items remain unchecked. | applied | **skipped** | `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- archive <name> --mark-tasks-complete --no-validate -y` |
...
2. **Check artifact completion status**
   ...
   Run `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- status --change "<name>" --json` to check artifact completion.
...
6. **Perform the archive**
   ...
   ```bash
   node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- archive <name>
   ```
...
- Use artifact graph (`node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- status --change "<name>" --json`) for completion checking
