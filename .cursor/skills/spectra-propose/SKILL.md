...
0. **Dispatch 路徑選擇（三選一選單）**
   ...
   **選項 B 專屬 NEVER**：
   ...
   - **NEVER** 在 Phase B-0b 派 Pi review 前，讓 artifacts 處於 parked 狀態（artifacts 在 SQLite blob、不在 disk，Pi 讀不到）。正常流程不會 park；若 draft 違規 park 了，先跑 `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- unpark "<name>"`
...
4. **Create the change directory**
   ...
   ```bash
   node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- new change "<name>" --agent claude
   ```
...
5. **Write the proposal**
   ...
   **IMPORTANT — file path rules for the `## Impact` section:**
   ...
   ```bash
   node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- instructions proposal --change "<name>" --json
   ```

   Generate the proposal content based on change type (see formats below), then write it via CLI:

   ```bash
   node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- new artifact proposal --change "<name>" --stdin <<'ARTIFACT_EOF'
   <proposal content>
   ARTIFACT_EOF
   ```
...
7. **Create remaining artifacts in sequence**
   ...
   - Get instructions:
     ```bash
     node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- instructions <artifact-id> --change "<name>" --json
     ```
   ...
   - Write the artifact via CLI (the CLI handles directory creation and format validation):
     ...
     ```bash
     node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- new artifact <artifact-id> --change "<name>" --stdin <<'ARTIFACT_EOF'
     <content>
     ARTIFACT_EOF
     ```
     ...
     ```bash
     node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- new artifact spec <capability-name> --change "<name>" --stdin <<'ARTIFACT_EOF'
     <delta spec content>
     ARTIFACT_EOF
     ```
   ...
   - After creating each artifact, re-run `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- status --change "<name>" --json`
...
9. **Analyze-Fix Loop** (max 2 iterations)
   1. Run `node .cursor/scripts/spectra-target-guard.ts --change <change-name> -- analyze <change-name> --json`
   ...
   4. If Critical/Warning findings exist:
      ...
      c. Re-run `node .cursor/scripts/spectra-target-guard.ts --change <change-name> -- analyze <change-name> --json`
...
11. **Commit artifacts and end the workflow**（clade fork：不 park，見 `docs/decisions/2026-07-31-propose-does-not-park.md`）
    ...
    **NEVER 在本 skill 執行 park lifecycle command**（2026-07-31 起；決策見 `docs/decisions/2026-07-31-propose-does-not-park.md`）
    ...
    下一步就是開工」是不同狀態；由 **user 在決定擱置某張 change 時顯式** `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- park <name>`。
