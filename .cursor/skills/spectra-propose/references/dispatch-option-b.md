...
## 選項 B 全流程（Phase B-0a Fable draft / B-0b Pi review / B-0c 主線 final check）
   ...
   3. **背景啟動 claude**（**Bash** tool 加 `run_in_background=true`）：
      ...
      預設 text 輸出（主線讀 tail）。
   4. **立刻**簡短回報：「已派 Claude Fable 5.1（effort: xhigh）在背景負責 draft `<change-name>`（bash job `<id>`）；完成後派 Pi review，再由主線 Fable final check」。
   ...
   2. **若 Fable draft 仍 the guarded `park` lifecycle command 了**（不該發生，draft prompt 已明令禁止）：先
      `node .cursor/scripts/spectra-target-guard.ts --change <change-name> -- unpark <change-name>` —— park 後 artifacts 只存 `.git/spectra-app/spectra.db` SQLite
