...
5. **Propose funnel**：開 Health wave 決策頁讓 user 挑最高優先 wave（[decision-page.md](decision-page.md) § Health wave；**NEVER** 「用 AskUserQuestion 或直接問」）→ 跑 **`/spectra-propose`**（brief 帶 findings + file:line + 卓越 delta 驗收）；其餘 wave 登 ROADMAP / tech-debt
