...
- 可用 clade script prototype（consumer sync 後位於 `.cursor/scripts/postgrest-resilience/`）：
  - `.cursor/scripts/postgrest-resilience/classify-migration.ts <migration.sql>`
  - `.cursor/scripts/postgrest-resilience/ready-watch.ts --url=<admin-ready-url>`
  - `.cursor/scripts/postgrest-resilience/smoke-runner.ts --endpoint=<name=url>`
