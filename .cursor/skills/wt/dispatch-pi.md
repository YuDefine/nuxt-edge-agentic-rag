...
When Step 1.8 routes to Pi for coding work, use `Bash run_in_background=true` to call the canonical dispatcher; Pi `openai-codex` is the execution transport. The worktree is the dispatcher's working directory.
...
1. **Prepare the brief**: Write to `/tmp/wt-pi-<slug>-prompt.md` using the Write tool. The brief MUST contain all of the following sections:
   ...
   **NEVER** 修改：`.vue` / `.tsx` / `.jsx` / `.css` / `.scss`，以及 `pages/` / `components/` / `layouts/` 底下的檔案。
   ...
   - **NEVER** 任何 main-bound ceremony：`/commit` / `/spectra-commit` /
     `/spectra-archive` / `wt-helper merge-back` / `git merge --squash`。
     你的權限止於 worktree 邊界 —— worktree 內的 commit 是你的，把它們 land
     到 main 不是。做完 brief 最後一項**不等於**取得 archive 授權：回報 done 就停。
