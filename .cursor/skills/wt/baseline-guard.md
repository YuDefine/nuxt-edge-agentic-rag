...
- **Dirty 非空** → 依 caller 路徑：
  - **OPSX workflow 路徑**（有 change context）走 **commit-then-fork**：主線從目前 canonical intent 的 requirement impacts 與 work specs 萃取 affected paths（scope-in），以正式 binding 確認實作 checkout，呼叫 `wt-helper add ... --task-summary "<一句話>" --precheck-baseline <change> --baseline-strategy commit --baseline-scope-paths <comma>` — helper selective stage + commit `baseline: <change> pre-fork sync` 上 main 再 fork；scope-out（跨 session WIP）留在 main 不動。
