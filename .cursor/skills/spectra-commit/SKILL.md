...
5. **Display commit plan**
   ...
   **Task 3: <task description>**
   ...
   Only artifact files will be committed. Use the guarded `task done` lifecycle command during apply to enable source file tracking.
   ```
...
6a. **Archive sub-flow** (only when the user selected "Archive first, then commit together")
    ...
    **6a-i. Incomplete task handling**
    ...
    - If **incomplete tasks exist**:
      ...
      - Use the **AskUserQuestion tool** to ask: "These tasks are still incomplete. Mark all as complete before archiving?"
        - **Yes**: set a flag to pass `--mark-tasks-complete` to the guarded archive command
    ...
    **6a-ii. Delta spec sync check**
    ...
    - If **delta specs exist**:
      - Use the **AskUserQuestion tool** to ask: "Delta specs found. Sync to main specs before archiving?"
        - **Yes**: run `node .cursor/scripts/spectra-target-guard.ts --change <name> -- sync <name>` before proceeding
    ...
    **6a-iii. Archive execution and file collection**
    ...
    ```bash
    node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- archive <name>          # without --mark-tasks-complete
    node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- archive <name> --mark-tasks-complete  # if user chose to mark tasks complete in 6a-i
    ```
