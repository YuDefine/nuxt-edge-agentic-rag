...
// findings to a Stop deep pass. AI Agent, Codex, and Grok Build dispatch
...
export function resolveHarness(env = {}, event = null) {
  ...
  // Codex turn-scoped events carry `turn_id`. AI Agent does not. Detecting
...
export async function runStopHook({ stdinJson, env = {}, cwd = process.cwd(), now = Date.now, detector } = {}) {
  ...
  try {
    ...
    // (AI Agent via hookSpecificOutput.additionalContext, Codex via a
    ...
    // before any scan. AI Agent and Codex both send this field: Codex
...
export function payload(text, eventName = 'PostToolUse', harness = 'claude') {
  ...
  // Codex shares AI Agent's PostToolUse additional-context shape, but its
