...
只可修改被維護的 `.cursor/skills/verify-<app>/`：`SKILL.md`、`features/` 與它自己擁有的 harness scripts。
...
找 `.cursor/skills/verify-*/SKILL.md`。零個 → 停止並 invoke `/verification-create`；多個 → 讓使用者指定，不靠名稱相似度猜。

### 1. Validate index hygiene

```bash
node .cursor/skills/verification-maintain/scripts/check-feature-map.mjs \
...
```json
{
  ...
  "target": ".cursor/skills/verify-<app>",
