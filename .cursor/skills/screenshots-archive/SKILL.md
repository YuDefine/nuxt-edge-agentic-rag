...
3. **找不到對應 topic** → 先跑 **backend-only 判定**，命中就 noop、**NEVER** 追問：
   ...
   純 backend change 沒有 UI 表面，**依定義**不會有截圖 —— 對它追問「要不要 sweep 別人的 topic」是把制度缺口變成使用者的決策負擔，且唯一正確答案永遠是「跳過」。**NEVER** 對命中 backend-only 的 change 開 AskUserQuestion。

4. **找不到對應 topic 且非 backend-only**（topic 名與 change 名不一致 / 已 sweep 過 / 該拍卻沒拍）：
   - 列 `screenshots/<env>/` 頂層所有候選 topic（排除 `_archive/`）
   - 用 AskUserQuestion 「找不到 `<change-name>` 對應 topic，候選列表：[topic-A, topic-B, ..., 跳過]」
   - user 選 topic → sweep 該 topic；選跳過 → noop 結束
   - 候選列表為空（頂層完全沒 topic）→ 直接 noop 回報「目前 `screenshots/<env>/` 已無 pending topic」

#### Mode C — 指定 topic

`/screenshots-archive <topic-name>` → 用 AskUserQuestion 確認「該 topic 未在 manual-review-archive 對齊，仍要 sweep 嗎？」，**MUST** 等明確 yes 才執行。
...
- **ALWAYS** Mode B 找不到對應 topic **且非 backend-only** 時用 AskUserQuestion 列候選 + 跳過選項，**NEVER** 預設靜默跳過（會讓 user 不知道 sweep 沒生效）；backend-only 命中則相反 —— **NEVER** 追問，回報一行帶過
