   ...
   **If the command succeeds**, check whether the change is parked (status can succeed even for parked changes):
   ...
   - **If the change IS in the parked list** (it's parked):
     ...
     **clade fork data-safety guard**（per `docs/pitfalls/2026-05-22-agent-tool-subagent-worktree-bypass.md`）：本路徑能命中表示 Step 0c.5 被跳過。先 `git rev-parse --git-dir`，**若路徑含 `.claude/worktrees/agent-`** → **STOP**，回報 `references/worktree-setup.md` § Step 2 parked fallback 的 STOP 訊息全文並交還 parent。**NEVER** 自行嘗試 unpark、**NEVER** 用 AskUserQuestion 給「強制 unpark」選項——沒有合法的「在 subagent 內 unpark」路徑。cwd 在 main / persistent worktree 才續走以下流程。
     ...
     ```bash
     node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- unpark "<name>"
     ```
     ...
     **Post-unpark commit**（clade fork addition；防 SQLite-only state）：unpark 後 **MUST** 立刻把 artifacts commit 進 git（`git add openspec/changes/<name>/` → commit）。**禁止** `git add -A`；`no changes to commit` 視為成功、hook fail 則 STOP。逐字指令見 `references/worktree-setup.md` § Step 2 parked fallback。
     ...
     ```bash
     node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- in-progress add "<name>"
     ```

     This is a silent operation — do not show the output to the user.

     Then re-run `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- status --change "<name>" --json` and continue normally.
     ...
     Wait for the user's response. If the user confirms, run `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- unpark "<name>"` + post-unpark commit + `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- in-progress add "<name>"`, and continue normally.
...
3. **Get apply instructions**
   ...
   ```bash
   node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- instructions apply --change "<name>" --json
   ```
...
Run `node .cursor/scripts/spectra-target-guard.ts --change <change-name> -- analyze <change-name> --json` to check cross-artifact consistency (Coverage, Consistency, Ambiguity, Gaps).
...
- **Both conditions met**: run `node .cursor/scripts/spectra-target-guard.ts --change <change-name> -- drift <change-name>`, display the report, then use the **AskUserQuestion tool**:
...
7. **Implement tasks (loop until done or blocked)**
   ...
   **Dispatch reminder**: For each phase, follow Step 6b's three-way classification:
   ...
   - Mark task complete by running: `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- task done --change "<name>" <task-id>`
     ...
     **Interrupted `task done` recovery（唯一合法路徑）**：先確認 current worktree 的具名 checkbox 已是 `[x]`，再依 current touched sidecar 對該 ordinal 的 record 數量執行 guard recovery；**NEVER** 直接呼叫 closed-source `spectra`：
     ...
     - 恰有 1 筆 record（checkbox + sidecar 都已 partial mutate）→ `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- task recover-done <task-id>`
     - 恰有 0 筆 record（只有 checkbox 被翻轉）→ `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- task recover-done --checkbox-only <task-id>`
     ...
     **Verification-only completion（唯一例外）**：若 task 只驗證已完成的 product paths、沒有自己的新 product diff，而且 guarded `task done` 已以 `MUTATION_POSTCONDITION` 明確證明「checkbox 有翻轉但 current sidecar 沒有新增 requested task record」並完成 rollback，才可改用 wrapper-owned completion：
     ...
     ```bash
     node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- \
...
7.5. **Gate Chain Pass — iterate-until-green**（clade fork addition — per [[verify-gate-chain]]）
   ...
   **觸發**：ALL phases（A + B + C）完成後、Step 8 Final check 之前。
   ...
   跑 consumer 的 gate chain（`.cursor/rules/local/verify-commands.mdc` 定義的 L0–L2 指令）：
   ...
   **Skip-condition**：consumer 的 `verify-commands.md` 不存在 → 退回既有行為（只跑 `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- instructions apply --change "<name>" --json` 確認 state）。不 block apply 流程。
...
- [ ] `node .cursor/scripts/spectra-target-guard.ts --change "<name>" -- instructions apply --change "<name>" --json` 回傳 `state: "all_done"`（貼該欄位輸出行）
