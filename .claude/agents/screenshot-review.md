...
- 有找到 → 從 process 取得 port（無 `--port` 則預設 3010）
- 沒找到 → 自動啟動：

```bash
for port in 3010 3011 3012 3013 3014; do
...
當 browser-use 不夠用時，寫 Playwright script 臨時跑或沉澱到 `e2e/screenshots/`：

```bash
# 一次跑完三個 viewport
npx playwright test e2e/screenshots/<topic>.spec.ts \
  --project=desktop --project=tablet --project=mobile
```

Script 骨架（`e2e/screenshots/<topic>.spec.ts`）：
...
for (const bp of BREAKPOINTS) {
  test(`<topic> @ ${bp.name}`, async ({ page }) => {
    ...
    await page.goto('http://localhost:3010/auth/_dev-login?redirect=/target')
...
若專案尚無 `playwright.config.ts`，先 `pnpm create playwright` 建立，並把 `testDir` 對齊到 `e2e/`。
