...
| <consumer-a> | 若 `_dev-login` alias 存在則 `as=admin\|manager\|staff\|unauthorized` | `role=admin\|manager\|staff\|unauthorized` | N/A |
...
Legacy <consumer-a>（`__test-login`，role + email + redirect 都 URL-encode）：
