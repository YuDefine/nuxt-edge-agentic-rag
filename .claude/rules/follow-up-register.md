...
`.claude/hooks/session-start-roadmap-sync.sh` 呼叫 `pnpm spectra:followups` 產生報告，含以下區塊：

- **Open Follow-ups** 數量 + top 5 priority
- **Unregistered Markers** —— tasks.md 有 `@followup[TD-NNN]` 但 register 沒登記（通常是剛加 marker 還沒跑 register 更新）
- **Orphaned Entries** —— register 有 entry 但 tasks.md 沒引用

每次 session 開始，agent 就能看到「有幾項技術債 open」→ 主動提醒使用者處理。
