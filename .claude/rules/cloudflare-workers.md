...
- **MUST** CI workflow（`.github/workflows/deploy*.yml`）使用 `cloudflare/wrangler-action@v3` action + `command: deploy`
  - 這是 fleet 全部 consumer 現狀（包含 NuxtHub 派 nuxt-edge-agentic-rag），document 它作為標準
...
|---|---|---|---|
| nuxt-edge-agentic-rag | Cloudflare D1 | ✅ | wrangler.jsonc |
| rental-scout | Cloudflare D1 | ✅ | wrangler.jsonc |
| co-purchase | Cloudflare D1 | ✅ | wrangler.jsonc |
| perno | Supabase | ❌ | wrangler.jsonc |
| yuntech-usr-sroi | Supabase | ❌ | wrangler.jsonc |
| TDMS | Supabase | ❌ | wrangler.jsonc |
