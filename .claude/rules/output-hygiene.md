...
`rules/core/code-discovery` + 各 MCP 規約（[[nuxt-ui-mcp]] / [[modern-web-mcp]] 等）已擋下「亂湊 API surface」，但沒擋「拿合法 MCP fact 合成 prescriptive pattern claim」。實證：2026-05-24 perno 撞到一次——主線把 `placeholder` prop（MCP fact）+ `modelValue` 型別接受 `null`（MCP fact）+ `resetModelValueOnClear` 預設 `null`（MCP fact）三條 synth 成「Nuxt UI v3 規範 pattern」貼上 doc 背書 label，被 user 標為**重大錯誤**。MCP 從沒明文宣告這個 pattern。
