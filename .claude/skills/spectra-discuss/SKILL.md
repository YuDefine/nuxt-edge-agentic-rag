...
| Option                           | 行為                                                                                       | 適用場景                                                                 |
| -------------------------------- | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------ |
| **A. GPT-5.6-sol via Pi（effort: max）**     | 主線 Claude 自己派 Pi 在背景跑（不要使用者切 CLI）                                         | propose 牽涉高度抽象決策、想要更高思考預算、或想用另一個模型獨立審視     |
...
- 選 **A** → 主線**先**輸出 `▶ 使用者在 discuss 選擇 A：GPT-5.6-sol via Pi（effort: max）— 接下來由 spectra-propose Step 0 派 Pi 在背景執行` → 接著 invoke `/spectra-propose <change-name>`
