...
在系統設計上，前端以 Nuxt 提供 Web 問答與管理後台，後端部署於 Cloudflare Workers，整合 D1、R2、KV、Workers AI 與 Vercel AI SDK；所有 Workers AI 呼叫前置至 Cloudflare AI Gateway，以提供 tokens、requests、cache hit rate 與 Neurons 額度等用量觀測基礎。`v1.0.0` 先收斂於可部署、可驗證、可答辯的最小閉環：Google OAuth 配合 Admin/Member/Guest 三級角色 RBAC、文件上傳與版本管理、AI Search 同步、Web 問答、可回放引用，以及分階段補齊之 4 個無狀態 MCP Tools。角色部分，Admin 由 `ADMIN_EMAIL_ALLOWLIST` seed，Member 由 Admin 於 UI 升格，Guest 權限由 `system_settings.guest_policy` dial（`same_as_member` / `browse_only` / `no_access`）統一控制，Web 與 MCP 入口均套用同一條閘。正式回答一律以 D1 驗證 `active/indexed/current` 狀態，並以前處理階段預先建立的 `source_chunks` 作為 `citationId` 回放來源。
...
本報告全文圖表合計 62 張（v47 新增表 2-29 預估月度運營成本、表 2-30 Scale Envelope 與擴展觸發點、表 3-6a v46 自動化測試覆蓋、表 3-6b v46 TC / UI-state 測試檔對照、表 3-10 驗收延後項收束契約、表 E-1 Workers AI 候選模型對照，共 6 張；表 2-9 為保留編號不列入索引），依編排規範列出圖表索引如下。頁碼以正式排版後為準。
...
表 2-30 `v1.0.0` Scale Envelope 與擴展觸發點 .....................................
...
表 D-2 `v1.0.0` 必要 Cloudflare 資源清單 .....................................
...
- 以 `@nuxtjs/mcp-toolkit`（`defineMcpTool` + `defineMcpHandler` middleware）實作 MCP Server 的 4 個核心 Tools，並統一透過 `Authorization: Bearer` + scope 控管；`v1.0.0` 初版交付時為 Nitro 原生 event handler，於 `migrate-mcp-to-toolkit` change 完成 runtime 遷移且保留同一組 scope 與治理契約，不變動 tool 名稱與 I/O 契約。[4][15][16][17]
...
- 應呈現內容：
  ...
  5. 右側以虛線標示後續延伸能力（含 Vercel AI SDK、workers-ai-provider、Cloudflare 原生 MCP），避免與 `v1.0.0` 核心架構混寫
...
- MCP 層：以 `@nuxtjs/mcp-toolkit` 的 `defineMcpTool` 建立 4 個核心 MCP tools，對應單一 `/mcp` JSON-RPC endpoint；透過統一 middleware 驗證 Bearer token 與 scope（`v1.0.0` 初版為 Nitro 原生 event handler，於 `migrate-mcp-to-toolkit` change 完成遷移，tool 名稱與 I/O 契約保持不變）。`v1.0.0` 的 MCP 採無狀態呼叫，不建立 `MCP-Session-Id` 相依性；Web 對話與 MCP 工具契約因此分別對應「D1 持久化對話輔助」與「單次請求契約」。若後續版本導入多輪上下文，runtime state 仍應保存於 KV，而非與 Web 對話混寫；若再升級至 Cloudflare 原生 MCP，仍須維持同一組 scope 與治理契約。[4][15][16]
...
| role_snapshot    | enum ('admin', 'member', 'guest')       | 三級角色快照；B16 擴張前舊值 `'user'` 於 migration `0006` 一次性升格為 `'member'`                      |
...
- `v1.0.0` 不建立 `admin_allowlists` 資料表；部署環境變數 `ADMIN_EMAIL_ALLOWLIST` 在 B16 之後語義從「允許登入閘門」收斂為**Admin seed 來源**，每次 Admin 登入由應用層比對後 upsert `role = 'admin'`，非 allowlist 成員則於 OAuth callback 建立為 `role = 'guest'`。
...
- `auth_source`、`status`（啟用／停用）欄位列為 `admin-ui-post-core` 階段擴充；`v1.0.0` 以 allowlist 與 role 控管入口權限，停用使用者改由 better-auth 的 `banned` 欄位承擔。
...
| created_by_user_id | string (FK → user.id)                   | 建立者；B16 擴張新增，用於在 MCP 入口以 token 創建者 role × `guest_policy` 做授權判定。migration `0006` 先以 nullable 加入，`0008` 收斂為 NOT NULL（NULL 舊資料先清 / 回填） |
...
- B16 擴張後，MCP 入口 middleware 會解析 token → 查 `created_by_user_id` 對應 role → 若為 Guest 則依 `system_settings.guest_policy` 決定可否呼叫；Admin / Member token 不受 dial 限制。
- `v1.0.0` 的 MCP 採無狀態呼叫，因此不建立 `mcp_sessions`。若後續導入多輪上下文，應另增對應 metadata table，並維持「KV 保存 runtime state、D1 僅保存 metadata」的原則。

**system_settings（B16 擴張，單列 KV 設定）**

B16 成員與權限擴張新增 `system_settings` 表，作為輕量 KV 型設定表，欄位為 `(key TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at TEXT, updated_by TEXT NOT NULL)`。`v1.0.0` 僅使用單一 key `guest_policy`，value 為 `same_as_member` / `browse_only` / `no_access` 三擇一（enum 由 `shared/types/auth.ts` 的 `guestPolicySchema` 在應用層驗證，不下推 DB constraint）。`updated_by` 接受三種值：better-auth `user.id`（Admin 透過 UI 變更）、`'system'`（migration seed / fallback）、`'db-direct'`（維運人員直接 SQL）。Dial 預設為 `same_as_member`，代表 Guest 與 Member 同權；切換為 `browse_only` 則 Guest 僅能瀏覽公開分類之已發布文件，不可提問；`no_access` 則 Guest 登入後僅見「帳號待審核」提示頁。

**member_role_changes（B16 擴張，角色升降 audit 表）**

B16 成員升降事件一律透過 `server/utils/member-role-changes.ts` 的 `recordRoleChange` 單一入口寫入此表，確保稽核軌跡完整。欄位為 `(id TEXT PK, user_id TEXT FK → user.id, from_role TEXT, to_role TEXT, changed_by TEXT, reason TEXT nullable, created_at TEXT)`；其中 `changed_by` 可為 `user.id`、`'system'`、`'db-direct'` 三類值，`reason` 自由文字承擔「`allowlist-seed` / `admin-ui` / manual」等操作背景。`(user_id, created_at)` 建立複合索引，供後續 `admin-ui-post-core` 階段接入讀取介面。`v1.0.0` 本身不提供 UI 讀取此表，內容以 `wrangler d1 execute` 查看。
...
- `v1.0.0` 採 better-auth 整合 Google OAuth，並以 `user_profiles` 承接 Admin/Member/Guest 三級角色、狀態與身分來源；Passkey 不納入本版。[13][20][22]
- B16 成員與權限擴張將授權模型由「Admin vs 非 Admin」二元模型升級為三級 RBAC：
...
- 角色升降事件一律透過 `recordRoleChange` 單一入口寫入 `member_role_changes` 表，以利稽核；`v1.0.0` 本身不提供對應讀取 UI，由 `admin-ui-post-core` 階段交付。
...
| MCP token（由 Admin/Member 建立，有 `knowledge.restricted.read`）      | `['internal', 'restricted']` | 可檢索與讀取 `restricted`；`v1.0.0` 仍維持無狀態呼叫                                                   |
...
本節於 v47 新增，作為回應「§1.4 預期效益」所宣稱之「降低基礎設施管理複雜度」與「以邊緣原生架構降低維運負擔」的量化根據。成本與容量皆以 2026-04 時點之 Cloudflare 公開計費與限額為基準；實際運行費用須於 Preview / Staging 實測後再回填。

#### 2.4.5.1 成本估算

`v1.0.0` 預期部署規模為單租戶、中小企業內部使用（估 5–50 位啟用帳號、每日 50–500 次問答、知識庫 50–500 份 md / txt 文件）。在此規模下，以 Cloudflare 免費方案與 Workers Paid（US$5/月）方案做雙層試算如下。

表 2-29 `v1.0.0` 預估月度運營成本

| 服務                    | 計費單位                       | `v1.0.0` 預估用量                                  | 免費額度 / 單價                                                   | 預估月費 (USD)     | 備註                                                                                   |
| ----------------------- | ------------------------------ | -------------------------------------------------- | ----------------------------------------------------------------- | ------------------ | -------------------------------------------------------------------------------------- |
| Workers (Paid)          | 每月請求數 + CPU 時間          | 約 50k 請求 / 月（問答 + 管理 + MCP）              | Paid：10M 請求 / 月含；CPU 30M ms / 月含；超額 $0.30/百萬請求     | $5.00              | Workers AI / AI Gateway 啟用通常需 Paid plan；是 `v1.0.0` 的基礎月費                   |
...
| Workers AI              | Neurons（依模型與 token 計費） | 500 次 / 日 × 30 日 × 平均 500 Neurons = 7.5M / 月 | Paid：10k Neurons / 日含（300k / 月）；超額約 $0.011 / 1k Neurons | $79.2（超額 7.2M） | 實際費用視所選模型而定，`v1.0.0` fallback synthesizer 不計費；接入實模型後成本大頭在此 |
...
| **合計（Fallback 期）** | —                              | —                                                  | —                                                                 | **$5.00**          | `v1.0.0` 以 fallback synthesizer 運行之月費                                            |
| **合計（實模型期）**    | —                              | —                                                  | —                                                                 | **≈ $84.20**       | `observability-and-debug` 階段接入 Workers AI 後之估算月費                             |

超額試算：若單月請求爆增至 500k、Workers AI 使用至 75M Neurons（10 倍規模），預估月費升至約 $825（$5 Workers + $820 Workers AI 超額），仍遠低於同等規模純雲端 GPT-4o / Claude 方案（以 $0.005 / 1k input token 計，75M Neurons 約等於 150M token，僅 input cost 即超過 $750，且未計 output）。

備註：

1. 上述 Workers AI Neurons 估算以「每次問答 500 Neurons」為粗估，實際隨模型與回答長度浮動，需於 `observability-and-debug` 階段以 AI Gateway 聚合數據回填。
...
`v1.0.0` 之 scale envelope 依 Cloudflare 平台限額與本系統設計常數推導。envelope 內指標滿足時可確保答辯版規格與核心治理不變；超出 envelope 時需觸發對應之擴展路徑，不得直接沿用本版設計。

表 2-30 `v1.0.0` Scale Envelope 與擴展觸發點

| 維度                      | `v1.0.0` 設計容量                              | 推導依據                                                                                                          | 超出時之擴展路徑                                                          |
| ------------------------- | ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
...
1. **Envelope 內**：`v1.0.0` 規格與治理契約可完整承受，不需額外結構變更。
2. **接近上限（> 80%）**：觸發 `/admin/usage` 儀表板告警（列為 v1.1+ 擴充），並於定期審視時重新評估擴展時程。
3. **超出 envelope**：屬明確擴展觸發點，需以獨立 change proposal 設計擴展路徑並重跑 frozen-final 驗收。

本節與 §2.4.3 表 2-27（平台限制與因應）互補：表 2-27 描述限制本質與因應原則，本節以量化數字描述 `v1.0.0` 在此限制下的可用範圍與擴展門檻。
...
| MCP Runtime             | `@nuxtjs/mcp-toolkit`                                  | 0.13.x（`migrate-mcp-to-toolkit` 遷移完成）   | 4 個核心 MCP Tools 實作（單一 `/mcp` JSON-RPC）                                                               |
| MCP Module（後續評估）  | Cloudflare 原生 MCP                                    | 後續版本評估                                  | MCP Resources、Session、Evals                                                                                 |
| AI Usage Observability  | Cloudflare AI Gateway                                  | `add-ai-gateway-usage-tracking` 階段接入      | 前置所有 Workers AI 呼叫；提供 tokens / requests / cache hit / Neurons 剩餘額度；搭配 `/admin/usage` 頁面呈現 |
| UI                      | Nuxt UI                                                | 4.x（首次通過核心閉環驗證時鎖定）             | 介面元件庫                                                                                                    |
| Accessibility           | `@nuxt/a11y`                                           | `responsive-and-a11y-foundation` 階段整合     | 無障礙檢查 dev report 與 WCAG AA 基線驗證                                                                     |
...
| 功能模組           | 說明                                                                                                                                                                                                  |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 身分驗證           | `v1.0.0` 支援 Google OAuth；角色採三級 RBAC（Admin/Member/Guest），Admin 由 `ADMIN_EMAIL_ALLOWLIST` seed，Guest 權限由 `system_settings.guest_policy` dial 控制                                       |
...
| MCP 介面           | 提供 4 個核心 Tools，支援 Bearer token 與 `knowledge.restricted.read` scope；B16 後 middleware 額外以 token 創建者 role × `guest_policy` 作為前置閘                                                   |
...
3. **frozen-final 同步回填**：mobile / tablet viewport 之實機截圖與 `@nuxt/a11y` dev report 將於 frozen-final 驗收階段統一補齊（詳見 §3.3.2.3 表 3-10 收束契約），不在本節逐張重拍。
...
本節 7 張實機截圖（圖 3-1 ~ 3-7）資產沿用 v46 於 2026-04-21 於 local dev 環境拍攝之 PNG（目錄 `screenshots/local/report-v46/`），v47 未重拍；v47 變更僅限報告文本（§3.3.2.3 收束契約、§2.4.5 成本與容量、附錄 E 選型等結構補強）。圖 3-2（含回答 + 引用卡片版本）、圖 3-7（loaded 版本）之補拍排程見 §3.3.2.3 表 3-10。

#### 3.2.3.1 登入畫面

圖 3-1 登入畫面實機畫面（2026-04-21 local dev，`screenshots/local/report-v46/3-1-login.png`）
...
圖 3-2 問答主畫面實機畫面（2026-04-21 local dev，`screenshots/local/report-v46/3-2-chat.png`）

實機狀態：loaded-empty onboarding；左欄「對話記錄」呈現 empty state、中央呈現「開始探索知識庫」引導文字與三個範例問題（`公司請假流程是什麼？` / `如何申請報帳？` / `專案管理的最佳實踐有哪些？`），底部輸入區顯示 `/ 聚焦 | Enter 送出 | Shift+Enter 換行 | Esc 清空` 快捷鍵與 `0 / 4000` 字數計數。此版本尚未跑完整 R2 + AI Search 閉環，所以未能拍到含回答與引用卡片的版本；該版本待 frozen-final 驗收階段補。
...
圖 3-3 知識庫管理畫面實機畫面（2026-04-21 local dev，`screenshots/local/report-v46/3-3-admin-documents.png`）
...
**MCP 不涉及 agent-initiated approval**：`v1.0.0` MCP 4 個 tool 全為 read-only（`searchKnowledge` / `askKnowledge` / `getDocumentChunk` / `listCategories`），不執行任何破壞性操作；因此表 3-5 的確認策略僅適用於 Web Admin 管理介面，不延伸至 MCP 契約。

#### 3.2.3.4 MCP Token 管理畫面

圖 3-4 MCP Token 管理畫面實機畫面（2026-04-21 local dev，`screenshots/local/report-v46/3-4-admin-tokens.png`）
...
圖 3-5 成員管理畫面實機畫面（2026-04-21 local dev，`screenshots/local/report-v46/3-5-admin-members.png`）
...
圖 3-6 訪客政策設定畫面實機畫面（2026-04-21 local dev，`screenshots/local/report-v46/3-6-admin-guest-policy.png`）
...
圖 3-7 AI Gateway 用量儀表板實機畫面（2026-04-21 local dev，`screenshots/local/report-v46/3-7-admin-usage.png`）

實機狀態：**error (graceful)**；因本機 `.env` 未設 `CLOUDFLARE_API_TOKEN_ANALYTICS`，`/api/admin/usage` 回非 2xx，前端顯示「無法載入用量資料」placeholder + 「Cloudflare Analytics API 可能暫時不可用，或 gateway / 金鑰尚未設定完成」說明 + 「重新載入」按鈕，不整頁崩潰。此畫面同時作為 §3.2.3.7 規格中「設定未完成時的 UI 降級態」驗證，對應表 2-27「平台限制與因應」的可降級原則，並與 UX Completeness 規則要求之 `empty / loading / error / unauthorized` 四態覆蓋一致。
...
- 注意事項：
  - 本頁不承擔 enforcement（即使剩餘額度見底亦不自動降載，列為 v1.1+ 工作）；MCP rate limit 仍走既有 KV 機制
...
本節分兩層：先以「`v46` 自動化覆蓋狀態」記錄已跑通之整合測試覆蓋，作為結構式正確性佐證；正式之 `frozen-final` 驗收統計（含平均延遲、P50 / P95、judge 觸發率等）仍依 §1.3.4 表 1-4 回填時機原則，待實模型接入與正式驗收後才寫入表 3-7、表 3-8。附錄 B 的 `gold facts`、必要引用與不可犯錯欄位，是逐案判定的主來源，不得只憑回答是否流暢或看似合理決定通過與否。

#### 3.3.2.1 v46 自動化覆蓋狀態（結構式正確性）
...
上述自動化測試僅驗證結構式正確性；實模型品質、延遲分布與 `frozen-final` 30–50 筆統計仍待驗收階段另行產出。

#### 3.3.2.2 `frozen-final` 正式驗收回填欄位（待回填）

本節於實作完成後回填。除情境彙總表外，另需同時保留按 `TC-xx` 填寫的逐案結果表，以及處理部署、身分與版本交易等非問答證據的 `EV-xx` 補充證據表，確保第三章資料可逐項回對第四章驗收命題。正式彙總表欄位如下：
...
| 證據編號 | 對應 Acceptance ID | 驗收重點                                     | 建議證據形式                                                                                                            | 通過條件                                                                                                                       | 既有 runbook / 驗證文件（v46 指向）                                                                                            |
| -------- | ------------------ | -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ |
...
| EV-02    | A08                | OAuth + 三級 RBAC + `guest_policy` dial 行為 | 登入截圖（圖 3-1、圖 3-5、圖 3-6）、Session 權限比對紀錄、allowlist / dial 異動前後操作結果、`member_role_changes` 稽核 | 管理員身分可隨 allowlist 異動即時重算，不殘留舊權限；dial 切換可即時影響 Web 與 MCP 入口                                       | `docs/verify/OAUTH_SETUP.md`、B16 change archive、圖 3-5 / 3-6                                                                 |
...
本節為 v47 新增，用以解決前兩小節「自動化覆蓋已完成，但 `frozen-final` 與實模型品質仍留待後續」的收束模糊問題。即便本報告在 `v1.0.0` 答辯時尚未回填表 3-7 / 3-8 與圖 3-2 / 3-7 的完整版本，仍應透過本節明確「延後項在何時、由誰、在哪個環境、用什麼資料集、通過哪些門檻才算結案」，避免延後項成為無限期空殼。

表 3-10 驗收延後項收束契約

| 延後項（對應 Acceptance ID）                      | 結案環境                              | 執行者   | 資料集 / 來源                           | 通過門檻                                                                                                                                                 | 預計觸發時機                                                           |
| ------------------------------------------------- | ------------------------------------- | -------- | --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| 表 3-7 實測情境彙總統計（A02 / A05 / A06 / A07）  | Preview → Staging（`v1.0.0` 同版）    | 作者本人 | `frozen-final` 30–50 筆凍結後測試集     | 覆蓋 §2.4.4.4 硬性驗收三項（Current-Version / Restricted / Redaction）均 100%，品質驗收四項（Citation / Answer / Refusal / MCP Success）達表 2-28 目標值 | `observability-and-debug` 階段接入 Vercel AI SDK + Workers AI 後跑一輪 |
...
| §5.1 組員心得（全域）                             | 紙本撰寫                              | 作者本人 | 實作過程筆記                            | 涵蓋模組責任、技術取捨、SDD / TDD / LLM pair programming 學習點，至少 500 字                                                                             | `frozen-final` 驗收完成後、交付前                                      |
...
上列延後項若屆時仍不可得（如 `CLOUDFLARE_API_TOKEN_ANALYTICS` 審核未通過、Workers AI 候選模型下架），需於交付版之 §5.2「檢討與改進」明列「無法結案之延後項」+「替代證據」+「若本專題延續至 v1.1+ 之補做計畫」，不得以靜默空白交付。
...
「目前狀態」欄採三級分級：**結構性保障**（由 schema / migration 強制、無需 runtime 驗證）、**自動化覆蓋**（v46 整合測試已驗證結構式正確性；實模型延遲與品質仍待 frozen-final）、**待驗收**（需 frozen-final 統計或部署環境實測才能結案）。

| Acceptance ID | 驗收目標                                                                                               | 對應章節      | 主要對應案例                      | 驗收證據                                                                                                          | 目前狀態                                                                                                                                         |
| ------------- | ------------------------------------------------------------------------------------------------------ | ------------- | --------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| A01           | 邊緣原生架構可部署                                                                                     | 1.2.1、1.3.2  | EV-01                             | 部署紀錄、系統架構圖、Smoke Test                                                                                  | 待驗收（v0.24.x 已多次部署上線；`docs/verify/DEPLOYMENT_RUNBOOK.md` + `production-deploy-checklist.md` 已備，正式 frozen-final deploy 為結案點） |
| A02           | 完成 AI Search 與自建 Agent 流程整合                                                                   | 1.2.1、2.1.2  | TC-01、TC-04、TC-06、EV-01        | 查詢日誌、引用紀錄、模型路由紀錄                                                                                  | 自動化覆蓋（TC-01/04/06 全綠；實模型延遲待 frozen-final）                                                                                        |
...
| A05           | Self-Correction 可改善模糊查詢                                                                         | 2.1.2、2.4.4  | TC-04                             | judge `reformulatedQuery` 重試前後比較報告                                                                        | 自動化覆蓋（TC-04 通過；實模型 reformulate 品質待 frozen-final）                                                                                 |
| A06           | 拒答機制可正確阻擋越界或高風險查詢                                                                     | 1.2.2、2.4.1  | TC-07、TC-08、TC-09、TC-15        | 測試集與拒答紀錄                                                                                                  | 自動化覆蓋（TC-07/08/09/15 全綠，含信用卡號 block 路徑）                                                                                         |
| A07           | MCP 4 個 Tools 可被外部 Client 正常使用                                                                | 2.2.2、3.2.2  | TC-12、TC-16、TC-17、TC-19、TC-20 | Claude Desktop / Cursor / MCP Inspector 測試結果                                                                  | 自動化覆蓋（MCP contracts 15 檔 51 tests 全綠；外部 Client 實連待 frozen-final）                                                                 |
| A08           | Google OAuth + 三級角色 RBAC + `guest_policy` dial 正常運作（`ADMIN_EMAIL_ALLOWLIST` 僅作 Admin seed） | 2.4.1、3.2.3  | EV-02、EV-06、TC-UI-01            | 登入截圖（圖 3-1 / 3-5 / 3-6）、升降／dial 切換前後的權限比對、`member_role_changes` 稽核紀錄、響應式與 a11y 證據 | 自動化覆蓋（TC-UI-01 7 assertions 含升降流程全綠；實機截圖齊；OAuth 實連待 frozen-final deploy）                                                 |
...
| A11           | 高風險輸入不會以原文寫入持久化紀錄（**結構性保障：v1.0.0 `messages` 表不存在 `content_text` 欄位**）   | 2.4.1、2.4.4  | TC-15、EV-05                      | migration schema + `messages.content_redacted` + `query_logs.status = 'blocked'` 稽核結果                         | 結構性保障（migration 0001 合併 schema 無 `content_text` 欄位；TC-15 覆蓋 blocked 路徑寫入）                                                     |
| A12           | 對外 MCP 契約不暴露內部診斷欄位                                                                        | 2.2.2、附錄 A | TC-20                             | Tool 契約測試、回應範例                                                                                           | 自動化覆蓋（TC-20 通過）                                                                                                                         |
| A13           | rate limit 與保留期限規則可被驗證                                                                      | 2.4.1         | EV-04                             | `429` 測試紀錄、`citation_records.expires_at` 清理作業摘要                                                        | 待驗收（`docs/verify/RETENTION_CLEANUP_VERIFICATION.md` 已備；frozen-final 需執行 backdated record 清理流程）                                    |

### 4.1.2 技術特色（分級：已達結構式驗證 / 實模型品質待後續）

本系統相對純雲端 LLM 方案，主要差異化定位於三個互補軸：**邊緣原生部署**、**Hybrid Managed 治理**，以及**可稽核的拒答機制**。以下八點特色依此三軸展開；考量 `v1.0.0` 以 fallback synthesizer 運行、實模型品質驗收留待後續階段，各特色之驗證狀態採兩級分級：

- **已達結構式驗證**：v46 自動化測試覆蓋已佐證結構式正確性（參見 §3.3.2.1 表 3-6a / 3-6b 與 §4.1.1 表 4-1 「自動化覆蓋」列）。
- **實模型品質待後續**：結構正確但回答品質、延遲分布仍待 `observability-and-debug` 階段接入實模型後以 `frozen-final` 驗收回填（§3.3.2.3 表 3-10 收束契約）。
...
2. **分段式信心判斷**（Hybrid Managed 軸）｜*已達結構式驗證；實模型 judge 精準度待後續*：先以 `retrieval_score` 做快路徑決策，再只在邊界情境追加 judge，以兼顧品質與延遲。v46 judge 由結構式判斷器承擔；實模型 judge 品質待 frozen-final。
...
5. **Web 與 MCP 契約分流**｜*已達結構式驗證*：`v1.0.0` Web 與 MCP 皆為單輪無狀態契約，`messages` 表僅保留遮罩後 `content_redacted`；Web 對話多輪指代輔助列為 `governance-refinements` 階段擴充，屆時補齊 `conversations` / `messages.content_text` 並維持 MCP 無狀態契約；MCP contracts 15 檔 51 tests 佐證。
...
8. **分階段落地**（邊緣原生軸）｜*已達結構式驗證*：先以 `v1.0.0` 完成核心版，再把 Passkey、MCP Session 與 Cloud fallback 留作後續版本；v46 已把 B16 三級 RBAC、AI Gateway 用量前置、響應式 / a11y foundation 納入核心版，示範分階段落地之工程可行性。
...
本版（v0.0.45）另補齊以下「已實作但尚未完成正式驗收」之規格：

1. **B16 成員與權限管理**：將二元 Admin vs 非 Admin 升級為 Admin/Member/Guest 三級 RBAC；Admin 身分仍以 `ADMIN_EMAIL_ALLOWLIST` 為單一真相來源，Member 由 Admin 於 UI 升格，Guest 權限由 `system_settings.guest_policy` dial 統一控制；升降事件透過 `recordRoleChange` 寫入 `member_role_changes` 表。migration `0006_three_tier_role_and_settings.sql` 在同一 transaction 完成既有 `user` 升級、`system_settings` 表建立、`mcp_tokens.created_by_user_id` FK 新增與 `member_role_changes` audit 表建立。
2. **Cloudflare AI Gateway 整合**：所有 Workers AI 呼叫前置至 AI Gateway，於 `/admin/usage` 呈現 tokens、requests、cache hit rate 與 Neurons 剩餘額度；本版僅供 observability，不承擔 enforcement（撞額度時不自動降載，留予後續版本）。
3. **Better Auth timestamp affinity 修正與 MCP token creator NOT NULL 收斂**：migration `0007` 統一 better-auth 欄位時區語意，`0008` 將 `mcp_tokens.created_by_user_id` 收斂為 NOT NULL（遷移前先清理 staging/local seed 資料並將 production test token 回填至有效 user id）。
4. **響應式與無障礙 foundation**：全頁採 mobile / tablet / desktop 三 breakpoint 設計，接入 `@nuxt/a11y` dev report，涵蓋關鍵操作路徑之鍵盤 walkthrough；驗證文件詳見 `docs/verify/RESPONSIVE_A11Y_VERIFICATION.md`。

上述項目均已進入生產環境並透過單元 / 整合 acceptance 測試（`test/integration/acceptance-tc-*.test.ts`）持續驗證；正式之 `frozen-final` 測試集跑報與延遲 / 品質統計仍依 §1.3.4 與 §3.3.2 規劃，留待實際驗收階段再回填。

v46 新增之「規格收斂於實機驗證」成果：

5. **自動化 acceptance 測試全綠**：2026-04-21 以 `pnpm verify:acceptance` + `pnpm test:integration` 跑完後，Unit acceptance 6、MCP contracts 51、Integration 260（1 skipped pre-existing）、其中 TC-01~20 與 TC-UI-01 合計 42 個 assertions 全綠。明細見 §3.3.2.1 表 3-6a / 3-6b；實模型延遲與品質仍以 fallback 合成器與結構式判斷器為基準，不宣稱已完成品質驗收。
6. **§3.2.3 實機截圖齊備**：2026-04-21 於 local dev 環境拍下七張主畫面（圖 3-1 ~ 3-7），覆蓋登入、問答 empty onboarding、文件管理（三 lifecycle 狀態）、Token 管理、成員管理（三級角色）、訪客政策 dial、AI Gateway 用量（graceful error 降級），整理於 `screenshots/local/report-v46/review.md`。其中 `/chat` 僅拍到 empty onboarding、`/admin/usage` 為 graceful error，兩者均為合法 UI 狀態，但含回答與含 gateway 資料之版本仍待 frozen-final 驗收環境補拍。
7. **v0.0.45 升版已 archive**：commit `971511d` / tag `v0.24.1` 完成骨架升版（補齊 B16 / AI Gateway / migration 0007-0008 / 響應式在第二、三章的敘述）；本版（v0.0.46）續補自動化驗證覆蓋、實機截圖、EV runbook 指向與表 4-1 狀態分級。
...
9. **§2.4.5 部署成本與容量規劃**：新增表 2-29 預估月度運營成本（Fallback 期 $5.00、實模型期 $84.20、10 倍規模 $825 超額試算）與表 2-30 Scale Envelope（同時在線 50 人、日問答 500 次、文件 500 份、chunks 500k 之明確上限與擴展觸發點），回應 §1.4 預期效益之量化根據並設定 scale 宣告。
10. **附錄 E 實模型選型參考**：新增表 E-1 Workers AI 候選模型對照（llama-3.3-70b / kimi-k2.5 / llama-3.1-8b / llama-3.2-3b / gpt-oss-20b 等），列 5 gate（可用性、延遲、結構化輸出、成本、中英文）選型規則與 4 步鎖定流程，供 `observability-and-debug` 階段接入實模型時對照。
11. **§4.1.2 特色分級敘述**：8 項技術特色改以「已達結構式驗證 / 實模型品質待後續」兩級分級呈現，每項標註對應 TC 測試證據，與 §4.1.1 表 4-1 三級狀態分級互補。
...
「敏感度」欄之「高」代表外流即需立即輪替並通知相關人員；「中」代表外流會洩漏特定個資或組織資訊，應盡速輪替；「低」代表本身不是祕密，但仍不宜隨意公開。Feature flags 在 `v1.0.0` 的預設狀態已對齊表 2-25「不納入本版之功能預設關閉」之原則。
...
`v1.0.0` 依賴三類 Cloudflare 邊緣資源與一個 AI Search 索引，名稱須與 `wrangler.jsonc` 的 binding 宣告一致。

表 D-2 `v1.0.0` 必要 Cloudflare 資源清單

| 資源類型       | Binding 名稱 | 正式命名                | 用途                                         |
| -------------- | ------------ | ----------------------- | -------------------------------------------- |
| D1 Database    | `DB`         | `agentic-rag-db`        | `v1.0.0` 主資料庫，保存 users/documents/logs |
...
`v1.0.0` 之 D1 migration 採「只新增不回滾」之政策：migration 檔一旦 apply 到任何環境即視為 production-committed，不得刪除或改寫舊檔，僅可追加新 migration 修正。此政策之理由見 §D.3.2 關於 D1 migration 退版成本之說明。
...
本附錄於 v47 新增，作為 §2.2.3.1 模型分工的量化補充。`v1.0.0` 核心閉環以 fallback synthesizer 運行，實模型接入屬 `observability-and-debug` 階段工作；為避免答辯時「接入哪個模型」成為空白，本附錄整理候選模型與選型規則，供接入時對照。

### E.1 候選模型對照

以下清單以 2026-04 時點 Cloudflare Workers AI 官方模型頁 [6][27] 可用模型為主；模型清單與 alias 可能變動，實際接入前須再次核對官方最新公告。

表 E-1 Workers AI 候選模型對照

| 模型 alias                        | 類別                   | Context Window | 預估每次呼叫成本（Neurons） | 強項                                   | 弱項                             | 建議對應角色                                           |
| --------------------------------- | ---------------------- | -------------- | --------------------------- | -------------------------------------- | -------------------------------- | ------------------------------------------------------ |
...
1. **可用性 gate**：Preview 環境實際 ping 過該模型、確認 alias 存在、Neurons 額度與限速設定允許 `v1.0.0` 預期流量。未通過此 gate 的模型直接從候選名單剔除。
...
3. **結構化輸出 gate**：`models.agentJudge` 須能穩定回傳 JSON schema（`{ shouldAnswer, reformulatedQuery? }`）；小樣本測試若出現 schema 破格率 > 5% 即淘汰該模型。
4. **成本 gate**：以 §2.4.5.1 表 2-29 之 `v1.0.0` 月度用量推估超額費用，若超過年度預算，降階為小模型或啟用 AI Gateway cache。
5. **中英文 gate**：附錄 B `frozen-final` 資料集含中文知識庫內容，候選模型須在中文 gold facts 命中率達品質驗收 > 80%（表 2-28 Answer Correctness）。
...
4. 跑一輪 `frozen-final` 測試集，將結果回填表 3-7、表 3-8 與 §3.3.2 前言。
5. 若後續更換模型，不得改變 §2.2.3.1 的路由條件與回傳契約；僅更新 alias 與 `config_snapshot_version`，再跑一次 `frozen-final` 對照前後差異。

### E.4 不選的替代方案

下列外部模型雖能力強，但 `v1.0.0` 明確不納入：

1. **Claude / GPT-4o / Gemini 雲端 API**：涉及資料外送、需治理閘道、會破壞「v1.0.0 不啟用跨雲 LLM API」之核心命題（§1.1.4）。
2. **自部署 LLM（vLLM / Ollama）**：與邊緣原生部署原則相悖，需額外維運 GPU 主機。
3. **Cloudflare 以外之邊緣模型**：若選型會引入新的 provider SDK，增加 Vercel AI SDK 之外的整合負擔，超出 `v1.0.0` 範圍。
