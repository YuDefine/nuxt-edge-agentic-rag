...
- Progress: 23/28 tasks（82%）；eval harness、dataset、scorer、docs、dev MCP token helper、bearer token client wiring 已落地。
- 剩餘：建立正式 baseline、驗證 regression fail-path 能可靠讓 command non-zero，並補齊 `docs/evals/mcp-tool-selection.md` 最終紀錄。
...
4. 完成 `add-mcp-tool-selection-evals` baseline 與 fail-path 驗證；目前 TD-043 記錄了 Evalite afterAll fail 不 propagate 的風險。
