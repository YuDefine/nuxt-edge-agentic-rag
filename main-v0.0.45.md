...
| A08           | Google OAuth + 三級角色 RBAC + `guest_policy` dial 正常運作（`ADMIN_EMAIL_ALLOWLIST` 僅作 Admin seed） | 2.4.1、3.2.3  | EV-02、TC-UI-01                   | 登入流程截圖、升降／dial 切換前後的權限比對、`member_role_changes` 稽核紀錄               | 待驗證   |
...
上述項目均已進入生產環境並透過單元 / 整合 acceptance 測試（`test/integration/acceptance-tc-*.test.ts`）持續驗證；正式之 `frozen-final` 測試集跑報、介面截圖與 EV 證據彙整則依 §1.3.4 與 §3.3.2 規劃，留待實際驗收階段再回填。
