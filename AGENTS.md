...
完整規則：[`.claude/rules/ux-completeness.md`](.claude/rules/ux-completeness.md)

### Required workflow integration

| Spectra phase                       | Gate script                                                                 | When to run                                                                |
| ----------------------------------- | --------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| Before `spectra-propose`            | `bash {{SCRIPTS_DIR}}/spectra-ux/pre-propose-scan.sh`                       | 注入 blast radius 要求，提醒必填區塊                                       |
| After `spectra-propose`             | `bash {{SCRIPTS_DIR}}/spectra-ux/post-propose-check.sh <change>`            | 驗證 proposal 完整性                                                       |
| After `spectra-propose`             | `bash {{SCRIPTS_DIR}}/spectra-ux/design-inject.sh <change>`                 | 若有 UI scope，提醒補上 `## Design Review` 區塊                            |
| Before `spectra-apply`              | `bash {{SCRIPTS_DIR}}/spectra-ux/pre-apply-brief.sh <change>`               | 簡報 user journeys                                                         |
| During UI edits                     | `bash {{SCRIPTS_DIR}}/spectra-ux/ui-qa-reminder.sh <file>`                  | 中途提醒 design / screenshot review，不要等到 archive 才檢查               |
| Before `spectra-archive`            | `bash {{SCRIPTS_DIR}}/spectra-ux/design-gate.sh <change>`                   | 阻擋未完成人工檢查或缺設計審查證據的 UI change                             |
| Before `spectra-archive`            | `bash {{SCRIPTS_DIR}}/spectra-ux/archive-gate.sh <change>`                  | 驗證 journey URL touch、schema drift、exhaustiveness                       |
| Before `spectra-archive` (v1.5+)    | `bash {{SCRIPTS_DIR}}/spectra-ux/followup-gate.sh <change>`                 | 驗證 tasks.md 的 `@followup[TD-NNN]` 都在 `docs/tech-debt.md` 有完整 entry |
...
spectra-ux 另外提供一組配套規則，預設安裝在與 `.claude/rules/ux-completeness.md` 同目錄：
