...
下表將驗收指標分為三類：硬性驗收（current-version-only / restricted 隔離 / redaction 完整性等不可違反 invariant）、品質驗收（回答品質、引用精準率、拒答精準率等需以正式驗收資料集統計者）、觀測指標（延遲與觸發率等需於 Preview / Staging 觀測者）。**硬性驗收已由 §3.3.2.1 結構式自動化測試承擔**；**品質驗收門檻為正式驗收資料集凍結後的判定基準，現階段尚未以實模型大樣本資料完成統計，相關品質統計列入 §4.2.3 研究限制與第三章 §3.3 之延伸驗證邊界**；**觀測指標以固定環境 Preview / Staging 觀測值佐證，不單獨作為 fail gate**。
...
對應 §2.1.2 圖 2-2 Agentic RAG 問答活動圖：
...
下方表 3-7 與表 3-8 為「正式驗收統計」的欄位定義模板，欄位本身屬交付版規格，數值列因現階段尚未跑完凍結之正式驗收資料集（30–50 筆）而保留為空。**現階段以表 3-6a / 3-6b 結構式自動化測試（pnpm verify:acceptance + pnpm test:integration）承擔結構式正確性驗證**；當凍結之正式驗收資料集執行完成後，本表將以實際數字填入。讀者不應將空欄位誤判為「實測未做」，而應理解為「實模型品質統計屬延伸驗證邊界，列入 §4.2.3 研究限制」。

表 3-7 實測情境彙總表（欄位定義；現階段以表 3-6a / 3-6b 結構式自動化測試承擔，待正式驗收資料集填入）
...
表 3-8 TC 逐案測試結果表（欄位定義；現階段以表 3-6b 結構式驗證代替；正式驗收後填入）
...
近期修訂進一步補強以下四類已實作成果，作為本版報告的核心支撐：
...
13. **其他清理**：圖表索引改為校方要求的圖目錄／表目錄格式、表 2-9 保留編號從索引去除佔位、參考文獻補入 OAuth 2.0 Bearer Token RFC[25]，並修正 Vercel AI SDK 引用[28]、目錄展開至節／小節層級。
...
以下清單以 2026-04 時點 Cloudflare Workers AI 官方模型頁[5][23]可用模型為主；模型清單與 alias 可能變動，實際接入前須再次核對官方最新公告。本附錄屬候選規劃與選型參考，不代表本版已實際部署下表模型。
