![image1](main-v0.0.25_assets/rId18.jpg)
...
10. 標記為可發布版本：僅當新版本 smoke retrieval 與映射驗證通過後，才可將 document_versions.index_status 標為 indexed，並將對應 ingestion_jobs.status 標為 completed。此時版本代表「可發布」，但尚未自動成為 current。
...
13. 一致性保護：AI Search metadata 僅為第一層快篩與觀測；回答前一律以 D1 post-verification 剔除非 active/indexed/current 片段，並丟棄無法映射到 source_chunks 的候選片段。若剔除後已無有效證據，則視為無結果，不得回退到舊版內容。
...
Admin 先取得一次性 signed URL 與 uploadId → 原始檔直傳 R2 staged 路徑 → 呼叫 finalize 驗證副檔名、MIME type、大小與 checksum → 建立 document_versions（預設 is_current = false、index_status = queued）→ 產生 normalized_text_r2_key 與 deterministic source_chunks → 寫入 AI Search metadata（含 document_version_id 與 folder 路徑；version_state 若存在僅作觀測提示）→ 建立 ingestion_jobs（status = queued）→ 觸發 instance 級同步 → AI Search 完成轉換、分塊、Embedding 與索引 → 任務與版本轉為 smoke_pending → 執行以 document_version_id 為主的 representative smoke retrieval，確認回傳片段皆可映射至既有 source_chunks → 通過後回寫 ai_search_file_id、index_status = indexed、indexed_at → Admin 顯式執行 publish，系統再以 transaction 將新版本切為 is_current = true 並寫入 published_at / published_by → 文件可供正式檢索
...
本章在 v0.0.23 中作為研究結果與結論回填骨架。現階段只先列出後續必須被證據支持的驗收命題與技術特色，避免在實測前先寫死結論。自 v0.0.25 起，版本切換採「indexed 後顯式 publish」與「restricted existence-hiding」兩項修訂原則，後續驗收亦應依此口徑比對。
...
補充說明：AskKnowledgeOutput 僅適用於授權成功且請求格式正確之情境；若 token 無效或工具本身 scope 不足，應直接回 401/403，不以 refused 包裝。若授權成功但可見集合中沒有足夠證據，則回 refused = true；此時即使目標內容只存在於 restricted 文件，也不得主動揭露其存在。decisionPath、retrievalScore、confidenceScore 與模型路由屬內部診斷資料，v1.0.0 不列為對外穩定契約；若需檢視，應由 Web Admin 透過 query_logs 取得。
...
正式擴充 frozen-final 時，至少還需補入 searchKnowledge、listCategories、OAuth／allowlist、current／archived 切換、rate limit、restricted existence-hiding 與 no-internal-diagnostics 等非問答契約案例。
