![image1](main-v0.0.21_assets/image1.jpeg)
...
本章在 `v0.0.21` 中定位為「預定實作成果與展示規格」。所有內容皆作為後續實作、截圖與資料回填的正式骨架，並不宣稱目前已完成實測。
...
本章於 `v0.0.21` 中作為驗收與結論撰寫骨架，不直接宣稱已完成成果，而是列出後續必須被驗證的對照項與預定結論方向。
...
本附錄在 `v0.0.21` 先建立 15 筆種子案例，後續於正式驗證前擴充至 30–50 筆。
