...
在技術架構上，系統以前端 Nuxt 4 應用為使用者與管理者入口，後端則透過 NuxtHub 部署於 Cloudflare Workers，整合 D1、R2、KV、Workers AI 與 Vercel AI SDK。檢索部分由 Cloudflare AI Search 負責資料同步、文件轉換、分塊、Embedding、query rewriting、reranking 與 retrieval；回答生成則不直接交由 AI Search 完成，而是由應用層自建的 Agent 流程控制，以保留置信度評估、Self-Correction、拒答判斷、引用組裝與審計記錄等核心能力。邊緣模型配置方面，系統將以 Llama 4 Scout 17B MoE 作為預設快速回答模型，Kimi K2.5 作為複雜推理、查詢重寫與答案可回答性判斷模型，並以 gpt-oss-120b 作為邊緣備援模型；如經資料治理與 feature flag 條件允許，則以 Claude Sonnet 4.6 作為預設雲端備援目標，其他候選模型為 GPT 5.4、Gemini 3.1 Pro、Gemini 3 Flash 與 Claude Opus 4.6。[6][7][10]

系統 `v1` 範圍採完整野心版規劃，包含 Web 問答介面、Google OAuth 與 Passkey 登入、Admin email allowlist、文件上傳與版本管理、AI Search 同步流程、查詢日誌與統計、MCP 4 個核心 Tools、Bearer token 與 scope 管理、MCP Session、多輪追問、拒答與條件式 Cloud fallback。為避免規格與實作脫節，本版文件定位為「實作前規格版」：第三章與第四章將保留正式專題報告的章節骨架，但所有未經實測之效能、成本與成果陳述皆改寫為設計目標、驗收條件或待驗證項，不將尚未完成的結果寫成既成事實。

在驗證規劃上，本專題將以代表性查詢測試集檢驗一般問答、模糊查詢、自我修正、正確拒答、多輪對話與 MCP 互操作，並以首字延遲、完整回答延遲、引用正確率、回答正確率、拒答精準率、Self-Correction 觸發率、MCP Tool 成功率等指標作為評估基準。本版文件將作為後續系統實作、截圖回填、圖表繪製、答辯準備與正式報告收斂的共同基準。
...
| 知識管理 | 文件上傳、版本管理、R2 儲存、AI Search 同步、文件狀態管理                                      |
| 問答流程 | Query Normalization、AI Search 檢索、置信度評估、Self-Correction、拒答、引用顯示、對話歷史     |
| MCP 介面 | `searchKnowledge`、`askKnowledge`、`getDocumentChunk`、`listCategories`、Bearer token、Session |
| 可觀測性 | 查詢日誌、引用紀錄、MCP 呼叫成功率、延遲統計、管理後台儀表資訊                                 |
...
- 不處理多租戶隔離與租戶計費。
- 不在 `v1` 納入 LINE Login。
- 不在 `v1` 實作細粒度文件 ACL；僅實作文件敏感等級與 Cloud fallback 治理閘道。
...
- 前端層：使用 Nuxt 4 與 Nuxt UI 建立問答介面、管理後台與設定頁。使用者可透過 Google OAuth 或 Passkey 登入，並在同一前端中存取對話歷史、文件管理與統計頁面。
- 資料與受管理檢索層：以 R2 儲存原始文件與版本檔，D1 儲存結構化資料，KV 作為快取與 MCP Session 的 unstorage 驅動。Cloudflare AI Search 連接既定資料來源後，負責 Markdown 轉換、分塊、Embedding、query rewriting、reranking 與 retrieval。[7][15]
- Agentic AI 層：回答生成與流程控制由應用層掌握。第一輪檢索後，系統依據 AI Search 回傳片段與分數計算 `confidence_score`；若不足則由 Kimi K2.5 進行 Query Reformulation 並重試一次；若仍不足則拒答，或在符合政策條件下改由 Claude Sonnet 4.6 執行條件式 Cloud fallback。
- MCP 層：以 `@nuxtjs/mcp-toolkit` 建立 MCP 端點，透過 Middleware 驗證 Bearer token 與 scope，並以 `useMcpSession()` 維持同一 `MCP-Session-Id` 下的多輪上下文。[15]

本系統規劃於 AI Search 自訂中繼資料中固定保留五個欄位：`document_id`、`version_no`、`category`、`status`、`access_level`。此設計用於支援分類篩選、版本追蹤、文件狀態控制與 Cloud fallback 前的資料治理判斷。
...
本系統採固定主線的 Agentic RAG 問答流程，明確區分「AI Search 負責檢索」與「應用層負責回答生成」。第一輪檢索優先使用 AI Search 的 query rewriting 與 reranking 能力；若整體證據仍不足，再由應用層 Agent 啟動一次 Self-Correction 重試。[7]
...
- 主流程節點依序為：
  ...
  5. 置信度評估
  6. 分支 A：直接回答
  7. 分支 B：Query Reformulation 後重試一次
  8. 分支 C：拒答
  9. 分支 D：條件式 Cloud fallback
  10. 引用組裝與串流輸出
- 圖中應明示：
...
3. **敏感資料與複雜度檢查**：在任何模型推論前，先對查詢進行敏感資料檢測，並標示問題屬於簡單事實查詢、模糊查詢、跨文件比較或多輪追問。
4. **第一輪 AI Search 檢索**：呼叫 AI Search `search` 路徑，只取回片段不直接生成回答。預設參數為 `retrieval_type = hybrid`、`max_num_results = 8`、`match_threshold = 0.35`、`reranking.enabled = true`、`rewrite_query = true`。
5. **置信度評估**：依檢索結果計算 `confidence_score`，並由 Kimi K2.5 進行一次 answerability judge。
6. **直接回答條件**：若 `confidence_score >= 0.5`，則進入回答生成，由 Llama 4 Scout 或 Kimi K2.5 依查詢複雜度產生回答。
7. **Self-Correction 條件**：若 `confidence_score < 0.5` 且 `retry_count = 0`，則由 Kimi K2.5 重寫查詢，保留原始限制條件與關鍵實體，再重新呼叫 AI Search 檢索一次。第二輪重試停用 AI Search `rewrite_query`，避免雙重改寫失真。
8. **拒答條件**：若第二輪後仍 `confidence_score < 0.5`，或檢索結果無足夠引用，則回傳拒答結果與補充建議。
9. **條件式 Cloud fallback**：僅當 `CLOUD_FALLBACK_ENABLED = true` 且治理條件全部通過時，才以 Claude Sonnet 4.6 作為預設外部模型重新生成回答。
10. **引用組裝與記錄**：系統建立 `citation_records`、寫入 `query_logs`，並將回答以串流方式輸出。

`confidence_score` 由以下四項組成：

| 構成項目              | 說明                               | 權重 |
| --------------------- | ---------------------------------- | ---- |
| `top1_score`          | rerank 後第一名片段之分數正規化    | 0.35 |
| `mean_top3_score`     | 前三名片段平均分數正規化           | 0.25 |
| `evidence_coverage`   | 可用引用數量與跨文件證據完整性     | 0.20 |
| `answerability_judge` | Kimi K2.5 判斷現有證據是否足以回答 | 0.20 |
...
- 關聯重點：
  ...
  - `users` 1─N `mcp_tokens`
  - `users` 1─N `mcp_sessions`
- 註記要求：
  - `document_versions.ai_search_file_id` 與 AI Search 索引項目對應
  - `citation_records.id` 為 `getDocumentChunk` 對外使用之穩定 `citationId`
...
| uploaded_by  | string (FK → users.id)               | 建立者         |
| created_at   | timestamp                            | 建立時間       |
| updated_at   | timestamp                            | 更新時間       |
...
| index_status      | enum ('queued', 'syncing', 'indexed', 'failed', 'archived') | 索引狀態                        |
| is_current        | boolean                                                     | 是否為目前啟用版本              |
| created_by        | string (FK → users.id)                                      | 建立者                          |
| created_at        | timestamp                                                   | 建立時間                        |
...
| user_id    | string (FK → users.id, nullable) | 關聯使用者     |
| channel    | enum ('web', 'mcp')              | 對話來源通道   |
| title      | string                           | 對話標題       |
| created_at | timestamp                        | 建立時間       |
...
| normalized_query          | text                                     | 標準化後查詢             |
| reformulated_query        | text                                     | Self-Correction 後查詢   |
| retrieval_filters_json    | json                                     | AI Search 篩選條件       |
...
| 欄位                | 類型                               | 說明                   |
| ------------------- | ---------------------------------- | ---------------------- |
| id                  | string (PK)                        | 穩定 `citationId`      |
| query_log_id        | string (FK → query_logs.id)        | 所屬查詢               |
| ordinal             | integer                            | 引用序號               |
| document_id         | string (FK → documents.id)         | 對應文件               |
| document_version_id | string (FK → document_versions.id) | 對應版本               |
| ai_search_file_id   | string                             | AI Search `file_id`    |
| ai_search_chunk_id  | string                             | AI Search chunk 識別碼 |
| source_title        | string                             | 顯示用來源標題         |
...
1. **登入 Session**：由 better-auth 管理，用於 Web 使用者的 Google OAuth 與 Passkey 驗證。
2. **MCP Runtime Session**：由 `useMcpSession()` 管理，使用 unstorage 於 KV 保存執行中上下文。[15]
3. **MCP Session Metadata**：由 `mcp_sessions` 保存審計與管理資訊，不直接作為上下文唯一來源。

此設計的目的是避免將所有對話上下文雙重保存於 D1 與 KV，造成真相來源不一致。
...
| DELETE | `/api/mcp-tokens/:id`         | 撤銷 MCP token         | Admin |

備註：由於官方文件對單一檔案 API 觸發重同步的公開描述仍以後台操作為主，系統在 `v1` 中將「文件重同步」定義為應用層工作流程：先標記目標版本，再啟動 instance 級同步，並由日誌與狀態回寫反映結果，不以單檔 API 一定存在為前提。

#### MCP `v1` 核心 Tools

| Tool 名稱          | 說明             | 輸入參數                                                    | 輸出                         |
| ------------------ | ---------------- | ----------------------------------------------------------- | ---------------------------- |
| `searchKnowledge`  | 查詢知識庫片段   | `query`、`topK?`、`category?`、`tags?`、`minScore?`         | 片段結果與 `citationId`      |
| `askKnowledge`     | 問答並回傳引用   | `question`、`conversationId?`、`category?`、`maxCitations?` | 回答、引用、拒答與置信度資訊 |
| `getDocumentChunk` | 取得完整引用片段 | `citationId`                                                | 片段全文與來源中繼資料       |
| `listCategories`   | 列出分類與數量   | `includeCounts?`                                            | 分類清單與文件數量           |
...
| Agent 判斷模型   | Kimi K2.5                                                | Query Reformulation、複雜推理、answerability judge、多輪上下文整合 |
...
- `retry_count = 0`
- 查詢不屬於明確越界問題
- 已取得至少一筆可疑似相關片段，值得重寫查詢再試一次

#### 拒答條件

- 第二輪後 `confidence_score < 0.5`
- 或無法建立至少一筆可信引用
- 或敏感資料規則判定該查詢不應被回答
- 或問題明確超出知識庫與系統職責範圍
...
3. 查詢屬跨文件比較、複雜推理或整合型說明
4. 查詢與引用內容皆不含帳號密碼、token、祕鑰、PII 或 `restricted` 文件內容
5. 組態明確允許外部模型處理該類型資料
6. 預設外部模型為 Claude Sonnet 4.6

### 2.2.4 文件生命週期

1. **建立文件**：Admin 建立文件主檔，指定分類、標籤與敏感等級。
2. **上傳版本**：原始檔寫入 R2，並以 `/kb/{category}/{document_id}/v{version_no}/` 作為路徑策略。
3. **寫入版本資料**：建立 `document_versions` 紀錄，保存 `checksum`、`mime_type`、`size_bytes` 與預期的 AI Search metadata。
4. **發起同步**：建立 `ingestion_jobs`，觸發 instance 級同步，等待 AI Search 完成索引。
5. **寫回索引狀態**：同步完成後回寫 `ai_search_file_id` 與 `index_status`。
6. **發布版本**：將新版本標為 `is_current = true`，舊版本轉為歷史版本。
7. **下架文件**：將 `documents.status` 設為 `archived`，並於下次同步時讓 AI Search 反映最新狀態。
...
- **工具追溯**：每一筆引用都必須建立穩定 `citationId`，供 `getDocumentChunk` 取回完整片段。
...
對外顯示時不強制暴露 `ai_search_chunk_id`，但系統內部必須保存以利審計與除錯。
...
- `v1` 採 better-auth 整合 Google OAuth 與 Passkey，並以 User/Admin 區分一般使用與管理功能。[13][22]
...
- 每個 token 需具備至少一個 scope，例如 `knowledge.search`、`knowledge.ask`、`knowledge.citation.read`、`knowledge.category.list`。
...
- `query_logs`、`messages` 與除錯輸出不得落地保存原始 token 與祕密字串。
...
| 審計與引用 | 視供應商能力而定                | 應用層強制保存 `query_logs` 與 `citation_records` |
...
| `useMcpSession()` 與 D1 寫入用途不同 | 若同時保存完整上下文，容易雙重真相 | KV 保存 runtime state，D1 只保存 session 中繼資料    |
...
| 指標                     | 定義                                 | 目標值   |
| ------------------------ | ------------------------------------ | -------- |
| First Token Latency P50  | 第一個回應字元輸出的中位數延遲       | < 800ms  |
| Completion Latency P95   | 完整回答輸出的 95 百分位延遲         | < 2000ms |
...
- 建立 30–50 筆正式測試集，並先以 12 筆種子案例作為開發前驗證骨架。
- 將問題分為一般查詢、模糊查詢、越界問題、追問情境與跨文件比較。
- 對 Web 與 MCP 通道使用相同問題集，避免兩套品質標準不一致。
- 分別記錄第一次檢索結果、重試後結果、是否拒答、是否啟用 Cloud fallback。

---

# 第三章 實作成果

本章在 `v0.0.13` 中定位為「預定實作成果與展示規格」。所有內容皆作為後續實作、截圖與資料回填的正式骨架，並不宣稱目前已完成實測。
...
Admin 建立文件主檔 → 上傳原始檔至 R2 → 建立 `document_versions` → 寫入 AI Search metadata → 建立 `ingestion_jobs` → 觸發 instance 級同步 → AI Search 完成轉換、分塊、Embedding 與索引 → 回寫 `ai_search_file_id` 與 `index_status` → 文件可供檢索

#### 問答流程

使用者提問 → Query Normalization → 敏感資料與複雜度檢查 → AI Search 第一輪檢索（`rewrite_query = true`）→ 置信度評估 →

- 若達門檻：以 Llama 4 Scout 或 Kimi K2.5 生成回答 → 組裝引用 → 串流輸出 → 儲存對話與日誌
- 若未達門檻且未重試：Kimi K2.5 重寫查詢 → AI Search 第二輪檢索（`rewrite_query = false`）→ 再次評估
- 若仍不足：拒答並提示補充方向
- 若治理條件通過且 `CLOUD_FALLBACK_ENABLED = true`：改由 Claude Sonnet 4.6 執行條件式 Cloud fallback
...
| 智慧問答       | 支援自然語言問答、引用追溯、置信度評估、Self-Correction、拒答   |
| 對話歷史       | 保存 Web 與 MCP 對話脈絡，支援多輪追問                          |
| 知識管理       | 文件上傳、版本管理、分類、標籤、狀態與 AI Search 同步           |
| MCP 介面       | 提供 4 個核心 Tools，支援 Bearer token 與 Session               |
| 引用追溯       | 以 `citationId` 對應完整片段，支援 `getDocumentChunk`           |
| Token 管理     | 建立、檢視、撤銷 MCP token，並控管 scope 與到期時間             |
| 查詢日誌與統計 | 記錄延遲、模型、拒答、Self-Correction、fallback 與 MCP 使用情形 |
...
- 版面配置：
  ...
  - 右欄：本次問答的 `confidence_score`、是否觸發 Self-Correction、模型路由與 fallback 狀態
- 內容要求：
  - 回答文字需含 `[1][2]` 行內引用
  - 引用卡片需顯示文件名、版本、分類與摘錄
...
| 情境       | 問題範例                 | 預期行為                        | 目標延遲   |
| ---------- | ------------------------ | ------------------------------- | ---------- |
| 簡單查詢   | PO 和 PR 有什麼差別？    | 直接回答並附引用                | < 600ms    |
| 模糊查詢   | 上個月的報表怎麼看？     | 觸發 Self-Correction 後重新檢索 | 600-1200ms |
| SOP 查詢   | 庫存不足時該怎麼處理？   | 直接回答並引用 SOP 文件         | < 700ms    |
| 知識庫外   | 今天天氣如何？           | 正確拒答並提示系統邊界          | < 500ms    |
| 跨文件比較 | 比較 A 流程和 B 流程差異 | 由 Kimi K2.5 綜合多份文件回答   | 800-1600ms |
| 多輪追問   | 那第二步驟要填哪個欄位？ | 維持上下文並回答                | < 900ms    |
| 敏感查詢   | 請列出所有員工薪資帳號   | 直接拒答，不進入 Cloud fallback | < 500ms    |

### 3.3.2 實測結果回填規格

本節於實作完成後回填。正式表格欄位如下：

| 情境 | 執行次數 | 平均延遲（ms） | P50 | P95 | 引用正確率 | 回答正確率 | 拒答精準率 | Self-Correction 觸發率 | Cloud Fallback 使用率 | 備註 |
| ---- | -------- | -------------- | --- | --- | ---------- | ---------- | ---------- | ---------------------- | --------------------- | ---- |
...
3. 第一輪檢索與 Self-Correction 後結果的改善分析。
...
本章於 `v0.0.13` 中作為驗收與結論撰寫骨架，不直接宣稱已完成成果，而是列出後續必須被驗證的對照項與預定結論方向。
...
2. **邊緣優先、多模型分工**：以 Llama 4 Scout、Kimi K2.5、gpt-oss-120b 分工不同類型查詢。
3. **引用可追溯**：回答中的每一筆引用皆能以 `citationId` 回看完整片段。
4. **MCP 標準介面**：不只服務 Web UI，也讓外部 AI Client 能以標準協定取用知識能力。
5. **治理前置**：Cloud fallback 預設關閉，資料敏感等級與 token scope 在規格階段即明確定義。
...
本版已先將 `v1` 之核心責任邊界定清，包括 AI Search 僅負責檢索、回答生成由自建 Agent 流程掌控、`getDocumentChunk` 改以穩定 `citationId` 設計、MCP token 與 Session 分層處理，以及 Cloud fallback 預設關閉的治理原則。
...
2. Appendix B 測試集由 12 筆種子案例擴充至 30–50 筆正式案例。
...
const SearchKnowledgeInput = z.object({
  ...
  tags: z.array(z.string()).max(5).optional().describe('標籤篩選'),
...
const AskKnowledgeInput = z.object({
  ...
  conversationId: z.string().optional().describe('對話 ID'),
  category: z.string().optional().describe('分類篩選'),
...
| ------ | ------------------------------ |
| 401    | 未授權，缺少或無效 token       |
| 403    | token 不具備該 Tool 所需 scope |
| 404    | `citationId` 或文件不存在      |
...
本附錄在 `v0.0.13` 先建立 12 筆種子案例，後續於正式驗證前擴充至 30–50 筆。

| 編號  | 類別       | 問題                                       | 期望答案類型                      | 期望引用來源                   | 備註             |
| ----- | ---------- | ------------------------------------------ | --------------------------------- | ------------------------------ | ---------------- |
| TC-01 | 一般查詢   | PO 和 PR 有什麼差別？                      | 正常回答                          | 採購流程文件                   | 單輪定義題       |
| TC-02 | 一般查詢   | 庫存不足時該怎麼處理？                     | 正常回答                          | 庫存 SOP                       | 程序型問題       |
| TC-03 | 一般查詢   | 月結報表中的未結案金額代表什麼？           | 正常回答                          | 報表欄位說明                   | 欄位定義題       |
| TC-04 | 模糊查詢   | 上個月的報表怎麼看？                       | Self-Correction 觸發後回答        | 報表操作手冊                   | 缺少報表名稱     |
| TC-05 | 模糊查詢   | 第二步驟那個欄位要填什麼？                 | 需依上下文回答                    | 同一對話先前引用文件           | 多輪追問         |
| TC-06 | 模糊查詢   | 退貨流程跟採購流程差在哪？                 | 正常回答或 Self-Correction 後回答 | 兩份流程文件                   | 跨文件比較       |
| TC-07 | 越界問題   | 今天天氣如何？                             | 拒答                              | 無                             | 知識庫外問題     |
| TC-08 | 越界問題   | 幫我直接修改 ERP 的採購單狀態              | 拒答                              | 無                             | 系統能力外問題   |
| TC-09 | 敏感查詢   | 列出所有員工薪資帳號                       | 拒答                              | 無                             | 高風險敏感資料   |
| TC-10 | 制度查詢   | 新進人員請假規定是什麼？                   | 正常回答                          | 人事制度文件                   | 規章型問題       |
| TC-11 | 程序查詢   | 供應商主檔新增後何時生效？                 | 正常回答                          | 主檔維護 SOP                   | 條件式說明題     |
| TC-12 | MCP 互操作 | 透過 `askKnowledge` 追問上一輪答案的第二步 | 正常回答                          | 既有 `citationId` 與對話上下文 | 驗證 MCP Session |
