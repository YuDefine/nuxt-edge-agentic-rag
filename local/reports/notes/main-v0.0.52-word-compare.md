...
- 來源 DOCX：`local/reports/latest.docx`
...
- 狀態：DOCX 有同名段落
- 原始標題：`### 1.1.3 Serverless 邊緣運算帶來的機會`
...
- 狀態：DOCX 無同名段落，需人工定位
- 原始標題：`#### 2.2.2.1 內部 REST API（前端與管理後台使用）`
...
- 狀態：DOCX 無同名段落，需人工定位
- 原始標題：`#### 2.2.2.3 MCP Resources、Dynamic Definitions、Evals`
...
- 狀態：DOCX 無同名段落，需人工定位
- 原始標題：`#### 2.2.3.3 常數與 feature flag 凍結規則`
...
- 狀態：DOCX 無同名段落，需人工定位
- 原始標題：`#### 2.2.3.5 Self-Correction 觸發條件`
...
- 狀態：DOCX 無同名段落，需人工定位
- 原始標題：`#### 2.2.3.7 不納入現階段的 Cloud fallback`
...
- 狀態：DOCX 無同名段落，需人工定位
- 原始標題：`#### 2.2.4.1 上傳與 Ingestion Guardrails`
...
- 狀態：DOCX 無同名段落，需人工定位
- 原始標題：`#### 2.4.1.2 `allowed_access_levels` 推導與存取矩陣`
...
- 狀態：DOCX 無同名段落，需人工定位
- 原始標題：`### 5.2.4 Fallback Synthesizer 取捨說明`
...
- 狀態：DOCX 無同名段落，需人工定位
- 原始標題：`#### D.1.3 Runtime Secrets 與憑證設定`
...
- 狀態：DOCX 無同名段落，需人工定位
- 原始標題：`#### D.1.4 Google OAuth Redirect URI 設定`
...
- 狀態：DOCX 無同名段落，需人工定位
- 原始標題：`#### D.2.3 Post-deploy 煙霧測試與 Tag 命名`
...
- 狀態：DOCX 無同名段落，需人工定位
- 原始標題：`#### D.3.4 Secrets 與 Env Var 還原`
