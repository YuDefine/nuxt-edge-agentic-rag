...
開始實作時，應先完成可部署、可驗證、可答辯的 Web 核心閉環，再將延伸能力納入後續規劃。

若時程或平台能力與規格衝突，應優先確保目前範圍可部署、可驗證、可答辯；後續規劃則作為架構演進補充。

建置順序以 Web 問答主線優先，MCP Tools 應共用同一套檢索、引用與治理核心，以維持 Web 與 MCP 通道的一致性。

因此，本報告在實作前的主要任務，不是把所有可能延伸能力都寫成既定成果，而是先確認哪些規則屬於答辯與落地的核心驗收契約，哪些細節則應保留到 Preview 驗證與實作收斂後再鎖定。

本階段聚焦於可部署、可驗證、可答辯的企業知識庫問答閉環；ERP 交易寫回、多租戶計費、跨雲模型備援、MCP 多輪 session、LINE Login 與細粒度 ACL，則作為後續架構演進與企業級導入時的延伸能力。
...
為避免把答辯稿中的佔位內容誤判為開始實作的阻礙，本節將「核心設計項目」與「實作後再回填」明確分開。開始實作前，只需凍結首批範圍、資料模型、決策常數與治理 invariant；圖面、實測數據與結論不得反向綁架開工排程。
...
本章以支撐實作前決策為目標。內容可分為兩類：其一是 `current-version-only`、引用回放、授權隔離與記錄遮罩等核心驗收規則；其二是會隨平台版本、SDK 與部署環境微調的實作細節。前者應先定清，後者則保留調整空間，但需維持核心契約一致。
...
3. **集中驗證**：mobile / tablet viewport 之實機截圖與 `@nuxt/a11y` dev report 由 EV-06 統一整理（詳見 §3.3.2.3 表 44），不在本節逐張重複呈現。
...
本節 7 張實機截圖（圖 7 至圖 13）皆於 2026-04-21 於 local dev 環境拍攝，呈現登入、問答、文件管理、MCP Token、成員、訪客政策與用量頁的代表狀態；補充證據矩陣另見 §3.3.2.3 表 44。
...
本節將問答案例、EV 證據、架構圖、流程圖、實機畫面與附錄 B 資料集整理成同一套交付版證據矩陣。其目的在於讓讀者能直接看出每類證據支撐哪些 Acceptance ID，並區分「核心驗收證據」與「營運觀測資料」兩種不同用途。

表 44 交付版驗收證據整理

| 證據類型                     | 對應 Acceptance ID                | 資料集 / 來源                          | 判定依據                                                                                                                                               | 報告用途                   |
| ---------------------------- | --------------------------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------- |
| 表 41 實測情境彙總統計       | A02 / A05 / A06 / A07             | 正式驗收資料集 30–50 筆                | 覆蓋 §2.4.4.4 硬性驗收三項（Current-Version / Restricted / Redaction）均 100%，品質驗收四項（Citation / Answer / Refusal / MCP Success）達表 30 目標值 | 品質統計格式與營運觀測基準 |
| 表 42 TC 逐案測試結果        | A02 / A05 / A06 / A07             | 附錄 B 測試案例                        | 每一筆案例皆符合 `gold facts` / 必要引用 / 不可犯錯三項判定原則，不可犯錯命中即整案 fail                                                               | 問答品質逐案追溯           |
| 圖 7 至圖 13 實機操作畫面    | A02 / A03 / A08 / A14             | 2026-04-21 local dev 實機截圖          | 呈現登入、問答入口、文件管理、MCP Token、成員管理、訪客政策與 AI Gateway 用量頁之核心狀態                                                              | 介面與操作流程佐證         |
| EV-01 至 EV-07 證據文件      | A01 / A03 / A08 / A11 / A13 / A14 | `docs/verify/` 與 evidence manifest    | 部署、權限、版本交易、retention、redaction、響應式與 Web chat persistence 均有獨立證據路徑                                                             | 驗收證據索引               |
| 圖 1 至圖 6 架構與流程圖     | A01 / A02 / A05                   | §1.3.2、§1.3.3、§2.1.2 敘述            | 圖面與正文敘述一致，核心四層邊界、活動流程、ER 關係與開發時程皆可對照                                                                                  | 架構與流程說明             |
| 附錄 B 測試案例集            | A02 / A05 / A06 / A07             | 初始驗證案例與正式驗收欄位定義         | 每筆案例保留適用通道、gold facts、必要引用、不可犯錯與預期 `http_status`                                                                               | 驗收規則與答辯展示案例來源 |
| 封面日期、目錄頁碼與圖表頁碼 | 全域                              | Word / Pandoc 排版輸出                 | 日期、頁碼與圖表目錄由正式排版工具產生，內容以本文件章節與圖表清單為準                                                                                 | 排版層資訊                 |
| 套件小版本號與實際模型名稱   | A01 / A02                         | `pnpm-lock.yaml` / Workers AI 模型清單 | 與通過核心閉環驗證之 lockfile、部署設定與 `query_logs.config_snapshot_version` 對齊                                                                    | 部署可追溯資訊             |

此表與既有表 4（佔位內容與回填時機）互補：表 4 說明規劃初期如何避免圖表佔位阻塞實作，表 44 則整理交付版中可用來支撐驗收命題的證據來源。兩者合起來，使報告從規劃、實作到驗收皆能維持同一套可追溯脈絡。
...
「目前狀態」欄採三級分級：**結構性保障**（由 schema / migration 強制）、**自動化覆蓋**（整合測試已驗證結構式正確性）、**營運觀測**（需長期流量、成本或實模型品質資料持續累積）。

| Acceptance ID | 驗收目標                                                                                               | 對應章節            | 主要對應案例                      | 驗收證據                                                                                                      | 目前狀態                                                                                                              |
| ------------- | ------------------------------------------------------------------------------------------------------ | ------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| A01           | 邊緣原生架構可部署                                                                                     | 1.2.1、1.3.2        | EV-01                             | 部署紀錄、系統架構圖、Smoke Test                                                                              | 自動化覆蓋（系統已多次完成部署與 smoke 驗證；EV-01 作為交付版部署證據索引）                                           |
...
| A07           | MCP 4 個 Tools 可被外部 Client 正常使用                                                                | 2.2.2、3.2.2        | TC-12、TC-16、TC-17、TC-19、TC-20 | Claude Desktop / Cursor / MCP Inspector 測試結果                                                              | 自動化覆蓋（MCP contracts 15 檔 51 tests 全綠；外部 Client 實連列入 EV 證據）                                         |
| A08           | Google OAuth + 三級角色 RBAC + `guest_policy` dial 正常運作（`ADMIN_EMAIL_ALLOWLIST` 僅作 Admin seed） | 2.4.1、3.2.3        | EV-02、EV-06、TC-UI-01            | 登入截圖（圖 7 / 11 / 12）、升降／dial 切換前後的權限比對、`member_role_changes` 稽核紀錄、響應式與 a11y 證據 | 自動化覆蓋（TC-UI-01 7 assertions 含升降流程全綠；實機截圖與 OAuth 證據已納入 EV-02 / EV-06）                         |
...
| A13           | rate limit 與保留期限規則可被驗證                                                                      | 2.4.1               | EV-04                             | `429` 測試紀錄、`citation_records.expires_at` 清理作業摘要                                                    | 自動化覆蓋與營運觀測（`docs/verify/RETENTION_CLEANUP_VERIFICATION.md` 已備；長週期清理列入例行維運）                  |
...
1. 本研究以已完成的核心閉環、自動化驗證與短期實機操作證據為主，長期營運期數據與大樣本使用者行為分析列入營運觀測。
...
8. **§3.3.2.3 交付版驗收證據整理**：新增表 44，將問答案例、EV 證據、架構圖、流程圖、實機畫面與附錄 B 資料集整理成同一套交付版證據矩陣，讓讀者能直接看出每類證據支撐哪些 Acceptance ID。
...
1. 圖 1 至圖 6 以 Mermaid 圖呈現，正式 Word / PDF 版維持相同內容並由文書軟體產生頁碼。
...
3. **成本控制**：規劃期模型清單與 alias 仍可能變動，fallback 合成器可支援初始驗證與校準資料集而不累積 token 費用；實模型僅於正式驗收與 Preview / Staging 環境實際呼叫。
