...
function printUnresolvedReport(projectRoot, unresolved) {
  ...
  console.error('❌ Vue 元件解析檢查失敗：發現未註冊元件標籤')
  console.error('')
  ...
  console.error('')
  console.error(`共 ${unresolved.length} 筆未解析元件使用。`)
  console.error('提示：若剛新增元件，請先確認檔案命名與路徑，再執行一次 nuxt prepare / typecheck。')
}

function printParseErrorReport(projectRoot, parseErrors) {
  console.error('❌ Vue 模板語法檢查失敗：發現無法解析的 template')
  console.error('')
  ...
  console.error('')
  console.error(`共 ${parseErrors.length} 個檔案有 template 語法錯誤。`)
}
...
async function run() {
  ...
  try {
    ...
    console.error('❌ 無法讀取 .nuxt/components.d.ts，請先執行 nuxt prepare 或 pnpm typecheck。')
  ...
  console.log(`✅ Vue 元件解析檢查通過（掃描 ${files.length} 個 .vue 檔）`)
}
