...
#
# 掃描 / glob matching / baseline 比對全部收斂在 scan.mts（pre-commit / pre-push / CI /
# audit 四入口共用，見 scan.mts 檔頭）。
#
# review-rules-baseline.json 不存在時，baseline 視為全空（等同零容忍——任何既有違規都算
# 「新增」）；第一次導入 ratchet 的 consumer 應先跑
#   node vendor/review-rules/scan.mts --all --layer all --write-baseline
