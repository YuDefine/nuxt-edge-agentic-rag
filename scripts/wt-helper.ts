 ...
 *
 * 實證（2026-08-22 <consumer-e>）：main checkout 在 `feat/self-host-evlog-admin`
...
async function cmdAdd(slug, opts: WtOptions = {}) {
  ...
  // `wt-helper cleanup` then permanently destroys them (incident: <consumer-a>
  ...
  if (pendingStashName) {
    try {
      ...
      // files, leading to scope leak in the next commit (<consumer-a> 2026-05-18 incident:
      ...
      //
      // See pitfall-pre-fork-baseline-hides-in-flight-feature (2026-05-18 <consumer-a>
...
// added after <consumer-a> 2026-05-17 incident where 47 baseline files lived only in
...
function detectUnlandedFiles(consumerRoot, branchName) {
  ...
  // commit 時，那 N 個 commit 動過的檔全部被算進來，而 branch 從沒碰過它們。實測 <consumer-g>
...
// 於 <consumer-g> 實測兩條**已確認 land** 的 branch：`git merge-tree --write-tree` 兩條都非
...
async function cmdCleanup(slug, opts) {
  ...
  // The third gate (uncommitted) was added after <consumer-a> 2026-05-17 incident
  ...
  if (squashLanded && !mergedBranches(consumerRoot).has(branchName)) {
    ...
    if (paths.length) {
      ...
      // gate below is what caught the <consumer-a> 2026-05-17 evaporation; this block was born 2026-09-06
  ...
  // 2026-08-22 於 <consumer-g> 實測：20 個 session worktree 有 8 個的髒檔 100% 屬於這一類。
  //
...
async function cmdMergeBack(slug, opts: WtOptions = {}) {
  ...
  // Pre-flight: worktree dirty tracked-file check (<consumer-a>-1J 2026-05-18 incident).
  ...
  // (cmdCleanup --force-discard-uncommitted gate, post-<consumer-a> 2026-05-17 fix).
  let baselineRefs = []
  try {
  ...
  if (blockers.length > 0) {
    ...
    if (!opts.autoStash && blockers.length <= 3 && blockers.every((b) => b.type === 'modified')) {
      ...
      try {
        ...
        // git 2.50.1 (<consumer-a> 2026-05-18: 22 blockers requested → 74 files stashed
