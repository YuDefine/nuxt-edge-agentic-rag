#!/usr/bin/env node
// 🔒 LOCKED — managed by clade · Source: vendor/scripts/spectra-advanced/roadmap-sync.ts · 改這裡無效，下次 propagate 會覆寫；請改 $CLADE_HOME/vendor/scripts/spectra-advanced/roadmap-sync.ts
/**
 ...
 *
 * Usage:
 *   node scripts/spectra-advanced/roadmap-sync.ts           # full sync (default)
 *   node scripts/spectra-advanced/roadmap-sync.ts --check   # validate, no write
 *   node scripts/spectra-advanced/roadmap-sync.ts --json    # emit report as JSON
 *
...
function parseArgs(argv: string[]): CliOptions {
  ...
  for (const arg of argv.slice(2)) {
    if (arg === '--check') opts.check = true
      ...
      console.log(
        'Usage: roadmap-sync.ts [--check] [--json]\n' +
