#!/usr/bin/env node
// 🔒 LOCKED — managed by clade · Source: vendor/scripts/spectra-advanced/claims-status.ts · 改這裡無效，下次 propagate 會覆寫；請改 $CLADE_HOME/vendor/scripts/spectra-advanced/claims-status.ts

import { collectClaims, formatAge, loadClaimsRuntimeConfig } from './claims-lib.ts'
...
function parseArgs(argv: string[]): CliOptions {
  ...
  for (const arg of argv.slice(2)) {
    if (arg === '--json') opts.json = true
      ...
      console.log(
        'Usage: claims-status.ts [--json] [--session-summary]\n' +
