 ...
 *
 * Usage:
 *   node scripts/spectra-advanced/audit-screenshot-quality.ts
 *   node scripts/spectra-advanced/audit-screenshot-quality.ts <change-name>
...
import { resolveEvidence } from '../lib/evidence-store.ts'
import type { ManualReviewItem } from '../review-gui.types.ts'

// review-gui.ts is hosted in clade central (per propagate manifest @ 76a202a, 2026-05-19);
// consumer doesn't have a local copy. Resolve dynamically: prefer co-located (clade self),
// fall back to ${CLADE_HOME}/vendor/scripts/review-gui.ts (consumer).
const REVIEW_GUI_PATH = (() => {
  const localPath = new URL('../review-gui.ts', import.meta.url).pathname
  if (existsSync(localPath)) return localPath
  const cladeHome = process.env.CLADE_HOME ?? `${process.env.HOME}/offline/clade`
  return `${cladeHome}/vendor/scripts/review-gui.ts`
})()
// Dynamic import remains runtime-resolved at this boundary. Audit consumes items as
// opaque dictionaries accessing canonical fields (id, kind, checked, raw, scoped, ...);
// canonical schema lives in clade/vendor/scripts/review-gui.ts (ManualReviewItem).
...
function parseArgs(argv: string[]): CliOptions {
  ...
  for (const arg of argv.slice(2)) {
    if (arg === '--json') {
      ...
      console.log(
        [
          'Usage: audit-screenshot-quality.ts [change-name] [--json] [--fail-on-issues]',
          '',
...
async function auditChange(changeName: string): Promise<{
  ...
  const parsed = (
    tasksContent ? parseManualReviewSections(tasksContent, { defaultKind }) : { items: [] }
  ) as { items: ManualReviewItem[] }
...
function applyMissingEvidenceRule(
  ...
  // 對齊 archive-gate.sh Check 4 與 review-gui.ts buildParentsWithScopedChildren —
...
// 對齊 review-gui.ts applyActionToLine / stripAnnotations 的 skip annotation
