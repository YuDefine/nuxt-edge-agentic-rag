#!/usr/bin/env node
// 🔒 LOCKED — managed by clade · Source: vendor/scripts/spectra-advanced/claim-work.ts · 改這裡無效，下次 propagate 會覆寫；請改 $CLADE_HOME/vendor/scripts/spectra-advanced/claim-work.ts
...
function usage(): string {
  return [
    'Usage:',
    '  claim-work.ts <change> [--task "..."] [--note "..."] [--path <repo-path>]',
    '                 [--owner <owner>] [--runtime <runtime>] [--session <id>]',
    '                 [--accept handoff|assign|manual] [--takeover] [--json]',
    '  claim-work.ts --heartbeat-from-path <file>',
  ].join('\n')
}
