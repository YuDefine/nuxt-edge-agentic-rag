...
# has no tunnel configured (e.g. <consumer-j>) — return exit 1 → main loop
...
run_tunnel_check() {
  ...
  local host=""
  local source_env=""
  # Scan common env file names + any `.env.<app>` for multi-app consumers (<consumer-g>).
...
# 92-line ## 人工檢查 block (<consumer-g> line-messaging-interaction: 41 s idle, past
...
for i in $(seq 0 $((PATTERN_COUNT - 1))); do
  ...
  for pos in $matched_pos; do
    ...
    # consumers like <consumer-j>); enrich when tunnel exists.
