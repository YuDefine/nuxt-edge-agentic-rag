...
function syncRoadmap(): SyncReport {
  ...
  // NOTE: when `cli.check && parkedSource === 'unavailable'`, we **don't**
  // short-circuit. Active Changes / Active Claims / Parallel Tracks are
  // computed from disk (no CLI needed) and MANUAL drift detection still runs
  // — short-circuiting here would make `--check` silently green even when
  // those have drifted. Parked block is preserved verbatim from existing
  // content below, so byte comparison stays clean as long as replaceBetween()
  // is normalisation-stable (which it is for unchanged input).
