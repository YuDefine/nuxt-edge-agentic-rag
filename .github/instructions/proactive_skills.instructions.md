...
Findings log 的分析由 `/design-retro` skill 負責（見 `.agents/skills/design-retro/SKILL.md`）。記錄本身只負責結構化紀錄，分析與改善建議交由 skill 在適當時機執行。
...
若 change 純後端（data model、API、config），不觸發 Design Checkpoint，直接走 Spectra 標準流程。判斷依據：change 的 tasks artifact 中是否有任何 task 涉及 `.vue` / `pages/` / `components/` / `layouts/` 檔案，且 git diff 中無 `.vue` 檔案。
...
**完整規則**：[`.github/instructions/ux-completeness.md`](./ux-completeness.md) — Definition of Done、Affected Entity Matrix、User Journeys、Exhaustiveness、State Coverage、心智模型、必禁事項。
