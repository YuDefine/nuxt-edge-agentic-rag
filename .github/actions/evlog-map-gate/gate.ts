#!/usr/bin/env node
// 🔒 LOCKED — managed by clade · Source: vendor/actions/evlog-map-gate/gate.ts · 改這裡無效，下次 propagate 會覆寫；請改 $CLADE_HOME/vendor/actions/evlog-map-gate/gate.ts

/**
 * gate.ts — evlog map ratchet gate
 *
...
interface EvlogRoute {
  checks?: Record<string, EvlogCheck>
  score?: number
  method?: string
  path?: string
  file?: string
 ...
 * 登記的豁免都會讓 gate 永久卡死。
 */
function normalizeMap(json: EvlogMap | null | undefined) {
...
function routeFailures(route: EvlogRoute) {
  const failed = []
