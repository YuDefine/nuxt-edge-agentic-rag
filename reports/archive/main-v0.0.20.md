![image1](main-v0.0.20_assets/image1.jpeg)
...
- 建立可追溯的查詢日誌、引用紀錄與 MCP 呼叫紀錄，作為後續驗證依據；管理統計儀表板改列 `v1.1` 延伸。
...
| 可觀測性 | 查詢日誌、`source_chunks` / `citation_records`、MCP 呼叫成功率、決策路徑與延遲統計、遮罩審計資訊                                     | 管理統計儀表板與延遲趨勢圖          |
...
- 應呈現內容：
  ...
  6. 以單一 Cloudflare Edge 邊界框包覆四層，強調邊緣優先與核心版不預設依賴外部模型
...
- 資料與受管理檢索層：以 R2 儲存原始文件與版本檔，D1 儲存結構化資料，KV 作為快取與 rate limit 計數器；Web Admin 文件上傳採應用層簽發一次性 signed URL 後直傳 R2。Cloudflare AI Search 連接既定資料來源後，負責 Markdown 轉換、分塊、Embedding、query rewriting、reranking 與 retrieval；應用層先以 metadata filter 套用 `status = active` 與可見 `access_level`，必要時可附帶 `version_state = current` 作為快篩提示，但不將遠端 metadata 視為發布真相來源。正式回答前一律以 D1 驗證 `document_version_id` 是否仍符合 `documents.status = active`、`document_versions.index_status = indexed` 與 `document_versions.is_current = true`，且引用必須對應到發布階段預先建立的 `source_chunks`。D1 才是 current-version-only 的唯一發布真相來源，AI Search metadata 僅作快篩與觀測用途。[7][15]
...
- MCP 層：以 `@nuxtjs/mcp-toolkit` 建立 MCP 端點，透過 Middleware 驗證 Bearer token 與 scope。`v1.0.0` 的 MCP 採無狀態呼叫，不建立 `MCP-Session-Id` 依賴；Web 對話與 MCP 工具契約因此分別對應「D1 持久化真相」與「單次請求契約」。若 `v1.1` 導入多輪上下文，runtime state 仍應保存於 KV，而非與 Web 對話混寫。[4][15][16][28]
...
- 以不依賴外部模型的 `v1.0.0` 核心版降低資料外送風險，後續若擴充外部模型再由治理閘道控管。
...
4. **第一輪 AI Search 檢索**：呼叫 AI Search `search` 路徑，只取回片段不直接生成回答。`v1.0.0` 初版預設參數為 `max_num_results = 8`、`ranking_options.score_threshold = 0.35`、`rewrite_query = true`，並強制套用 `status = active` 與 `access_level in allowed_access_levels`；若遠端 metadata 已同步 `version_state`，可額外帶入 `version_state = current` 作為快篩提示，但不得把它視為發布真相。若實作採用 Workers binding 或 REST API 封裝，參數名稱可依 SDK 調整，但應以應用層抽象欄位 `retrieval.maxResults`、`retrieval.minScore`、`retrieval.queryRewrite`、`retrieval.filters` 作為內部契約，避免不同 SDK 名稱直接滲入業務規格。[25] 取得候選片段後，應用層必須先以 D1 驗證 `document_version_id` 仍符合 `active/indexed/current` 可用版本，未通過者一律視為無效證據。
...
| locator_hash        | string                             | 由可取得的定位資訊雜湊而成，例如 chunk ordinal、檔名路徑、段落標記或應用層切塊索引；若供應商未提供頁碼或標題路徑，則不以其為必要前提 |
...
- 若供應商重切塊導致定位資訊改變，系統可為同一 `document_version_id` 建立新的 `source_chunks` 映射；`v1.0.0` 驗收重點是引用可回放，不要求跨重切塊維持相同 `citationId`。
- `source_chunks` 必須於 `smoke_pending` 階段預先建立完成；未完成者，該版本不得發布。
...
6. **Smoke retrieval 驗證**：遠端同步回報完成後，任務與版本先進入 `smoke_pending`。系統需以維運用查詢路徑針對目標 `document_version_id` 執行 smoke retrieval，確認候選片段可被取回，且可預先建立 `source_chunks` 映射。若無法建立可回放 `citationId`，則視為驗證失敗。
7. **發布版本**：僅當新版本 smoke retrieval 通過且 `source_chunks` 預建完成後，才可將 `document_versions.index_status` 標為 `indexed`，並以單一 transaction 把新版本切為 `is_current = true`、寫入 `published_at / published_by`，同時將舊版降級為歷史版本。此步驟受「每份文件僅一個 current 版本」之唯一約束保護。
...
9. **一致性保護**：AI Search metadata 僅為第一層快篩與觀測；回答前一律以 D1 post-verification 剔除非 `active/indexed/current` 片段。若剔除後已無有效證據，則視為無結果，不得回退到舊版內容。
...
- `v1.0.0` Web 上傳一律採一次性 signed URL 直傳 R2；應用伺服器不轉送大檔，僅負責簽發 upload URL、驗證 metadata 與建立版本紀錄。
...
- smoke retrieval 驗證除確認可檢回片段外，亦需確認片段文字足以預先建立 `source_chunks` 與引用回放資料；若只能取得摘要而無法回放片段，該版本不得發布。
...
| 成本控制   | 以外部 token 計費為主           | 以邊緣模型承擔常見查詢，`v1.0.0` 不啟用外部模型                    |
...
本章在 `v0.0.20` 中定位為「預定實作成果與展示規格」。所有內容皆作為後續實作、截圖與資料回填的正式骨架，並不宣稱目前已完成實測。
...
| Managed Retrieval       | Cloudflare AI Search                | Beta（以 2026-04 官方文件為準）               | 受管理檢索                           |
...
補充說明：AI 輔助開發流程以 Claude Code 與 spectra 為主，對應之規格驅動方法參考 [18][19][23]。
...
Admin 先取得一次性 signed URL 與 `pending_object_key` → 原始檔直傳 R2 → 驗證副檔名、MIME type、大小與 checksum → 建立 `document_versions`（預設 `is_current = false`、`index_status = queued`）→ 寫入 AI Search metadata（含 `document_version_id` 與 `folder` 路徑；`version_state` 若存在僅作觀測提示）→ 建立 `ingestion_jobs`（`status = queued`）→ 觸發 instance 級同步 → AI Search 完成轉換、分塊、Embedding 與索引 → 任務與版本轉為 `smoke_pending` → 執行以 `document_version_id` 為主的 smoke retrieval 並預建 `source_chunks` → 通過後回寫 `ai_search_file_id`、`index_status = indexed`、`indexed_at` → 以 transaction 將新版本切為 `is_current = true` 並寫入 `published_at / published_by` → 文件可供正式檢索
...
- 若 `retrieval_score >= 0.70` 且 `cross_document_gate_failed = false`：依固定模型路由以 Llama 4 Scout 或 Kimi K2.5 生成回答 → 查找既有 `source_chunks` → 建立 `citation_records` → 串流輸出 → 儲存遮罩後日誌
...
| 查詢日誌與營運治理 | 記錄延遲、judge、拒答、Self-Correction、版本與遮罩執行情形；管理儀表板改列 `v1.1`                                           |
...
本章於 `v0.0.20` 中作為驗收與結論撰寫骨架，不直接宣稱已完成成果，而是列出後續必須被驗證的對照項與預定結論方向。
...
[26] Cloudflare, "Release note - Cloudflare AI Search docs," 2025-2026. URL: https://developers.cloudflare.com/ai-search/platform/release-note/
...
[28] Nuxt Modules, "Nuxt MCP Toolkit," 2025-2026. URL: https://mcp-toolkit.nuxt.dev/
...
本附錄在 `v0.0.20` 先建立 15 筆種子案例，後續於正式驗證前擴充至 30–50 筆。
...
| TC-06 | 跨文件比較      | 退貨流程跟採購流程差在哪？                                                 | `judge_pass` 或 `self_corrected` | 正常回答                   | 兩份流程文件         | 跨文件比較                                                    |
...
| TC-12 | MCP 互操作      | 先以 `askKnowledge` 取得回答，再用 `getDocumentChunk` 回看其中一筆引用片段 | `direct` 或 `judge_pass`         | 正常回答並可回放引用       | 既有 `citationId`    | 驗證無狀態 MCP 工具鏈                                         |
| TC-13 | 權限治理        | 以未具 `knowledge.restricted.read` 的 token 查詢 restricted 文件           | 403                              | 不可取得 restricted 內容   | 無                   | 驗證 scope 過濾                                               |
| TC-14 | restricted 存取 | Admin 於 Web 問答查詢 restricted 制度內容                                  | `direct` 或 `judge_pass`         | 正常回答                   | restricted 文件      | 驗證 Admin Web 可讀 restricted                                |
