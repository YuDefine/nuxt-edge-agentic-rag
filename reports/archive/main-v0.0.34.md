...
在系統設計上，前端以 Nuxt 提供 Web 問答與管理後台，後端部署於 Cloudflare Workers，整合 D1、R2、KV、Workers AI 與 Vercel AI SDK。`v1.0.0` 核心版先收斂於可部署、可驗證、可答辯的最小閉環：Google OAuth 與管理員 allowlist、文件上傳與版本管理、AI Search 同步、Web 問答、可回放引用，以及 4 個無狀態 MCP Tools。為避免版本真相混亂，正式回答一律以 D1 驗證 `active/indexed/current` 狀態，並以前處理階段預先建立的 `source_chunks` 作為 `citationId` 回放來源。

在問答決策上，本系統採分段式信心判斷：Query Normalization 僅做規則式標準化；第一輪檢索可使用 query rewrite；僅於邊界區間才由較強推理模型執行 answerability judge 與 Query Reformulation；若證據仍不足則拒答。驗證面將以分層資料集檢驗回答正確率、拒答精準率、引用正確率、MCP Tool 成功率、current-version-only 檢索正確性與遮罩完整性，作為後續實作、測試與正式報告回填基準。此外，本研究將 Web 對話持久化與 MCP 無狀態契約分流，並在設計階段先界定 `restricted` 存取、rate limit、記錄遮罩、對話刪除與 180 天稽核保留等治理規則，使實作過程能直接對照可驗收的資料模型與測試案例。在資料處理上，核心驗收以 `md`、`txt` 與預先轉 Markdown 且經人工校閱之文件為主，先確保 current-version-only 與引用回放穩定，再逐步擴充 rich format 支援。正式模型名稱、套件版本、畫面截圖與統計數據，待核心版完成後再依實證結果回填。
...
- MCP 層：以 `@nuxtjs/mcp-toolkit` 建立 MCP 端點，透過 Middleware 驗證 Bearer token 與 scope。`v1.0.0` 的 MCP 採無狀態呼叫，不建立 `MCP-Session-Id` 相依性；Web 對話與 MCP 工具契約因此分別對應「D1 持久化對話輔助」與「單次請求契約」。若後續版本導入多輪上下文，runtime state 仍應保存於 KV，而非與 Web 對話混寫。[4][15][16]
...
| 記錄治理 | `message_state`、`content_text`、`content_redacted` 與 `query_logs` 遮罩欄位              | 高風險輸入不得以原文落地，也不得回到後續模型上下文                         |
...
| auth_source  | enum ('google')                        | 主要登入來源；`v1.0.0` 固定為 `google` |
...
| deleted_at   | timestamp                           | 使用者刪除對話之時間；一旦設定即不得再出現在一般列表、詳情 API 或後續模型上下文                        |
...
| content_text     | text                                                   | 僅保存通過安全檢查、允許持久化之 Web 對話原文，供多輪指代輔助與使用者本人查看；不作知識真相來源；使用者刪除對話後應立即清空、硬刪除或進入不可回復狀態 |
...
- 對話一旦進入刪除流程，即使底層仍暫存遮罩後審計資料，相關 `messages` 亦不得再回到一般使用者讀取路徑或後續模型上下文。
...
- `messages.content_text` 僅隨未刪除對話保留；使用者刪除對話時，對應原文與標題應立即清空、硬刪除或轉為不可回復狀態，並自一般 API 與後續模型上下文排除。
...
| Auth                    | Better Auth                              | 1.4.x（首次通過核心閉環驗證時鎖定）           | Google OAuth                                   |
| Managed Retrieval       | Cloudflare AI Search                     | 以 2026-04 官方公開功能為準                   | 受管理檢索                                     |
| AI SDK                  | Vercel AI SDK                            | 6.x（首次通過核心閉環驗證時鎖定）             | 回答生成與串流                                 |
| Edge Answer Role        | Workers AI（Preview 通過後鎖定實際模型） | -                                             | 單文件回答模型角色                             |
| Agent Judge Role        | Workers AI（Preview 通過後鎖定實際模型） | -                                             | Query Reformulation、複雜推理與 judge 模型角色 |
...
| 高風險輸入保護        | TC-15               | 貼上疑似 API token 或 PII 字串                                                                                         | 直接拒答，僅保存遮罩記錄，不寫入 `messages.content_text`                        | < 800ms      |
...
| A02           | AI Search 與自建 Agent 流程整合完成                         | 1.2.1、2.1.2  | TC-01、TC-04、TC-06、EV-01        | 查詢日誌、引用紀錄、模型路由紀錄                 | 待驗證   |
...
| A05           | Self-Correction 可改善模糊查詢                              | 2.1.2、2.4.4  | TC-04                             | 重試前後比較報告                                 | 待驗證   |
...
| A11           | 高風險輸入不會以原文寫入持久化紀錄                          | 2.4.1、2.4.4  | TC-15                             | `messages` / `query_logs` 稽核結果               | 待驗證   |
| A12           | 對外 MCP 契約不暴露內部診斷欄位                             | 2.2.2、附錄 A | TC-20                             | Tool 契約測試、回應範例                          | 待驗證   |
| A13           | rate limit 與保留期限規則可被驗證                           | 2.4.1         | EV-04                             | `429` 測試紀錄、清理作業摘要                     | 待驗證   |
...
2. 先把對話刪除、`messages.content_text` 清理、180 天稽核保留與 `citationId` 全域唯一性等治理語意封口，避免實作中途重改資料模型。
