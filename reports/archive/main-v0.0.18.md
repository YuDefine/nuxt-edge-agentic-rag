...
在技術架構上，系統以前端 Nuxt 4 應用為使用者與管理者入口，後端則透過 NuxtHub 部署於 Cloudflare Workers，整合 D1、R2、KV、Workers AI 與 Vercel AI SDK。檢索部分由 Cloudflare AI Search 負責資料同步、文件轉換、分塊、Embedding、query rewriting、reranking 與 retrieval；回答生成則不直接交由 AI Search 完成，而是由應用層自建的 Agent 流程控制，以保留分段式置信度評估、Self-Correction、拒答判斷、引用組裝與審計記錄等核心能力。身份與權限資料採 better-auth 核心表搭配應用層 `user_profiles` 分層管理；引用追溯則以應用層 `source_chunks` 保存可回放的 `citationId` 映射，並以 `citation_records` 保存單次回答中的引用快照，避免把單次查詢紀錄誤當成長期穩定來源識別碼。`v1` 對外契約僅保證 `citationId` 可回放本次引用片段與其版本脈絡，不保證跨版本或跨供應商重切塊後仍維持相同值。邊緣模型配置方面，系統將以 Llama 4 Scout 17B MoE 作為預設快速回答模型，Kimi K2.5 作為複雜推理、查詢重寫與邊界情境 answerability judge 模型，並以 gpt-oss-120b 作為邊緣備援模型；如經資料治理與 feature flag 條件允許，則以 Claude Sonnet 4.6 作為預設雲端備援目標，其他候選模型為 GPT 5.4、Gemini 3.1 Pro、Gemini 3 Flash 與 Claude Opus 4.6。[6][7][10]
...
為降低實作前期風險，`v1` 採「核心優先、延伸後置」原則，建議實作順序如下：

- `v1-core`：Web 問答、文件版本治理、AI Search 同步流程、引用回放、基本權限控管、查詢日誌與遮罩。
- `v1-plus`：Passkey、MCP Session、多輪追問、管理統計儀表板、條件式 Cloud fallback。

若時程或平台能力與規格衝突，應優先確保 `v1-core` 可部署、可驗證、可答辯，再擴充 `v1-plus`。
...
- 資料與受管理檢索層：以 R2 儲存原始文件與版本檔，D1 儲存結構化資料，KV 作為快取與 MCP Session 的 unstorage 驅動。Cloudflare AI Search 連接既定資料來源後，負責 Markdown 轉換、分塊、Embedding、query rewriting、reranking 與 retrieval；應用層先以 metadata filter 套用 `status = active` 與可見 `access_level`，必要時可附帶 `version_state = current` 作為快篩提示，但不將遠端 metadata 視為發布真相來源。正式回答前一律以 D1 驗證 `document_version_id` 是否仍符合 `documents.status = active`、`document_versions.index_status = indexed` 與 `document_versions.is_current = true`。D1 才是 current-version-only 的唯一發布真相來源，AI Search metadata 僅作快篩與觀測用途。[7][15]
...
- MCP 層：以 `@nuxtjs/mcp-toolkit` 建立 MCP 端點，透過 Middleware 驗證 Bearer token 與 scope，並以 `useMcpSession()` 維持同一 `MCP-Session-Id` 下的多輪上下文。Web 對話與 MCP Session 為兩條不同的真相來源：前者以 `conversations/messages` 持久化，後者以 KV 作為 runtime state，而 D1 僅保存 `mcp_sessions` metadata 與遮罩後查詢日誌。[15][28]

Cloudflare AI Search 每個 instance 最多支援 5 個 custom metadata 欄位。[7][24] 本系統 `v1` 固定保留 4 個核心欄位：`document_version_id`、`category`、`status`、`access_level`；`version_state` 僅於需要輔助後台觀測或同步檢查時作為第 5 個選用欄位，不作正式回答的硬性判斷依據。其中 `document_id` 與 `version_no` 不再額外占用 custom metadata，而是由 `folder = /kb/{category}/{document_id}/v{version_no}/` 路徑策略與 D1 回推。此設計是為了符合 AI Search custom metadata 上限，同時保留分類篩選、版本追蹤與 Cloud fallback 前的資料治理判斷。`documents.tags` 僅保留於 D1 供後台管理與後續延伸，不同步至 AI Search，也不作為 `v1` MCP 對外檢索契約參數。
...
4. **第一輪 AI Search 檢索**：呼叫 AI Search `search` 路徑，只取回片段不直接生成回答。`v1` 初版預設參數為 `max_num_results = 8`、`ranking_options.score_threshold = 0.35`、`rewrite_query = true`，並強制套用 `status = active` 與 `access_level in allowed_access_levels`；若遠端 metadata 已同步 `version_state`，可額外帶入 `version_state = current` 作為快篩提示，但不得把它視為發布真相。若實作採用 Workers binding 或 REST API 封裝，參數名稱可依 SDK 調整，但應以應用層抽象欄位 `retrieval.maxResults`、`retrieval.minScore`、`retrieval.queryRewrite`、`retrieval.filters` 作為內部契約，避免不同 SDK 名稱直接滲入業務規格。[25] 取得候選片段後，應用層必須先以 D1 驗證 `document_version_id` 仍符合 `active/indexed/current` 可用版本，未通過者一律視為無效證據。
...
| `evidence_coverage` | 由引用充足度、跨文件覆蓋度與驗證完整度計算                                   | 0.20 |
...
citation_sufficiency =
  min(valid_citation_count, required_citation_count) / required_citation_count
...
verification_integrity =
  verified_results.length === 0
    ? 0
    : valid_citation_count / verified_results.length

evidence_coverage =
  0.60 * citation_sufficiency +
  0.25 * document_diversity_score +
  0.15 * verification_integrity
```

上述定義的目的，是讓 `retrieval_score` 在 `v1` 階段即具備可重現、可測試、可比對的計算方式，且不會因單文件但證據明確的查詢被機械式低估。文中的 `0.35`、`0.45`、`0.55`、`0.70` 皆為 `v1.0.0` 初版預設值，屬部署設定而非對外 API 契約；正式上線前可依 15 筆種子案例與 30–50 筆正式測試集校準，但校準後必須同步更新文件與驗證報告，不得由不同路徑各自採用不同門檻。
...
| 欄位                | 類型                               | 說明                                                                                                                                 |
| ------------------- | ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| id                  | string (PK)                        | 版本範圍內可回放的 `citationId`，為 opaque ID；`v1` 僅保證能回放既有引用，不保證跨版本或跨重切塊後維持相同值                         |
...
- 若供應商重切塊導致定位資訊改變，系統可為同一 `document_version_id` 建立新的 `source_chunks` 映射；`v1` 驗收重點是引用可回放，不要求跨重切塊維持相同 `citationId`。
...
3. **Web 對話持久化**：僅將通過安全檢查、允許持久化的 Web 問答原文寫入 `conversations/messages.content_text`，作為 Web 多輪上下文唯一真相來源；若輸入命中高風險規則，則不得保存原文，只能寫入 `message_state = rejected_marker` 或 `redacted_only` 的事件紀錄、遮罩後審計副本與拒答結果，以保留 UI 時序但不把原文放入後續上下文。`content_redacted` 僅供審計與統計使用。
...
備註：Cloudflare AI Search 已提供同步 REST API，且後續新增了單檔同步能力；但 `v1` 仍將「文件重同步」定義為應用層工作流程：先標記目標版本，再呼叫可用的同步能力，並由日誌與狀態回寫反映結果，不把供應商特定 API 直接綁死在論文契約中。[26] 若同一 `document_version_id` 已存在 `queued`、`syncing` 或 `smoke_pending` 任務，應回傳既有任務或 `409`，避免重複排程。
...
為避免文件管理規格與 AI Search 實際限制脫節，`v1` 補充以下上傳與 ingestion 邊界：

- `v1` 優先支援 `md`、`txt`、`pdf`、`docx` 四類文件；其他格式先拒收，不承諾自動轉換。
...
- `v1` 不把供應商的自動轉檔品質視為保證值；若同一來源在不同 reindex 產生明顯不同切塊，應以最新發布版本重新驗證，而非假定舊有 chunk 對應仍然有效。
...
本章在 `v0.0.17` 中定位為「預定實作成果與展示規格」。所有內容皆作為後續實作、截圖與資料回填的正式骨架，並不宣稱目前已完成實測。
...
使用者提問 → Query Normalization → 權限、敏感資料與複雜度檢查（推導 `allowed_access_levels`）→ AI Search 第一輪檢索（`rewrite_query = true`，且 `status = active`；`version_state = current` 若存在僅作快篩提示）→ D1 post-verification 剔除非 `active/indexed/current` 片段 → 計算 `retrieval_score` →
...
本章於 `v0.0.17` 中作為驗收與結論撰寫骨架，不直接宣稱已完成成果，而是列出後續必須被驗證的對照項與預定結論方向。
...
本版已先將 `v1` 之核心責任邊界定清，包括 AI Search 僅負責檢索、回答生成由自建 Agent 流程掌控、`getDocumentChunk` 改以 `source_chunks.id` 作為可回放 `citationId`、Web 與 MCP 上下文分層處理、`restricted` scope 與 Cloud fallback 的治理原則，以及查詢與訊息落地前必須先完成遮罩的記錄規範。
...
補充說明：`GetDocumentChunkOutput` 的 `sourceLocator` 為 best-effort 欄位；若供應商未提供頁碼、標題路徑或穩定段落定位資訊，該欄位可省略。`v1` 保證的是片段可回放，而不是所有定位欄位都一定存在。
...
本附錄在 `v0.0.17` 先建立 15 筆種子案例，後續於正式驗證前擴充至 30–50 筆。
