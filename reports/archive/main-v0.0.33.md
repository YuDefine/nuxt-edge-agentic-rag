...
目前尚未完成正式圖表、實作畫面、測試資料與答辯支撐材料，因此後續需依本文規格逐項回填，並在實作階段再次核對官方平台能力與實際限制。

### 實作前優先補強重點

後續應優先完成以下項目：

1. 先補齊附錄 B 的 `Acceptance ID`、`gold facts`、必要引用、不可犯錯與預期 `http_status`，並凍結 `seed`、`dev-calibration` 與 `frozen-final` 的使用邊界。
2. 先把對話刪除、`messages.content_text` 清理、180 天稽核保留與 `citationId` 全域唯一性等治理語意封口，避免實作中途重改資料模型。
3. 優先完成圖 1-1 至圖 2-4 的正式繪製，先補功能圖、架構圖、活動圖、ER 圖與甘特圖等會直接影響實作溝通的圖面。
4. 答辯簡報、展示素材與報告主文一致化；第三章介面截圖待核心閉環真正跑通後再回填。
