...
本文暫保留圖表索引位置。正式繪圖與截圖回填時，應同步更新圖號、圖表目錄、第三章圖說與第四章驗收證據，以維持圖文一致。
...
| TC 編號 | Acceptance ID | 適用通道 | 實際結果摘要 | 是否通過 | `http_status` | judge | Self-Correction | 引用／拒答證據 | `config_snapshot_version` |
| ------- | ------------- | -------- | ------------ | -------- | ------------- | ----- | --------------- | -------------- | ------------------------- |
...
本附錄先建立 20 筆 `seed` 案例，供欄位檢查與早期 dry run 使用；正式驗收前應另建立 30–50 筆 `frozen-final` 測試集。若需校準門檻、prompt 或模型路由，應使用獨立 `dev-calibration` 案例，不得以 `frozen-final` 反覆調參；答辯展示案例 `defense-demo` 可自 `frozen-final` 挑選，但不得因展示需求回改驗收規則。每筆案例至少需定義適用通道、gold facts、必要引用、不可犯錯與預期 `http_status`。
