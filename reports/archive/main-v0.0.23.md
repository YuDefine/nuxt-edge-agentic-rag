![image1](main-v0.0.23_assets/image1.jpeg)
...
9. **Smoke retrieval 映射驗證**：遠端同步回報完成後，任務與版本先進入 `smoke_pending`。系統需以維運用查詢路徑針對目標 `document_version_id` 執行 representative smoke retrieval，確認候選片段可被取回，且皆可映射至既有 `source_chunks`。若無法建立可回放 `citationId`，則視為驗證失敗。
...
- `v1.0.0` 核心驗收資料集優先支援 `md`、`txt`，以及可先轉為 Markdown 並經人工校閱之文件；`pdf`、`docx` 視轉檔品質列為條件支援。
...
- `frozen-final`：30–50 筆正式驗收集，凍結後不得再改 threshold、prompt、route 或題目標註規則；若需調整，應建立下一版驗收集並重跑。
...
補充說明：`documents.tags` 保留於後台管理與未來延伸，但不納入 `v1.0.0` MCP 對外檢索契約，也不同步至 AI Search custom metadata。內部分數、`documentVersionId` 與授權判定細節屬內部診斷資料，不列為對外穩定欄位。
