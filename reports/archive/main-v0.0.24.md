![image1](main-v0.0.24_assets/rId18.jpg)
...
在問答決策上，v1.0.0 採固定責任分工：Query Normalization 僅做規則式標準化；AI Search 第一輪檢索可使用 rewrite_query；僅在邊界區間才由部署當下可用之較強推理模型執行 answerability judge 與 Query Reformulation；單文件明確回答預設由邊緣回答模型生成，跨文件整合則交由較強整合模型處理。實際模型名稱於 Preview 驗證通過後才鎖定至部署設定與報告，以避免平台更新造成論文與實作脫鉤。[6][27] 驗證面將以分層資料集檢驗回答正確率、拒答精準率、引用正確率、MCP Tool 成功率、current-version-only 檢索正確性與遮罩完整性，作為後續實作、截圖回填與正式報告收斂基準。[18][19]

關鍵字：代理式檢索增強生成（Agentic RAG）、邊緣原生架構（Edge-Native）、Cloudflare AI Search、Self-Correction、Model Context Protocol（MCP）、規格驅動開發（SDD）

# 目錄

TOC \o "1-3" \h \z \u請在 Word 中更新目錄
...
• 應呈現內容： 使用者端：自然語言問答、對話歷史、引用查看、拒答提示

• 管理後台：文件 CRUD、版本管理、查詢紀錄檢視、AI Search 同步、MCP token 管理

• Agentic 核心：置信度評估、Query Reformulation、Self-Correction、拒答判斷

• MCP 介面：searchKnowledge、askKnowledge、getDocumentChunk、listCategories

• 以虛線標示 v1.1 延伸：Passkey、MCP Session 與條件式 Cloud fallback
...
• 應呈現內容： 前端層：Nuxt 4、Nuxt UI、@ai-sdk/vue useChat 串流介面

• 資料與受管理檢索層：Cloudflare Workers、NuxtHub、Drizzle ORM、D1、R2、KV、Cloudflare AI Search

• Agentic AI 層：Vercel AI SDK、workers-ai-provider、部署時鎖定之回答模型與判斷／整合模型

• MCP 層：Nuxt MCP Toolkit、Middleware、Bearer Auth

• 右側以虛線標示 v1.1 延伸：Passkey、MCP Session 與條件式 Cloud fallback

• 以單一 Cloudflare Edge 邊界框包覆四層，強調邊緣優先與核心版不預設依賴額外跨雲 LLM API
...
• MCP 層：以 @nuxtjs/mcp-toolkit 建立 MCP 端點，透過 Middleware 驗證 Bearer token 與 scope。v1.0.0 的 MCP 採無狀態呼叫，不建立 MCP-Session-Id 依賴；Web 對話與 MCP 工具契約因此分別對應「D1 持久化對話輔助」與「單次請求契約」。若 v1.1 導入多輪上下文，runtime state 仍應保存於 KV，而非與 Web 對話混寫。[4][15][16]
...
| 引用回放   | 確認驗收資料集的候選片段可穩定映射至既有 source_chunks                                                   | 用於驗收與答辯的 current 文件皆能通過 smoke retrieval 與 citationId 回放                             | 核心驗收資料集先限 md、txt 或預先轉 Markdown 之文件；pdf、docx 改為條件支援                    |
...
• 邊界區間 answerability judge

• 分支 A：直接回答

• 分支 B：Query Reformulation 後重試一次

• 分支 C：拒答

• 既有 source_chunks 查找、citation_records 組裝與串流輸出

• 圖中應明示： Query Normalization 不呼叫模型
...
此處 distinct_verified_document_count 必須由 D1 依 document_version_id 解析回 document_id 後計算；由於 document_id 不列入 AI Search custom metadata，上述文件多樣性判斷不可直接依賴供應商回傳欄位。
...
• 實體應至少包含： better_auth_users（better-auth 核心表，以淡色或虛線表示）
...
| smoke_test_queries_json   | json                                                             | 由前處理產生之代表性 smoke probes；供發布前檢索與映射驗證使用                     |
...
5. smoke retrieval 的目的不是列舉供應商所有 chunk，而是驗證 AI Search 實際回傳之候選片段能否映射到既有 source_chunks。凡 smoke_test_queries_json 中通過權限與分數過濾的候選片段，皆必須能成功映射，否則該版本不得發布。
...
• smoke retrieval 屬維運用驗證流程，需以目標 document_version_id 的 smoke_test_queries_json 執行候選片段檢查，並確認可建立 source_chunks 映射；若目標版本原先尚未 indexed，驗證失敗時任務與版本狀態皆應標記為 failed；若屬已 indexed 版本之 maintenance reindex，僅同步任務標記為 failed，版本仍維持最近一次可服務之 indexed 狀態。
...
| citations_json   | json                                                   | 行內引用對應資料；至少需含 citationId 與當次回答依據之 document_version_id                      |
...
| POST   | /api/documents/:id/reindex  | 對既有版本觸發同版重同步與映射驗證                  | Admin |
...
9. Smoke retrieval 映射驗證：遠端同步回報完成後，任務與版本先進入 smoke_pending。系統需以 smoke_test_queries_json 針對目標 document_version_id 執行 representative smoke retrieval，確認各 probe 的有效候選片段可被取回，且皆可映射至既有 source_chunks。若無法建立可回放 citationId，則視為驗證失敗。

10. 發布版本：僅當新版本 smoke retrieval 與映射驗證通過後，才可將 document_versions.index_status 標為 indexed，並以單一 transaction 把新版本切為 is_current = true、寫入 published_at / published_by，同時將舊版降級為歷史版本。此步驟受「每份文件僅一個 current 版本」之唯一約束保護。

11. 正式檢索規則：只有 documents.status = active、document_versions.index_status = indexed、document_versions.is_current = true 的內容可進入正式回答流程。

12. 一致性保護：AI Search metadata 僅為第一層快篩與觀測；回答前一律以 D1 post-verification 剔除非 active/indexed/current 片段，並丟棄無法映射到 source_chunks 的候選片段。若剔除後已無有效證據，則視為無結果，不得回退到舊版內容。

13. 下架文件：將 documents.status 設為 archived，並由應用層檢索過濾立即停止對外回答；後續同步再讓 AI Search 反映最新狀態。
...
• rich format 文件（例如 pdf、docx）若超出 Cloudflare AI Search 當前公開限制，應在上傳前提示管理員改傳 Markdown/TXT，或先經應用層轉換後再同步；答辯核心資料集不得把供應商自動轉檔當成唯一依賴路徑。以 2026-04 查核時，官方公開 rich format 上限已提升至 4 MB。[26]
...
• rich format 若要納入正式驗收，必須先在 seed 與 dev-calibration 證明 smoke probes、引用映射率與 getDocumentChunk 回放皆穩定，再升級進入 frozen-final。
...
• 里程碑： M1：專案初始化、NuxtHub 部署、D1 Schema（W1–W2）
...
• Admin 不採首位註冊者自動升權；管理員名單真相來源為部署環境變數 ADMIN_EMAIL_ALLOWLIST，避免部署初期產生權限歧義。所有 Admin 專屬操作於授權時仍須依目前 Session email 重新比對 allowlist，不得僅依賴既有 D1 角色快照。
...
• seed：15 筆，供欄位檢查、早期 dry run 與流程走通，不納入正式統計。
...
本章在 v0.0.23 中定位為「實作成果回填骨架」。目的是先定義後續必須蒐證的欄位與畫面證據，不預先鎖死最終版面、文案或套件版本；凡表格內標示「待於建置時鎖定」或「依實際版本回填」者，應以第一次可重現部署成功之版本為準，並同步寫入部署設定與 query_logs.config_snapshot_version。
...
• 若 retrieval_score >= 0.70 且 cross_document_gate_failed = false：依固定模型路由以 models.defaultAnswer 或 models.agentJudge 生成回答 → 將有效候選片段映射至既有 source_chunks → 建立 citation_records → 串流輸出 → 儲存遮罩後日誌
...
| 權限不足查詢          | 以未具 knowledge.restricted.read 的 token 查詢 restricted 文件 | 直接回 403，不包裝為拒答                                                      | < 800ms      |
...
本節於實作完成後回填。除情境彙總表外，另需保留按 TC-xx 填寫的逐案結果表與 Acceptance ID 對照表，確保第三章數據可逐項回對第四章驗收命題。正式表格欄位如下：
...
本章在 v0.0.23 中作為研究結果與結論回填骨架。現階段只先列出後續必須被證據支持的驗收命題與技術特色，避免在實測前先寫死結論。
...
2. 附錄 B 測試集由 15 筆 seed 案例擴充為 30–50 筆 frozen-final 正式案例。
...
補充說明：documents.tags 保留於後台管理與未來延伸，但不納入 v1.0.0 MCP 對外檢索契約，也不同步至 AI Search custom metadata。內部分數、documentVersionId 與授權判定細節屬內部診斷資料，不列為對外穩定欄位。若查無任何通過授權與 D1 驗證的有效片段，應回傳 200 與 results: []，不以 404 表示「沒有命中」。
...
補充說明：GetDocumentChunkOutput 的 sourceLocator 為 best-effort 欄位；若供應商未提供頁碼、標題路徑或穩定段落定位資訊，該欄位可省略。v1.0.0 保證的是片段可回放，而不是所有定位欄位都一定存在。getDocumentChunk 回放的是當次已被引用之版本快照，不等同於再次查詢 current 版本。
...
本附錄在 v0.0.23 先建立 15 筆 seed 種子案例，供欄位檢查與早期 dry run 使用；正式驗收前應另建立 30–50 筆 frozen-final 測試集。若需校準門檻、prompt 或模型路由，應使用獨立 dev-calibration 案例，不得以 frozen-final 反覆調參；答辯展示案例 defense-demo 可自 frozen-final 挑選，但不得因展示需求回改驗收規則。每筆案例至少需定義適用通道、gold facts、必要引用、不可犯錯與預期 http_status。
...
| TC-13 | 權限治理        | MCP       | 以未具 knowledge.restricted.read 的 token 查詢 restricted 文件         | 403            | 必須在回答生成前阻擋，且不得洩漏 restricted 摘錄                             | 不可取得 restricted 內容   | 無                   | 驗證 scope 過濾                                             |
...
正式擴充 frozen-final 時，至少還需補入 searchKnowledge、listCategories、OAuth／allowlist、current／archived 切換、rate limit 與 no-internal-diagnostics 等非問答契約案例。
