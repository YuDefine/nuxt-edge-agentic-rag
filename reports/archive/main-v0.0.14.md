...
- 資料與受管理檢索層：以 R2 儲存原始文件與版本檔，D1 儲存結構化資料，KV 作為快取與 MCP Session 的 unstorage 驅動。Cloudflare AI Search 連接既定資料來源後，負責 Markdown 轉換、分塊、Embedding、query rewriting、reranking 與 retrieval；應用層則再以 metadata filter 強制套用 `status = active`、`is_current = true` 與可見 `access_level`，避免歷史版本、下架文件或未授權內容被納入回答。[7][15]
...
本系統規劃於 AI Search 自訂中繼資料中固定保留七個欄位：`document_id`、`document_version_id`、`version_no`、`category`、`status`、`access_level`、`is_current`。此設計用於支援分類篩選、版本追蹤、current-version-only 檢索、文件狀態控制與 Cloud fallback 前的資料治理判斷。
...
3. **權限、敏感資料與複雜度檢查**：在任何模型推論前，先對查詢進行敏感資料檢測，推導本次 `allowed_access_levels`，並標示問題屬於簡單事實查詢、模糊查詢、跨文件比較或多輪追問。
4. **第一輪 AI Search 檢索**：呼叫 AI Search `search` 路徑，只取回片段不直接生成回答。預設參數為 `retrieval_type = hybrid`、`max_num_results = 8`、`match_threshold = 0.35`、`reranking.enabled = true`、`rewrite_query = true`，並額外強制套用 `status = active`、`is_current = true`、`access_level in allowed_access_levels`。
5. **第一階段置信度評估**：系統先以 `top1_score`、`mean_top3_score` 與 `evidence_coverage` 計算 `retrieval_score`。
6. **直接回答條件**：若 `retrieval_score >= 0.70`，則不再呼叫 judge，直接進入回答生成，由 Llama 4 Scout 或 Kimi K2.5 依查詢複雜度產生回答。
7. **邊界區間 judge**：若 `0.45 <= retrieval_score < 0.70`，則由 Kimi K2.5 進行一次 answerability judge，合成最終 `confidence_score`。
8. **Self-Correction 條件**：若 `confidence_score < 0.55` 或 `retrieval_score < 0.45`，且 `retry_count = 0`，並且至少存在一筆弱相關片段可供重寫，則由 Kimi K2.5 重寫查詢，保留原始限制條件與關鍵實體，再重新呼叫 AI Search 檢索一次。第二輪重試停用 AI Search `rewrite_query`，避免雙重改寫失真。
9. **拒答條件**：若第二輪後仍 `confidence_score < 0.55`，或檢索結果無足夠引用，或唯一可用證據皆屬未授權 `restricted` 內容，則回傳拒答結果與補充建議。
10. **條件式 Cloud fallback**：僅當 `CLOUD_FALLBACK_ENABLED = true`、治理條件全部通過、且第二輪後仍有部分可用 `internal` 證據但不足以在邊緣完成高品質整合推理時，才以 Claude Sonnet 4.6 作為預設外部模型重新生成回答。
11. **引用組裝與記錄**：系統先依 `document_version_id + chunk_hash` upsert `source_chunks`，再建立本次查詢的 `citation_records`、寫入遮罩後 `query_logs`，並將回答以串流方式輸出。

`retrieval_score` 由以下三項組成：

| 構成項目            | 說明                                             | 權重 |
| ------------------- | ------------------------------------------------ | ---- |
| `top1_score`        | rerank 後第一名片段之分數正規化                  | 0.45 |
| `mean_top3_score`   | 前三名片段平均分數正規化                         | 0.35 |
| `evidence_coverage` | 可用引用數量、跨文件證據完整性與 metadata 一致性 | 0.20 |
...
| 欄位                | 類型                               | 說明                                                          |
| ------------------- | ---------------------------------- | ------------------------------------------------------------- |
| id                  | string (PK)                        | 穩定 `citationId`，由 `document_version_id + chunk_hash` 決定 |
...
| chunk_index         | integer                            | 該片段於版本中的順序（若可取得）                              |
...
| content_redacted | text                                 | 經遮罩後保存的訊息內容 |
...
3. **Web 對話持久化**：由 `conversations/messages` 保存 Web 問答歷史，作為 Web 多輪上下文唯一真相來源。
...
| `rewrite_query`     | `true`                                                                          |
| metadata filters    | `status = active`、`is_current = true`、`access_level in allowed_access_levels` |
...
| `retrieval_score < 0.45`         | 若有弱相關片段則進入 Self-Correction，否則直接拒答  |
...
- `retry_count = 0`
- 查詢不屬於明確越界問題
- 已取得至少一筆弱相關片段或存在明確遺漏實體，值得重寫查詢再試一次
...
3. **寫入版本資料**：建立 `document_versions` 紀錄，保存 `checksum`、`mime_type`、`size_bytes`、`is_current = false` 與預期的 AI Search metadata。
4. **發起同步**：建立 `ingestion_jobs`，觸發 instance 級同步，等待 AI Search 完成索引。
5. **寫回索引狀態**：同步完成後回寫 `ai_search_file_id`、`index_status` 與 `indexed_at`。
6. **發布版本**：僅當新版本 `index_status = indexed` 且 smoke retrieval 通過時，才將新版本標為 `is_current = true`，並把舊版轉為歷史版本。
7. **正式檢索規則**：只有 `documents.status = active`、`document_versions.index_status = indexed`、`document_versions.is_current = true` 的內容可進入正式回答流程。
8. **下架文件**：將 `documents.status` 設為 `archived`，並由應用層檢索過濾立即停止對外回答；後續同步再讓 AI Search 反映最新狀態。
...
- Admin 可於 Web 後台管理 `internal` 與 `restricted` 文件；MCP 則由 token scope 控制是否可讀 `restricted` 內容。
...
- 原始 token、祕密字串與高風險輸入只存在於單次請求記憶體；`query_logs`、`messages` 與除錯輸出僅保存遮罩後版本。
...
| 供應商 chunk ID 不適合作為公開契約   | reindex 後可能變動，直接外露不利相容性 | 以應用層 `source_chunks.id` 作為穩定 `citationId`，供應商 ID 僅內部保存                                |
...
| Current-Version Retrieval Accuracy  | 回答僅引用 `is_current = true` 且 `status = active` 版本之比例 | 100%     |
...
- 另以人工檢查比對 `source_chunks`、`citation_records`、`document_versions.is_current` 與 `query_logs.redaction_applied`，驗證引用穩定性與記錄治理。

---

# 第三章 實作成果

本章在 `v0.0.14` 中定位為「預定實作成果與展示規格」。所有內容皆作為後續實作、截圖與資料回填的正式骨架，並不宣稱目前已完成實測。
...
Admin 建立文件主檔 → 上傳原始檔至 R2 → 建立 `document_versions`（預設 `is_current = false`）→ 寫入 AI Search metadata（含 `document_version_id`、`is_current`）→ 建立 `ingestion_jobs` → 觸發 instance 級同步 → AI Search 完成轉換、分塊、Embedding 與索引 → 回寫 `ai_search_file_id`、`index_status`、`indexed_at` → smoke retrieval 驗證通過 → 將新版本切為 `is_current = true` → 文件可供正式檢索

#### 問答流程

使用者提問 → Query Normalization → 權限、敏感資料與複雜度檢查 → AI Search 第一輪檢索（`rewrite_query = true`，且 `status = active`、`is_current = true`）→ 計算 `retrieval_score` →

- 若 `retrieval_score >= 0.70`：直接以 Llama 4 Scout 或 Kimi K2.5 生成回答 → upsert `source_chunks` → 建立 `citation_records` → 串流輸出 → 儲存遮罩後日誌
...
- 版面配置：
  ...
  - 右欄：本次問答的 `retrieval_score`、`confidence_score`、是否觸發 judge / Self-Correction、模型路由與 fallback 狀態
- 內容要求：
  - 回答文字需含 `[1][2]` 行內引用
...
| 權限不足查詢 | 以未具 `knowledge.restricted.read` 的 token 查詢 restricted 文件 | 不回傳 restricted 引用；必要時拒答或回 403 | < 700ms     |
...
5. `is_current` 過濾、`restricted` scope 與記錄遮罩的驗證摘要。

---

# 第四章 結論

本章於 `v0.0.14` 中作為驗收與結論撰寫骨架，不直接宣稱已完成成果，而是列出後續必須被驗證的對照項與預定結論方向。
...
5. **治理前置**：Cloud fallback 預設關閉，`restricted` scope、版本發布規則與記錄遮罩在規格階段即明確定義。
...
本附錄在 `v0.0.14` 先建立 13 筆種子案例，後續於正式驗證前擴充至 30–50 筆。
...
| TC-13 | 權限治理   | 以未具 `knowledge.restricted.read` 的 token 查詢 restricted 文件     | `refused` 或 403                 | 不可取得 restricted 內容   | 無或僅 internal 引用                | 驗證 scope 過濾  |
