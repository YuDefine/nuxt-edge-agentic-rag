...
- 資料與受管理檢索層：以 R2 儲存原始文件與版本檔，D1 儲存結構化資料，KV 作為快取與 MCP Session 的 unstorage 驅動。Cloudflare AI Search 連接既定資料來源後，負責 Markdown 轉換、分塊、Embedding、query rewriting、reranking 與 retrieval；應用層則再以 metadata filter 強制套用 `status = active`、`version_state = current` 與可見 `access_level`，避免歷史版本、下架文件或未授權內容被納入回答。[7][15]
...
4. **第一輪 AI Search 檢索**：呼叫 AI Search `search` 路徑，只取回片段不直接生成回答。預設參數為 `max_num_results = 8`、`ranking_options.score_threshold = 0.35`、`rewrite_query = true`，並額外強制套用 `status = active`、`version_state = current`、`access_level in allowed_access_levels`。若實作採用 provider 或 binding 封裝，參數名稱可依 SDK 調整，但語意必須與此處一致，不再另立第二套契約名稱。
...
10. **條件式 Cloud fallback**：僅當 `CLOUD_FALLBACK_ENABLED = true`、治理條件全部通過、且第二輪後仍有部分可用 `internal` 證據但不足以在邊緣完成高品質整合推理時，才以 Claude Sonnet 4.6 作為預設外部模型重新生成回答。Cloud fallback 在 `v1` 中明確定位為 `synthesis-only` 路徑：不補抓新證據、不放寬權限篩選、不擴張檢索範圍，只基於已核可的引用摘錄完成整合與表述。
11. **引用組裝與記錄**：系統先依 `document_version_id + locator_hash + chunk_hash` upsert `source_chunks`，再建立本次查詢的 `citation_records`、寫入遮罩後 `query_logs`，並將回答以串流方式輸出。
...
| `evidence_coverage` | 由可用引用數量、跨文件覆蓋度與 metadata 一致性計算 | 0.20 |
...
metadata_consistency =
  all_results_match_required_filters ? 1 : 0
...
上述定義的目的，是讓 `retrieval_score` 在 `v1` 階段即具備可重現、可測試、可比對的計算方式，避免不同實作者各自詮釋分數來源。
...
| content_text     | text                                 | Web 對話原文，供多輪上下文延續與使用者本人查看 |
...
3. **Web 對話持久化**：由 `conversations/messages.content_text` 保存 Web 問答原文，作為 Web 多輪上下文唯一真相來源；`content_redacted` 僅供審計與統計使用。
...
6. **發布版本**：僅當新版本 `index_status = indexed` 且 smoke retrieval 通過時，才將新版本標為 `is_current = true`，並把舊版轉為歷史版本。此步驟必須以單一 transaction 完成，並受「每份文件僅一個 current 版本」之唯一約束保護。
...
- 原始 token、祕密字串與高風險輸入只存在於單次請求記憶體；`query_logs` 與除錯輸出僅保存遮罩後版本。`messages.content_text` 僅保存對話延續所需原文，`messages.content_redacted` 才作為審計副本。
...
- 建立 30–50 筆正式測試集，並先以 13 筆種子案例作為開發前驗證骨架。
...
本章在 `v0.0.15` 中定位為「預定實作成果與展示規格」。所有內容皆作為後續實作、截圖與資料回填的正式骨架，並不宣稱目前已完成實測。
...
Admin 建立文件主檔 → 上傳原始檔至 R2 → 建立 `document_versions`（預設 `is_current = false`）→ 寫入 AI Search metadata（含 `document_version_id`、`version_state = candidate` 與 `folder` 路徑）→ 建立 `ingestion_jobs` → 觸發 instance 級同步 → AI Search 完成轉換、分塊、Embedding 與索引 → 回寫 `ai_search_file_id`、`index_status`、`indexed_at` → smoke retrieval 驗證通過 → 以 transaction 將新版本切為 `is_current = true` 並同步 `version_state = current` → 文件可供正式檢索

#### 問答流程

使用者提問 → Query Normalization → 權限、敏感資料與複雜度檢查 → AI Search 第一輪檢索（`rewrite_query = true`，且 `status = active`、`version_state = current`）→ 計算 `retrieval_score` →
...
本章於 `v0.0.15` 中作為驗收與結論撰寫骨架，不直接宣稱已完成成果，而是列出後續必須被驗證的對照項與預定結論方向。
...
2. Appendix B 測試集由 13 筆種子案例擴充至 30–50 筆正式案例。
...
本附錄在 `v0.0.15` 先建立 13 筆種子案例，後續於正式驗證前擴充至 30–50 筆。
