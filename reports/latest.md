...
2026-04-23 晚間的下一輪 hotfix 已把可疑面再縮小到 Better Auth catch-all router 與 Worker body 物件邊界：新增 app 自己的精準 route [server/api/auth/passkey/verify-authentication.post.ts](/Users/charles/offline/yuntech-project/repo/nuxt-edge-agentic-rag/server/api/auth/passkey/verify-authentication.post.ts:1)，不再讓這條路徑走 `@onmax/nuxt-better-auth` 注入的 `/api/auth/**` 通用 handler，而是直接在 Nitro route 內呼叫 `auth.api.verifyPasskeyAuthentication({ asResponse: true, headers, body })`。同時新增 [server/utils/passkey-verify-authentication.ts](/Users/charles/offline/yuntech-project/repo/nuxt-edge-agentic-rag/server/utils/passkey-verify-authentication.ts:1) 與對應單元測試 [test/unit/passkey-verify-authentication.test.ts](/Users/charles/offline/yuntech-project/repo/nuxt-edge-agentic-rag/test/unit/passkey-verify-authentication.test.ts:1)，先在 app 邊界驗證 payload 至少具有 `response` record，並把 top-level `response` materialize 成 plain object 後再轉交 Better Auth。後續在 `pnpm check` 的 typecheck 階段又暴露出另一個邊界：`serverAuth(event)` 的 `AuthInstance` 型別未顯式暴露 `verifyPasskeyAuthentication`，因此 helper 已再收斂成 runtime guard，只有在 passkey feature flag 與 RP 設定都完整時才開放這條 override route；若 route 已啟用但 direct endpoint 缺失，則明確回 `503 Passkey authentication unavailable`，避免把「module catch-all 被覆蓋」誤演成新的 silent 500。再下一輪 CI 對齊驗證又抓到 Node unit runner 會在載入 util 時找不到 `h3` bare import，因此 `h3` 錯誤包裝已回收至 route handler，util 改丟自家 `PasskeyVerifyAuthenticationRouteError`，避免 test-only module resolution 把 release 擋在 CI gate 之前。`pnpm check`、`pnpm test test/unit/passkey-verify-authentication.test.ts`、`pnpm test:unit`、`pnpm build` 目前都已通過，且本地 preview 實測 `POST /api/auth/passkey/verify-authentication` 對空 JSON 會優先命中新 route 並回 `400 Passkey authentication payload invalid`，證明 route precedence 仍成功覆蓋 module catch-all；同時 feature 關閉時的既有 `404` 語意也已由 guard 保留。這版尚未 deploy，因此 production `a14.ownKeys...` 是否解除仍待下一次發布後驗證。
