...
在問答決策上，本系統採分段式信心判斷：Query Normalization 僅做規則式標準化；第一輪檢索可使用 query rewrite；僅於邊界區間才由 answerability judge 與 Query Reformulation 介入；若證據仍不足則拒答。驗證面以分層資料集、整合測試、引用回放與權限稽核為主，聚焦回答正確性、拒答精準性、引用正確性、MCP Tool 契約穩定性、current-version-only 檢索正確性與遮罩完整性。Web 對話與 MCP 無狀態契約分流；核心驗收資料以 `md`、`txt` 與預先轉 Markdown 且經人工校閱之文件為主，並以本版已完成之介面截圖、自動化驗證結果與部署治理設計作為答辯依據。
...
表 2-10 conversations 資料表（Drizzle schema 已定義，正式 migration 於後續治理深化階段補齊）.....................................
...
| 記錄治理 | `messages.content_redacted`、`query_logs.risk_flags_json` / `redaction_applied` / `status`（現階段無 `content_text`、`message_state` 欄位，原文零落地為結構性保障） | 高風險輸入不得以原文落地，也不得回到後續模型上下文；`content_text` / `message_state` 屬後續治理深化階段擴充 |
...
- **實作狀態**：`conversations` 資料表之 Drizzle schema 已於 `server/db/schema.ts` 宣告；對應 migration 與 `/api/conversations` CRUD 保留至後續治理深化階段補齊。在此之前，`/api/chat` 暫不接受 `conversationId`；現階段 Web 問答皆為單輪、無持久化對話歷史。
...
- 現階段**不保留原文欄位 `content_text`**：任何寫入 `messages` 的內容都先經遮罩管線處理並寫入 `content_redacted`，形成「原文零落地」的結構性保障，直接滿足 A11。`message_state = persisted / redacted_only / rejected_marker` 的三態語意改由 `query_logs.status` 與 `risk_flags_json` 承擔，`messages` 層不再細分。
...
- Web 多輪上下文、`conversation_id` 串接與 stale 判定列為後續治理深化階段交付；屆時另以獨立 migration 加上 `conversation_id`、`content_text`、`message_state`、`citations_json`、`model_name`、`metadata_json` 欄位，並建立 stale 重算流程。
...
| 方法   | 路徑                                                      | 說明                                                                                                                                             | 權限  |
| ------ | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ----- |
| POST   | `/api/chat`                                               | Web 問答；現階段為單輪、非串流（一次性回傳 `{ answer, citations, refused }`）                                                                    | User  |
...
| 方法   | 路徑                                              | 說明                                                | 對應擴充階段         |
| ------ | ------------------------------------------------- | --------------------------------------------------- | -------------------- |
| GET    | `/api/conversations`                              | 取得對話列表                                        | 後續治理深化階段     |
| GET    | `/api/conversations/:id`                          | 取得單一對話詳情                                    | 後續治理深化階段     |
| DELETE | `/api/conversations/:id`                          | 刪除對話                                            | 後續治理深化階段     |
...
- 現階段的 Web `/api/chat` 本身也是單輪無狀態（`conversationId` 保留至後續治理深化階段補齊）；MCP 更是明確拒絕 `conversationId` 與 `MCP-Session-Id` 於 header / body 中出現，若偵測到將直接回 `400`。
...
- 現階段不保留 Web 對話原文（`messages.content_text` 欄位與 `conversations` 刪除流程屬後續治理深化階段擴充）；所有 `messages` 內容皆以遮罩後 `content_redacted` 形式寫入，退出階段由 retention cleanup 一併清理。
...
- 原始 token 與祕密字串只存在於單次請求記憶體；`query_logs` 與除錯輸出僅保存遮罩後版本。由於 現階段的 `messages` 表不存在 `content_text` 欄位，任何進入 `messages` 的內容皆走遮罩管線寫入 `content_redacted`，這是「原文零落地」的**結構性保障**；高風險輸入額外在 `query_logs.status` 寫入 `blocked`、`risk_flags_json` 記錄命中規則、`redaction_applied` 記錄遮罩執行情況。
...
- 另以人工檢查比對 `source_chunks`、`citation_records`、`document_versions.is_current`、`query_logs.redaction_applied` 與 `messages.content_redacted` 皆已完成遮罩（現階段不存在 `content_text` 欄位，故該欄位漏遮罩為結構性不可能），驗證引用可回放性與記錄治理。
...
| EV-05    | A11                | `content_text` 欄位不存在之結構性保障        | migration schema dump、整合測試 `acceptance-tc-15` 命中時 `content_redacted` 寫入紀錄                                   | `messages` 表不存在 `content_text` 欄位（migration 0001~0008 合併 schema 無此欄位）且 `query_logs.status = 'blocked'` 寫入成功 | `server/database/migrations/0001_bootstrap_v1_core.sql`（無 `content_text` 欄位）、`test/integration/acceptance-tc-15.test.ts` |
...
| A11           | 高風險輸入不會以原文寫入持久化紀錄（**結構性保障：現階段 `messages` 表不存在 `content_text` 欄位**）   | 2.4.1、2.4.4  | TC-15、EV-05                      | migration schema + `messages.content_redacted` + `query_logs.status = 'blocked'` 稽核結果                         | 結構性保障（migration 0001 合併 schema 無 `content_text` 欄位；TC-15 覆蓋 blocked 路徑寫入）                 |
...
5. **Web 與 MCP 契約分流**｜*已達結構式驗證*：現階段 Web 與 MCP 皆為單輪無狀態契約，`messages` 表僅保留遮罩後 `content_redacted`；Web 對話多輪指代輔助保留至後續治理深化階段補齊，屆時補齊 `conversations` / `messages.content_text` 並維持 MCP 無狀態契約；MCP contracts 15 檔 51 tests 佐證。
...
2. 180 天稽核保留（`citation_records.expires_at` 直接承載 retention window）與 `citationId` 全域唯一性等治理語意已先封口，避免實作中途重改資料模型；對話刪除與 `content_text` 清理則保留至後續治理深化階段一併補上。
