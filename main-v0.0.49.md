...
此 workflow 現已包含可手動 dispatch 的 `deploy-staging` job；staging 之 build-time `NUXT_PUBLIC_SITE_URL` 由 `STAGING_SITE_URL` 注入，wrangler route 綁定 `agentic-staging.yudefine.com.tw`。deploy 後的 `smoke-test` / `smoke-test-staging` 共用 `scripts/check-deploy-health.mjs`；若 GitHub runner 僅因 Cloudflare WAF / Bot protection 回 `403`，workflow 會以 warning 記錄而不把 deploy 本體誤判為失敗。
