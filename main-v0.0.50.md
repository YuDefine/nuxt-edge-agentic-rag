...
此 workflow 現已包含可手動 dispatch 的 `deploy-staging` job；staging 之 build-time `NUXT_PUBLIC_SITE_URL` 由 `STAGING_SITE_URL` 注入，wrangler route 綁定 `agentic-staging.yudefine.com.tw`。deploy workflow 另已改為先驗證「同一個 commit SHA 已經有成功的 `.github/workflows/ci.yml` run」，避免在 deploy 內重複再跑一次 CI；deploy 後的 `smoke-test` / `smoke-test-staging` 共用 `scripts/check-deploy-health.mjs`，若 GitHub runner 僅因 Cloudflare WAF / Bot protection 回 `403`，workflow 會以 warning 記錄而不把 deploy 本體誤判為失敗。

#### D.2.3 Post-deploy 煙霧測試與 Tag 命名

部署完成後 30 秒內執行健康檢查：首頁 HTTP 200、`wrangler tail` 觀察 60 秒無非預期 5xx、以 Admin 身分進入管理介面確認列表載入。若 GitHub Actions 的 smoke test 僅顯示 runner 端 `403` 且已標記為 WAF warning，仍須從人工網路環境補做一次 canary，避免把外部防護誤判為站點異常；若任一項實際失敗且新版 code 無 schema 變更，應即時走 §D.3.1 應用層 rollback。
