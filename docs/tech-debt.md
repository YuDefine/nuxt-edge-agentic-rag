...
| TD-053 | `fix-user-profile-id-drift` production observation — 立即採樣 `wrangler tail --env production` 5-10 分鐘 + 撈最近 24h logs 搜 `user_profiles sync failed` 確認無預期外觸發                                                                                    | low      | open        | 2026-04-25 fix-user-profile-id-drift archive                     | —     |
...
- [ ] Manual QA 在 production 環境觀察 30s+ 延遲的 chat 不再觸發 `NetworkError / connection closed`（**post-archive follow-up；立即驗：本機開 production URL 發一條會觸發長 reasoning 的 query，確認 SSE stream 30s+ 不掉線**）
- [ ] 立即採樣 production logs（取代原「7 天觀察」）：`wrangler tail --env production --format json` 採樣 10-15 分鐘 + 撈 production D1 最近 24h `query_logs` 計算 `chat.error` 比例 vs v0.49.0 deploy 前同期。若比例無顯著上升即標 done（**post-archive follow-up**，移自原 add-sse-resilience tasks 7.3）
- [ ] 立即抽 production D1 最近 24h `query_logs` 10 條 chat run，比對 `first_token_ts - created_at` delta 落在合理區間（< 5s 為健康），確認 first-token-ts 對應第一個 `delta` event 未被 `: keep-alive` 行誤計（**post-archive follow-up**，移自原 add-sse-resilience tasks 7.4；驗證 design 內 evlog 不被 heartbeat 干擾的假設）
...
- [ ] 立即採樣 wrangler tail 5-10 分鐘 + 最近 24h evlog drain 結果並紀錄到此 entry
- [ ] Status 標 `done`（無 regression）或開新 change（有 regression）
