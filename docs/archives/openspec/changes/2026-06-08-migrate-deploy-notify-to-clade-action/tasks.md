...
- [x] 1.1 Replace the inline `Notify Discord` step with a two-step composition satisfying the "Notify job uses two-step composition" requirement: a "Build results JSON" pre-step (env-based, `id: build`) and a `uses: ./.github/actions/discord-deploy-notify` step
