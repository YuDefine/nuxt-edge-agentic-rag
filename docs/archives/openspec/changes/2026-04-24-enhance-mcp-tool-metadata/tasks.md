...
- [x] 3.3 以 MCP Inspector 或 Claude Desktop 連線，handshake 後檢查 `tools/list` 回傳含新 `description` / `annotations` / `inputExamples`，並確認 tool `name` 仍為 `askKnowledge` / `searchKnowledge` / `getDocumentChunk` / `listCategories` — 2026-04-24 PASS：production `https://agentic.yudefine.com.tw/mcp` 以真實 MCP bearer token 連線，4 個 tool name 穩定；field descriptions / annotations 皆正確；`askKnowledge` / `searchKnowledge` / `getDocumentChunk` 透過 `_meta.inputExamples` 回傳 2 / 2 / 1 筆 examples

## 4. 人工檢查

- [x] 4.1 使用者逐段 review 4 個 tool 的 field-level `.describe()` 文案，確認能幫助 LLM 理解 retrieval 語意與輸入限制 — 2026-04-24 PASS：production `tools/list` 實際回傳的 `query` / `citationId` / `includeCounts` descriptions 語意清楚，且包含 retrieval 行為與輸入限制
- [x] 4.2 使用者確認 `inputExamples` 樣本覆蓋典型使用情境（至少 specific-topic 與 category-flavored 兩種；不出現 PII 或敏感內容）— 2026-04-24 PASS：production `_meta.inputExamples` 覆蓋 specific-topic、category-flavored query 與 citation id placeholder，未含 PII
- [x] 4.3 使用者確認 `annotations` 值對齊 handler 實際行為（特別是 `openWorldHint` 是否反映「只查 governed knowledge corpus，不訪外網」）— 2026-04-24 PASS：4 個 tool 皆為 `readOnlyHint=true`、`destructiveHint=false`、`openWorldHint=false`、`idempotentHint=true`，符合 governed corpus read-only 行為
