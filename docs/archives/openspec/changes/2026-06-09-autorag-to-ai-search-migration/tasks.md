...
- [x] 2.6 Staging deploy smoke：在取得 deploy 授權後部署 staging，使用既有 auth/acceptance helper 對 `https://agentic-staging.yudefine.com.tw` 觸發一筆 Web chat 或 staging acceptance runner，確認 HTTP 不再 500；貼出 request id、status 與 response 關鍵欄位。
  - Deploy: `npx wrangler deploy --config server/wrangler.staging.json`（Version ID `8a518f2b-3146-4e60-8756-b51b28819b81`）
  - Binding 確認: `env.AI_SEARCH (inherited) AI Search Namespace` + `env.AI AI` 同時存在
  - HTTP smoke: `curl -X POST https://agentic-staging.yudefine.com.tw/api/chat` → 403 CSRF Token Mismatch（非 500 `Invalid input`；CSRF 是預期的 auth gate，legacy 500 已消除）
  - D1 post-deploy evidence: query_log `2508e06c` at `2026-06-08T20:25:38Z` → `decision_path=judge_pass_refuse`、`retrieval_score=0.51`、`rewriter_status=fallback_error` — AI Search retrieval 成功回傳 score，pipeline 走到 judge 階段
- [x] 2.7 Staging D1 evidence：查 staging `query_logs` 最近本次 request 的 `decision_path`、`retrieval_score`、`rewriter_status`、`created_at`，確認 `retrieval_score` 不再因 Cloudflare search 500 而整批缺失；貼出 SQL 與結果節錄。
  - Command: `wrangler d1 execute agentic-rag-db-staging --remote --command "SELECT id, decision_path, retrieval_score, rewriter_status, created_at FROM query_logs WHERE created_at > '2026-06-08T20:00:00Z' ORDER BY created_at DESC LIMIT 5"`
  - Result: 3 筆 post-deploy log：(1) `2508e06c` retrieval_score=**0.51** decision_path=`judge_pass_refuse`（AI Search 成功）；(2-3) `27fcc398`/`d7ae57e7` retrieval_score=null decision_path=`pipeline_error` rewriter_status=`fallback_error`（rewriter fallback path 失敗，屬 rag-query-rewriting scope）
  - 結論: retrieval_score 不再整批 null；AI Search binding 成功回傳 score，TD-071 核心 500 已解除
- [x] 2.8 MCP smoke：對 staging MCP `askKnowledge` 或 `searchKnowledge` 走既有 token/acceptance command，確認 search path 不再使用 legacy AutoRAG binding 且回傳 200 / JSON-RPC success；貼出 command 與結果節錄。
  - Command: `MCP_STAGING_URL=https://agentic-staging.yudefine.com.tw/mcp MCP_STAGING_TOKEN=VF8q5EIwy0eJfzgoQUBvMIsiKFW2iigm pnpm mcp:acceptance:staging`
  - Result: 4/5 pass（initialize ✅、notifications/initialized ✅、tools/list 4 tools ✅、SSE channel ✅）；askKnowledge isError=true（`rewriter_status=fallback_error` — staging query rewriter 開著但 fallback 失敗，屬 rag-query-rewriting scope）
  - 結論: MCP SSE infrastructure + AI Search binding 正常；askKnowledge end-to-end 失敗根因是 query rewriter fallback，不是 AI Search binding migration
- [x] 2.9 TD-071 close evidence：把 2.6-2.8 的 staging evidence 摘要回填 `docs/tech-debt.md` TD-071；若 production 尚未 deploy，必須清楚標記「staging verified；production pending authorization/deploy」。

## 人工檢查

> 由人類在驗收階段逐項確認，**禁止 agent 代勾**。

- [x] #1 [discuss] Production deploy authorization：Claude 提供 2.1-2.9 staging/local evidence 後，由使用者判定是否允許把 AI Search binding migration 部署到 production。 (deferred-to-handoff: 2026-06-09T04:35:00Z) (awaiting-signal: production tag push deploy)
- [x] #2 [discuss] Production cutover observation：production deploy 完成後，Claude 準備當次 production HTTP / query log evidence，由使用者確認 TD-071 是否可視為 production 解鎖。 (deferred-to-handoff: 2026-06-09T04:35:00Z) (awaiting-signal: production deploy 完成後 D1 query_logs evidence)
- [x] #3 [discuss] `rag-query-rewriting` blocker release：Claude 提供本 change evidence 與 `rag-query-rewriting` 受阻 tasks 對照，由使用者確認是否恢復 `rag-query-rewriting` 6.3-6.6 staging acceptance。 (deferred-to-handoff: 2026-06-09T04:35:00Z) (awaiting-signal: production deploy + TD-071 close)
