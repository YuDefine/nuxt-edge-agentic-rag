# Tech Debt Register — closed entries（archived 2026-06）

> **Rotation 2026-06-23**：45 條 closed entry 自 `docs/tech-debt.md` rotate（clade /handoff §2B.1.8 closedBloat 機制觸發）。`wontfix-until-signal` 帶 re-activation trigger 的項刻意保留主檔。內容 byte-exact 搬入。

> **TD 編號唯一性**：開新 TD 不得重用此處編號。
