...
目前資料集 12 筆，每次 `pnpm eval` 會對 Claude Sonnet 4.6 發出 12 次 tool-selection 請求。實際費用依 Anthropic 當期價格與 `tools/list` metadata token 數浮動；跑完後以 Anthropic Console usage 為準。若只想先檢查 wiring，先不要跑 `pnpm eval:watch`，避免 watch mode 重複觸發 LLM 呼叫。

新增 sample 前先確認它代表真實使用者提問，而不是為了讓模型猜答案的模板句；新增後也要更新本節的 sample count 與成本提醒。修改 MCP tool metadata、description 或 input schema 後，應手動跑一次 eval 並檢視 per-sample 結果。
