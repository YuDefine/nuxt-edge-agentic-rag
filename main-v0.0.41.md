...
為避免使用者（即使是 Admin）於高速操作時誤觸破壞性動作造成內容遺失或服務中斷，本畫面對五個操作按鈕依「風險等級」採取對應的確認策略；所有破壞性動作以 `LifecycleConfirmDialog` 元件統一呈現，顯示動作名稱、影響範圍（例如刪除時顯示將移除的版本與 chunks 數）與當前 Admin email 以供再次確認。
...
| 發布為 current | 高       | 二次確認 + 顯示將被替換的舊 current 版本        | 將指定版本切為 `current = true`，影響所有後續檢索與問答。Modal 顯示「舊 current: vN」→「新 current: vN+1」與既有引用版本降級警示。        |
| 重新同步       | 中       | 二次確認（若處於 `sync_status=syncing` 則阻擋） | 若 AI Search 同步失敗需重試；對正在 `syncing` 的版本則不可重複觸發，避免重複 index job。                                                  |
| 下架（刪除）   | 最高     | 輸入確認（需輸入文件標題才能送出）              | 下架會將 `documents.status` 改為 `archived` 並標記所有版本為不可檢索；若誤觸將影響所有引用。輸入確認避免點擊錯列產生誤刪。                |
