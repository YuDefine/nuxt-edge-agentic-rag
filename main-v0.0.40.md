...
本報告全文圖表合計 44 張，依編排規範列出圖表索引如下。頁碼以正式排版後為準。
...
- 以 Nitro 原生 event handler 實作 MCP Server 的 4 個核心 Tools，並統一透過 `Authorization: Bearer` + scope 控管；本專題 `v1.0.0` 採 Nitro-native 實作（4 個 endpoint + 6 支 util，已完成並具備測試覆蓋），遷移至 `@nuxtjs/mcp-toolkit` 已建立 Spectra change proposal（`openspec/changes/migrate-mcp-to-toolkit/`），列為 post-core 第一個 change，待 `bootstrap-v1-core-from-report` 與 `add-v1-core-ui` 歸檔後再行 apply；遷移前後均須維持同一組 scope 與治理契約。[4][15][16][17]
...
| 知識管理 | 一次性 signed URL 上傳至 R2、版本管理、`source_chunks` 預建、AI Search 同步、文件狀態管理、`is_current` 發布規則；核心驗收資料集優先採 `md`、`txt` 或預先轉 Markdown 之文件 | 後台統計摘要、進階營運檢視與 rich format 條件支援擴充             |
...
- 應呈現內容：
  ...
  4. MCP 層：Nitro 原生 event handler、Middleware、Bearer Auth；遷移至 `@nuxtjs/mcp-toolkit` 為同版後置第一個 change（見 `openspec/changes/migrate-mcp-to-toolkit/`），Cloudflare 原生 MCP 列為更後續的評估選項
  5. 右側以虛線標示後續延伸能力（含 Vercel AI SDK、workers-ai-provider、mcp-toolkit 遷移），避免與 `v1.0.0` 核心架構混寫
...
- MCP 層：以 Nitro 原生 event handler 建立 4 個核心 MCP 端點，透過統一 Middleware 驗證 Bearer token 與 scope。`v1.0.0` 的 MCP 採無狀態呼叫，不建立 `MCP-Session-Id` 相依性；Web 對話與 MCP 工具契約因此分別對應「D1 持久化對話輔助」與「單次請求契約」。若後續版本導入多輪上下文，runtime state 仍應保存於 KV，而非與 Web 對話混寫。遷移至 `@nuxtjs/mcp-toolkit` 已建立 Spectra change（`openspec/changes/migrate-mcp-to-toolkit/`），為同版後置第一個 change；前置研究已確認四項關鍵相容性：Cloudflare Workers 相容（官方 preset）、Inspector build-time gate 可保證 production 零洩漏、單層 middleware 可保證 auth → rate-limit → tool handler 順序，以及已知需搬檔至 `server/mcp/tools/` 並接受 URL 從 `/api/mcp/*` 統一為 `/mcp`。遷移前後均須維持同一組 scope 與治理契約；Cloudflare 原生 MCP 列為更後續的評估選項。[4][15][16]
...
| ---------- | -------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| 檢索介面   | 確認 AI Search 在目標環境可穩定提供搜尋 API 或 Workers binding，並回傳足以做 D1 post-verification 的欄位 | 可對指定 `document_version_id` 完成 smoke query，取得分數、必要 metadata 與可用候選片段              | 先保留 Web 問答、後台同步與引用回放能力為核心閉環；`searchKnowledge` / `listCategories` 等 MCP 對外包裝與進階 filter 可延後到核心檢索穩定後再接入 |
...
- Admin 專屬路由與後台操作在授權時，不得僅信任 `role_snapshot`；仍須以目前 Session email 對正規化 allowlist 重新判定，避免 allowlist 異動後殘留舊權限。
...
`v1.0.0` 之 Admin 後台除上傳與發布外，需補齊生命週期營運操作。下列操作在 `bootstrap-v1-core-from-report` 與 `add-v1-core-ui` 之外規劃為獨立 change `admin-document-lifecycle-ops`，以免污染核心閉環 scope。

驅動情境：staged upload wizard 中斷後，文件可能停留於 `documents.status = draft`、`current_version_id = NULL`、`document_versions.sync_status = pending`，現有 UI 無任何操作入口，Admin 只能直接進 D1 清理。以下設計以此類殘留復原為首要優先項。

表 2-22a 生命週期營運操作分級

| 操作                           | 對象                                                | 語意         | `v1.0.0` 範圍        | 理由                                                                             |
| ------------------------------ | --------------------------------------------------- | ------------ | -------------------- | -------------------------------------------------------------------------------- |
| Retry sync                     | `document_versions.sync_status IN (pending,failed)` | 重新觸發同步 | 納入                 | 直接解開半途失敗版本，避免 `documents` 與 `document_versions` 殘留               |
| Delete draft                   | `documents.status = draft` 且從未 `published_at`    | 硬刪除       | 納入                 | 從未對外、無 `citation_records`，cascade 清除 `document_versions` 與 chunks 安全 |
| Archive active                 | `documents.status = active`                         | 軟下架       | 規劃納入（時程允許） | 綁定既有 `documents.archivedAt` 欄位與 `documents.status = archived` 轉移        |
| Unarchive                      | `documents.status = archived`                       | 恢復         | 規劃納入（時程允許） | Archive 的逆操作；若 archive 入列則一併納入                                      |
| Hard delete published document | `documents.status IN (active,archived)`             | 物理刪除     | 不納入               | 破壞 `citation_records` 回放鏈，與 `retention-cleanup-governance` 衝突           |

設計決策：

- Hard delete 僅允許 `draft-never-published`。判定條件為「不存在任何 `document_versions.published_at IS NOT NULL` 的歷史版本」；若曾經發布過，即使當前為 draft，也只允許 archive。
- Published 內容不得硬刪除。若需停用對外服務，走 archive：將 `documents.status` 設為 `archived`、寫入 `archivedAt`，檢索過濾立即隔離；`source_chunks.chunk_text` 與 `citation_records` 保留至 `retention-cleanup-governance` 之 180 天 retention window 期滿後由 cleanup job 接手。
- Retry sync 不重建 `document_versions` 紀錄、不改 `index_status`，僅將 `sync_status` 由 `pending` 或 `failed` 重新轉為 `running`；若前處理尚未完成（`index_status = preprocessing`），retry 需先補齊 `normalized_text_r2_key` 與 `source_chunks`，否則不得推進。
- 權限邊界沿用 runtime admin middleware。`v1.0.0` 不引入額外角色層或二階段確認；破壞性操作（delete draft、archive）於 UI 以確認對話框提示影響範圍。
- API 表面規劃為 `server/api/admin/documents/**` 之 mutating endpoint：`POST …/versions/[versionId]/retry-sync`、`DELETE …/[id]`（draft only）、`POST …/[id]/archive`、`POST …/[id]/unarchive`。

表 2-22b 與既有治理 change 之關係

| 依賴對象                                              | 關係                                                                       |
| ----------------------------------------------------- | -------------------------------------------------------------------------- |
| `governance-refinements/retention-cleanup-governance` | 本 change 只負責產生 archive 狀態；`source_chunks.chunk_text` 清理由其負責 |
| `governance-refinements/config-snapshot-governance`   | 本 change 不引入新 threshold 常數                                          |
| `admin-ui-post-core`                                  | Token / query-log / dashboard UI 與本 change 平行，不互相阻擋              |
...
| MCP Module（後續升級）  | `@nuxtjs/mcp-toolkit`（已規劃遷移，見 Spectra change `migrate-mcp-to-toolkit`）／Cloudflare 原生 MCP（更後續評估） | 同版後置第一個 change                         | MCP Resources、Session、Evals                       |
...
4. **Web 與 MCP 契約分流**：`v1.0.0` Web 與 MCP 皆為單輪無狀態契約，`messages` 表僅保留遮罩後 `content_redacted`；Web 對話多輪指代輔助列為 `governance-refinements` 階段擴充，屆時補齊 `conversations` / `messages.content_text` 並維持 MCP 無狀態契約。
...
7. **分階段落地**：先以 `v1.0.0` 完成核心版，再把 Passkey、MCP Session 與 Cloud fallback 留作後續版本。
...
4. 生命週期營運操作（retry sync、delete draft、archive / unarchive）於人工驗收揭露為現有 UI 之殘留復原 gap，規劃為 `admin-document-lifecycle-ops` change。Priority 1（retry sync）與 Priority 2（delete draft）納入 `v1.0.0` 範圍，Archive 視時程允許納入；Hard delete published 明確不納入並交由 `retention-cleanup-governance` 承接。詳見 2.2.4.2。
5. 圖面、第三章介面截圖、第四章驗收狀態與答辯簡報屬回填項，待核心閉環真正跑通後再補，避免以假資料或示意版面反向綁架實作。
